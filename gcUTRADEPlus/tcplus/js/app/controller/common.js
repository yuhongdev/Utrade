...
function updateActivePanelTask(view, record, cidx, activateItem) {
    ...
    try {
        ...
        if(equityPrtfPanel){
        	if(grid.id == equityPrtfPanel.id){
        		ordRec.accbranchNo = equityPrtfPanel.accbranchNo;
                        ...
                        if (record) {
                            ordRec.payment = record.get('SettOpt'); // current payment mode 
                        }
        	}
        }
        	...
        	if (cell == fieldBuy || cell == 'Buy' || cell == fieldBqty) {
        		...
        		if (recData.OrdNo != null || recData.TktNo != null) {
        			...
        			if (sts) {
        				...
        				ordRec.trailType = recData.trailType;
        ...
        if(isMf(stockutil.getExchange(stkname))){
            //Force pass in ord minimum investment
            //Get exchg from stock code
            ordRec.minInvest = recData[fieldMinInvestment];
            ordRec.minRedeem = recData[fieldMinRedemptionQty];
        }
        
        // activate orderpad
        // all sync will affect orderpad
        if (mode && stkcode) {
        ...
        if (grid.type == "mf") {
            syncGroupManager.syncMutualFundComps(grid, stkcode, stkname);
        } else {
            syncGroupManager.syncAllComps(grid, stkcode, stkname);
        }
...
function calculationOfPortFolioDetail(quantity, unitPrice, transFee, action) {
    ...
    if (quantity != null && unitPrice != null && transFee != null) {
        quantity = jsutil.getNumFromFormat(quantity, 0);
        unitPrice = jsutil.getNumFromFormat(unitPrice, 0);
        transFee = jsutil.getNumFromFormat(transFee, 0);
...
function getActiveTabStkPrice(mode) {
    ...
    if (stkobj != null) {
        ...
        else if (mfOrderStatusPanel != null && comp.id == mfOrderStatusPanel.id) {
            var rec = comp.getSelectionModel().getSelection();
            if (rec != null)
                stkobj.price = getBuySellPrice(rec, mode);
        }
...
function getRecById(id) {
    ...
    if (!alreayGet) {
        if (mfOrderStatusPanel != null) {         // mutual fund order status 
            store = mfOrderStatusPanel.getStore();
            rec = store.getById(id);
            if (rec != null) {
                alreayGet = true;
            }
    ...
    if (!alreayGet) {
        if (mfEquityPrtfPanel != null) {          // port folio
            store = mfEquityPrtfPanel.getStore();
            rec = store.getById(id);
            if (rec != null) {
                alreayGet = true;
            }
...
function checkContextMenuItems(intradayChartBtn, ibBtn, stkEx){
    ...
    for (var i = 0; i < newItemList.length; i++) {
        ...
        if (intradayChartBtn && obj.name == intradayChartBtn.text) {
            if (obj.status) {
    ...
    //check if ibillionaire context menu should be enabled    
    if(ibBtn){
    	ibBtn.disable();
...
function getColorBar(rate, showZero, type, oneColor) {
    var bgpc = '';
    var cls1 = N2NCSS.BackUp;
    var cls2 = N2NCSS.BackDown;
    if (type === 's') { // sell rate
        cls1 = N2NCSS.BackDown;
        cls2 = N2NCSS.BackUp;
    }
    ...
    if (!isNaN(rate)) {
        if (rate == 0 && !showZero) {
            ...
            bgpc = '<div class="bgpc ' + cls2 + '">' + pc1 + ' <div class="bgtext">' + parseFloat(rate) + '% </div></div>';
        }
    }
            ...
            for (var j = 0; j < properties.length; j++) {
                ...
                switch (property[0]) {
                    ...
                    case "oh":
                        ...
                        break;
                    case "bo":
                        createBasketOrderPanel(portalcols[i], j);
                        break;
                    case "mo":
                    ...
                    case "ms":
                        ...
                        break;
                    case "fc":
                        createFundamentalCPIQWin(null, null, portalcols[i], j);
                        break;
                    ...
                    case "hd":
                        ...
                        break;
                    case "en1":
                        n2ncomponents.openElasticNews({key:null, name:null, conf:portalcols[i], tabCt:j, newsOpt:'1'});
                        break;
                    case "en2":
                        n2ncomponents.openElasticNews({key:null, name:null, conf:portalcols[i], tabCt:j, newsOpt:'2'});
                        break;
                    case "bc":
                    	n2ncomponents.createBreakEvenCalc(portalcols[i], j);
                        break;
                    case "ec":
                    	n2ncomponents.createMFCalc(portalcols[i], j);
                        break;
                }
            }
        }
...
function getPriceString(value, lacp, prependStr) {
    var cssCls = N2NCSS.FontUnChange;
    lacp = parseFloat(lacp); // avoid accident sting comparison which gives unexpected result
    ...
    return '<label class="' + cssCls + '">' + prependStr + getDisplayValue(value) + '</label>';
}
...
function getIdssSuspendItems(suspendType) {
    var sItems = [];
...
function getIdssLabel(idssStr, suspendType) {
    var idssSuspendItems = getIdssSuspendItems(suspendType);

    for (var i = 0; i < idssSuspendItems.length; i++) {
        var sItem = idssSuspendItems[i];
        // Add colors
        var sLabel = '<span style="color:#fff;" class="N2N_BackDown" title="' + languageFormat.getLanguage(20070, 'Suspended') + '">' + sItem + '</span>';
        idssStr = idssStr.replace(sItem, sLabel);
    }
...
function requestImportWl(wlObj, callback) {
    ...
    ajaxObj.callback = callback || function(op, success, response) { // default callback
        ...
        if (success && res && res.s) {
            msgutil.info(languageFormat.getLanguage(30136, 'Imported successfully.'));
...
function resetGridScroll(panel) {
    if (panel.bufferedRenderer && !panel.doSwitchRefresh && panel.didSelect) {
        // helper.resetGridScroll(panel); // disabled reset scroll every refresh
    }
}
...
function setCSSRule(img) {
    if (img && img.width && img.height) {
		...
		'.main-menu .x-btn-inner.x-btn-inner-default-toolbar-medium {font-size:' + N2N_CONFIG.menuFontSize + 'px;}' +
		'.main-menu .x-btn-default-toolbar-medium, .main-menu .x-btn-text.x-btn-icon-top .x-btn-icon-el-default-toolbar-medium {width:' + img.width + 'px;}' + 
		...
		'.main-menu .x-btn-button-default-toolbar-medium {height:' + (img.height) + 'px;}' + 
...
function n2br(str) {
    return n2other(str, '<br/>');
}

function n2other(str, other) {
...
function reloadWebUrls(){
	if(n2ncomponents.elasticNews['elastic_news']){
		menuHandler.elasticNews('1');
	}
	
	if(n2ncomponents.elasticNews['nikkei_news']){
		menuHandler.elasticNews('2');
	}
	
	if(userReport['userReport_pse_boardlot']){
		menuHandler.pseBoardLotTable();
	}
}
...
function getDefaultFontSettings(themeName) {
    var defFontObj = {};
    var fontArr = N2N_CONFIG.defFontSetting.split(',');

    for (var i = 0; i < fontArr.length; i++) {
        var fontItem = fontArr[i].split("_");
        if (fontItem[0] == "ft") {
            var fontSet = fontArr[i].substring(fontArr[i].indexOf("_") + 1);
            var fontSetArr = fontSet.split(fontSizeDel);

            defFontObj['fontType'] = fontSetArr[0];
            defFontObj['fontSize'] = fontSetArr[1];
        }

        if (fontItem[0] == "fcl") {
            var wUpColor = fontItem[1].split("~")[1];
            var bUpColor = fontItem[1].split("~")[2];
            var wDownColor = fontItem[2].split("~")[1];
            var bDownColor = fontItem[2].split("~")[2];
            var wUnChgColor = fontItem[3].split("~")[1];
            var bUnChgColor = fontItem[3].split("~")[2];
            var wYellowColor = fontItem[4].split("~")[1];
            var bYellowColor = fontItem[4].split("~")[2];

            if (isBlackTheme(themeName)) {
                defFontObj['up'] = bUpColor;
                defFontObj['down'] = bDownColor;
                defFontObj['unchg'] = bUnChgColor;
                defFontObj['yellow'] = bYellowColor;
            } else {
                defFontObj['up'] = wUpColor;
                defFontObj['down'] = wDownColor;
                defFontObj['unchg'] = wUnChgColor;
                defFontObj['yellow'] = wYellowColor;
            }
        }
    }
...
function updateWlMappingList(list) {
    for (var i = 0; i < list.length; i++) {
        var obj = list[i];
        fullList['wl'][obj[fieldStkCode]] = obj;
    }
}
...
function setExchange(exchange) {
    ...
    // display exchange disclaimer
    if (topPanelBar) {
        if (N2N_CONFIG.exchDisclaimerList.indexOf(exchangecode) == -1)
            helper.hide(topPanelBar.tLabelDisclaimer);
        else
            helper.show(topPanelBar.tLabelDisclaimer);
    }
...
function updateBrokerData(feedObj) {
            		...
            		if ( global_brokerlist != null ) {
            			if ( global_brokerlist.len != null && global_brokerlist.scpt != null ) {
            				for (var j = 0; j < global_brokerlist.len; j ++) {
            					...
            					if ( V.value == buyBrokerList[i] ){
            						buyBrokerList[i] = V.key;
            					}
            					if ( V.value == sellBrokerList[i] ){
            						sellBrokerList[i] = V.key;
            					}
            				}
            			}
            		}
...
function isStockCode(str) { // suppose key with . is a stock code
    if (typeof str === 'string') {
...
function getMappedStockName(str) {
    if (isStockCode(str) && mappedList) {
        if (mappedList[str]) {
            return mappedList[str];
        }
    }
...
function updateMappedList(stkCode, stkName) {
    if (!mappedList[stkCode]) {
        mappedList[stkCode] = stkName;
    }
}

function webBuySell(mode, stkcode, stkname, source){
	closedOrderPad = false;
	global_OrderSource = source;
	createOrderPad(stkcode, stkname, mode, null, true);
}

function getExchangeShortName(exCode) {

    if (exCode == 'KL')
        return languageFormat.getLanguage(20401, 'BURSA');
    if (exCode == 'KLL')
        return languageFormat.getLanguage(20402, 'BURSA (Live)');
    if (exCode == 'KLD')
        return languageFormat.getLanguage(20403, 'BURSA (Delayed)');
    if (exCode == 'SI' || exCode == 'SG')
        return languageFormat.getLanguage(20404, 'SGX');
    if (exCode == 'SID' || exCode == 'SGD')
        return languageFormat.getLanguage(20405, 'SGX (Delayed)');
    if (exCode == 'HK')
        return languageFormat.getLanguage(20406, 'HKEX');
    if (exCode == 'HKD')
        return languageFormat.getLanguage(20407, 'HKEX (Delayed)');
    if (exCode == 'BK')
        return languageFormat.getLanguage(20408, 'SET');
    if (exCode == 'BKD')
        return languageFormat.getLanguage(20409, 'SET (Delayed)');
    if (exCode == 'JK')
        return languageFormat.getLanguage(20410, 'IDX');
    if (exCode == 'JKD')
        return languageFormat.getLanguage(20411, 'IDX (Delayed)');
    if (exCode == 'PH')
        return languageFormat.getLanguage(20412, 'PSE');
    if (exCode == 'PHD')
        return languageFormat.getLanguage(20413, 'PSE (Delayed)');
    if (exCode == 'N')
        return languageFormat.getLanguage(20414, 'NYSE');
    if (exCode == 'ND')
        return languageFormat.getLanguage(20415, 'NYSE (Delayed)');
    if (exCode == 'A')
        return languageFormat.getLanguage(20416, 'NYSE MKT');
    if (exCode == 'AD')
        return languageFormat.getLanguage(20417, 'NYSE MKT (Delayed)');
    if (exCode == 'O')
        return languageFormat.getLanguage(20418, 'NASDAQ');
    if (exCode == 'OD')
        return languageFormat.getLanguage(20419, 'NASDAQ (Delayed)');
    if (exCode == 'HC')
        return languageFormat.getLanguage(20420, 'HSX');
    if (exCode == 'HN')
        return languageFormat.getLanguage(20421, 'HNX');
    if (exCode == 'SA')
        return languageFormat.getLanguage(20422, 'Tadawul');
    if (exCode == 'AX')
        return languageFormat.getLanguage(20423, 'ASX');
    if (exCode == 'L')
        return languageFormat.getLanguage(20424, 'LSE');
    if (exCode == 'T')
        return languageFormat.getLanguage(20425, 'TSE');
    if (exCode == 'SZD')
        return languageFormat.getLanguage(20427, 'Shenzhen (Delayed)');
    if (exCode == 'SSD')
        return languageFormat.getLanguage(20428, 'Shanghai (Delayed)');
    //Mutual Fund
    if (exCode == 'PHMF')
        return languageFormat.getLanguage(33500, 'Mutual Fund');

    if (exCode)
        return exCode;
}

function getExchangeFullName(exCode) {
    
    if (exCode == 'KL')
        return languageFormat.getLanguage(20302, 'Bursa Malaysia');
    if (exCode == 'KLD')
        return languageFormat.getLanguage(20303, 'Bursa Malaysia (Delayed)');
    if (exCode == 'MY')
        return languageFormat.getLanguage(20329, 'Bursa Malaysia Derivatives');
    if (exCode == 'SG' || exCode == 'SI')
        return languageFormat.getLanguage(20304, 'Singapore Exchange Ltd');
    if (exCode == 'SGD' || exCode == 'SID')
        return languageFormat.getLanguage(20305, 'Singapore Exchange Ltd (Delayed)');
    if (exCode == 'SA')
        return languageFormat.getLanguage(20307, 'Saudi Stock Exchange');
    if (exCode == 'HC')
        return languageFormat.getLanguage(20308, 'Ho Chi Minh Stock Exchange');
    if (exCode == 'HN')
        return languageFormat.getLanguage(20309, 'Hanoi Stock Exchange');
    if (exCode == 'UP')
        return languageFormat.getLanguage(20310, 'Vietnam UPCoM');
    if (exCode == 'OM')
        return languageFormat.getLanguage(20311, 'Oman Equities');
    if (exCode == 'JK')
        return languageFormat.getLanguage(20312, 'Indonesia Stock Exchange');
    if (exCode == 'JKD')
        return languageFormat.getLanguage(20313, 'Indonesia Stock Exchange (Delayed)');
    if (exCode == 'HK')
        return languageFormat.getLanguage(20314, 'Hong Kong Stock Exchange');
    if (exCode == 'HKD')
        return languageFormat.getLanguage(20315, 'Hong Kong Stock Exchange (Delayed)');
    if (exCode == 'A')
        return languageFormat.getLanguage(20316, 'NYSE MKT LLC');
    if (exCode == 'AD')
        return languageFormat.getLanguage(20317, 'NYSE MKT LLC (Delayed)');
    if (exCode == 'N')
        return languageFormat.getLanguage(20318, 'New York Stock Exchange');
    if (exCode == 'ND')
        return languageFormat.getLanguage(20319, 'New York Stock Exchange (Delayed)');
    if (exCode == 'O')
        return languageFormat.getLanguage(20320, 'Nasdaq Stock Market');
    if (exCode == 'OD')
        return languageFormat.getLanguage(20321, 'Nasdaq Stock Market (Delayed)');
    if (exCode == 'P')
        return languageFormat.getLanguage(20322, 'ARCA Equities');
    if (exCode == 'YM')
        return languageFormat.getLanguage(20323, 'E-Mini Dow ($5) Futures Equities');
    if (exCode == 'ES')
        return languageFormat.getLanguage(20324, 'E-Mini S&P 500 Futures Equities');
    if (exCode == 'ZS')
        return languageFormat.getLanguage(20325, 'Soybean Equities');
    if (exCode == 'UC')
        return languageFormat.getLanguage(20326, 'Crude Palm Oil Equities');
    if (exCode == 'PH')
        return languageFormat.getLanguage(20330, 'Philippines Stock Exchange');
    if (exCode == 'BK')
        return languageFormat.getLanguage(20327, 'Stock Exchange of Thailand');
    if (exCode == 'BKD')
        return languageFormat.getLanguage(20328, 'Stock Exchange of Thailand (Delayed)');
    if (exCode == 'KLL')
        return languageFormat.getLanguage(20331, 'Bursa Malaysia (Live)');
    if (exCode == 'AX')
        return languageFormat.getLanguage(20332, 'Australia Stock Exchange');
    if (exCode == 'L')
        return languageFormat.getLanguage(20333, 'London Stock Exchange');
    if (exCode == 'ENG')
        return languageFormat.getLanguage(20334, 'CME Energy');
    if (exCode == 'MT')
        return languageFormat.getLanguage(20335, 'CME Metal');
    if (exCode == 'WIDX')
        return languageFormat.getLanguage(20336, 'World Indices');
    if (exCode == 'T')
        return languageFormat.getLanguage(20337, 'Tokyo Stock Exchange');
    if (exCode == 'SZD')
        return languageFormat.getLanguage(20343, 'Shenzhen Equities (Delayed)');
    if (exCode == 'SSD')
        return languageFormat.getLanguage(20344, 'Shanghai Equities (Delayed)');
    //Mutual Fund
    if (exCode == 'PHMF')
        return languageFormat.getLanguage(33500, 'Mutual Fund');
...
function markExchange(exch) {

    if (mainMenuBar) {
        // get exchange menu
        var exMenu = mainMenuBar.getComponent('tbEx');

        if (exMenu && exMenu.menu) {
            var mItems = exMenu.menu.items.items;
            ...
            for (var i = 0; i < mItems.length; i++) {
                ...
                if (mi.ex) {
                    if (mi.ex === exch) {
                        mi.addCls('current-exch');
                    } else {
                        mi.removeCls('current-exch');
                    }
                } else if (mi.menu) { // grouping
                    mi.removeCls('current-exch');

                    var inItems = mi.menu.items.items;
                    for (var j = 0; j < inItems.length; j++) {
                        ...
                        if (inI.ex) {
                            if (inI.ex === exch) {
                                mi.addCls('current-exch');
                                inI.addCls('current-exch');
                            } else {
                                inI.removeCls('current-exch');
                            }
                        }
                    }
                }
...
/*
 * Display console log if front debug is enabled
 */
function tLog() {
    if (N2N_CONFIG.fDebug) {
        jsutil.runFn(console.log, arguments);
    }
}

function toStreamerOptionRec(str, skipFirst) {
    var optArr = str.split(',');
    var optRec = [];

    if (!skipFirst) {
        optRec.push({
            text: languageFormat.getLanguage(11089, 'Select Filter'),
            value: 0
        });
    }
    ...
    for (var i = 0; i < optArr.length; i++) {
        ...
        if (opt.indexOf('&') > -1) {
            var oInnerArr = opt.split('&');
            var textArr = [];
            var objVal = {};

            for (var a = 0; a < oInnerArr.length; a++) {
                var innerRec = toStreamerOptionRec(oInnerArr[a], true);

                if (innerRec[0]) {
                    textArr.push(innerRec[0].text);
                    Ext.merge(objVal, innerRec[0].value);
                }
            }
        ...
        if (del) {
            var oArr = opt.split(del);

            if (oArr[1]) {
                oArr[1] = oArr[1].trim();
                var val = jsutil.getNumFromRound(oArr[1].trim());


                if (!isNaN(val) && val > 0 && oArr[0]) {
                    oArr[0] = oArr[0].trim();

                    if (oArr[0] === 'vol') {
                        optRec.push({
                            text: languageFormat.getLanguage(10118, 'Vol') + del + formatutils.formatNumber(val, 0, true, 0, 0, true, 0),
                            type: oArr[0],
                            value: {
                                volSign: del,
                                vol: val
                        ...
                        optRec.push({
                            text: languageFormat.getLanguage(10127, 'Value') + del + formatutils.formatNumber(val, 0, true, 0, 0, true, 0),
                            type: oArr[0],
                            value: {
                                valueSign: del,
                                value: val
...
//Function to convert symbol"==" into ascii numeric or vice versa
function stringToAscii(param) {
    ...
    for (i = 0; i < param.length; i++) {
        if (i == param.length - 1) {
            ascii += param.charCodeAt(i).toString();
        } else {
            ascii += param.charCodeAt(i).toString() + ",";
        }
    }
...
function asciiToString(param) {

    if (!param)
        return;

    var string = "";
    var asciiArray = param.split(",");

    for (i = 0; i < asciiArray.length; i++)
        string += String.fromCharCode(parseInt(asciiArray[i]));
...
function getGridRowHeight() {
    if (isMobile) {
        ...
        return parseInt(globalFontSize) + 9.75;
    }
}

function validateStreamerExch(exch) {
    if (N2N_CONFIG.streamerExch) {
        return streamerExchArr.indexOf(exch) > -1;
    }

    return true;
}

function isDelayedIndex(exch) {

    if (delayIndicesExList.length > 0) {
        var delayIndicesList = delayIndicesExList.split(',');

        for (var i = 0; i < delayIndicesList.length; i++) {
            if (delayIndicesList[i] === exch) {
                return true;
            }
        }
    }
...
function getTrxFeeFromATPRules(exch, act, price, totUnits) {
    ...
    var trxFeesObj = {};
    var arTrxFeeFormulaList = getTrxFeeATPRules(exch);

    try {
        for (var i = 0; i < arTrxFeeFormulaList.length; i++) {
            if (arTrxFeeFormulaList[i].formulaName == 'CommRate') {
                arTrxFeeFormulaList[i].thisComm = false;
                var commObj = arTrxFeeFormulaList[i].formula[0];

                if (isMatchedPrice(amount, commObj.opfr, commObj.frval) && isMatchedPrice(amount, commObj.opto, commObj.toval)) {
                    arTrxFeeFormulaList[i].value = commObj.cr;
                    arTrxFeeFormulaList[i].crRate = commRate;
                    arTrxFeeFormulaList[i].thisComm = true;
                }
                ...
                if (arTrxFeeFormulaList[i].formula.indexOf('<') != -1) {
                    ...
                    tLog('Amount: ' + arTrxFeeFormulaList[i].value);
                }
            }
        }

        if (OrdQty > 0) {
            trxFeesObj = trxFeesCalculator.calcTrxFee(arTrxFeeFormulaList, act, amount);
            tLog('trxFeesObj', arTrxFeeFormulaList, act, amount, trxFeesObj);
...
function isMatchedPrice(price, operator, opValue) {
    var matched = false;
...
function getTrxFeeATPRules(exch) {

    if (atpRule && atpRule.s && atpRule.d && atpRule.size) {
        var len = atpRule.size;

        for (var i = 0; i < len; i++) {
            if (atpRule.d[i].ex == exch) {
                return atpRule.d[i].trxfee;
            }
        }
    }
...
function processDecimal(value, decimal, removeZero) {

    if (!value) {
        //Do nothing if null, empty string and empty var
    } else {
        value = parseFloat(value);
        if (N2N_CONFIG.setPriceDecimal) {
            //Round decimals based on the value and remove zero
            var absValue = Math.abs(value);
            switch (true) {
                case absValue >= 0.0001 && absValue < 0.0100 :
                    value = formatutils.formatNumber(value, null, null, 4, true);
                    break;
                case absValue >= 0.0100 && absValue < 0.5000 :
                    value = formatutils.formatNumber(value, null, null, 3, true);
                    break;
                case absValue >= 0.5000 && absValue < 1000 :
                    value = formatutils.formatNumber(value, null, null, 2, true);
                    break;
                case absValue >= 1000 :
                    value = formatutils.formatNumber(value, null, null, 0, true);
                    break;
                default:
                    value = jsutil.getFloat(value).toString();
            }
        } else {
            value = formatutils.formatNumber(value, null, null, decimal, removeZero);
        }
    }
...
function getLotValue(value, lotSize) {
...
function isMf(exch) {
    return exch === N2N_CONFIG.mfExchCode;
}

function setAppMsg(msg) {
    tLog('setAppMsg...');
