...
//		this.accNo = defAccNo;
        ...
        for (var ii = 0; ii < global_ExchangeList.length; ii++) {
            if(N2N_CONFIG.mfExchCode == global_ExchangeList[ ii ].exchange)
                continue;
    ...
    localSearch: function () {
        ...
        panel.getStore().filterBy(function (record, id) {
            ...
            if (panel.filterExtOpt == 0) {
                ...
                var mfExcode = formatutils.removeDelayExchangeChar(N2N_CONFIG.mfExchCode);
                
                if (record.get('ExCode').toUpperCase().indexOf(tempExchangeCode) != -1
                    && record.get('ExCode').toUpperCase().indexOf(mfExcode) == -1
                ) {
                    blnFilterEx = true;
                } else {
                    blnFilterEx = false;
...
//							}
                        ...
                        var buff = [
                            ...
                            {id: cmap_osTrailType, value: d.trailType, cellFormat: 'price'},
        ...
        if (showV1GUI == "TRUE") {
            ...
            if (showStkInfoHeader == "TRUE") {
                ...
                if (N2N_CONFIG.features_BrokerQ) {
                    ...
                    btns.push({
                        id: this.menuBrokerQId,
                        text: languageFormat.getLanguage(31800, 'Broker Queue'),
                        handler: function() {
                            n2ncomponents.createBrokerQ({key: orderStatusPanel.contextMenu.stkCode, name: orderStatusPanel.contextMenu.stkName});
                        }
                    });
                }
            }
            ...
            if (N2N_CONFIG.hasNews2) {
                ...
                btns.push({
                    ...
                    handler: function() {
                        menuHandler.openNews('', orderStatusPanel.contextMenu.stkCode);
                    }
                });
            }
            ...
            if (N2N_CONFIG.iBillionaireURL.length > 0) {
                ...
                btns.push({
                    ...
                    handler: function () {
                    	n2ncomponents.openIBillionaire(orderStatusPanel.contextMenu.stkCode);
                    }
                });
            }
...
//}
        ...
        var osCol = layoutProfileManager.getCol('os');
        if (osCol != null && osCol != "") {
            if (osCol.indexOf("~") != -1) {
                return true;
            }
...
// region start custom column functions
    ...
    currentColumnSetting: function () {
        var currentColumnId = layoutProfileManager.getCol('os');
    ...
    generateColumnName: function (id) {
        switch (id) {
            ...
            case cmap_osTrailType:
                return languageFormat.getLanguage(10946, 'Step');
            case cmap_osRemark:
    ...
    requestSaveColumns: function(newColumnIdArr) {
        ...
        colutils.saveColumn('os', newColumnIdArr);
    },
    /**
...
//			value = tempObj.value;
        ...
        for (var i = 0; i < colSettingList.length; i++) {
            ...
            switch (columnID) {
                ...
                case cmap_osTrailType:
                    columnList.push({
                        itemId: ordSta + columnID,
                        header: languageFormat.getLanguage(10946, 'Step'),
                        dataIndex: 'trailType',
                        sortable: true,
                        hidden: columnVisible,
                        menuDisabled: isMobile,
                        ...
                        align: 'right',
                        renderer: function (value, meta, record) {
                            value = jsutil.getFloat(value, 0, 0);
                            if (value) {
    ...
    _setCookieId: function () {
        ...
        me.cookieKey = columnWidthSaveManager.getCookieColKey('ordStatus');
        me.paramKey = N2N_CONFIG.constKey + 'OS';
        me.colWidthKey = 'os';
    },
