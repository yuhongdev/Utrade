...
//    },
    ...
    initComponent: function() {
        ...
        if (isMobile) {
            ...
            htmlbuf.push('</tr>');
            htmlbuf.push('<tr>');
            htmlbuf.push('<td class="ordDetailLabel ' + N2NCSS.stockInfoLabel_color + '">' + languageFormat.getLanguage(10922, 'StopLimit'), + '</td>');
            htmlbuf.push('<td id="' + panelid + '_OrdDet_StopLimit" colspan="3"></td>');
            htmlbuf.push('</tr>');
            htmlbuf.push('<tr>');
            ...
            htmlbuf.push('<td class="ordDetailLabel ' + N2NCSS.stockInfoLabel_color + '">' + languageFormat.getLanguage(20848, 'Short Sell') + '</td>');
            htmlbuf.push('<td id="' + panelid + '_OrdDet_ShortSell"></td>');
            htmlbuf.push('<td class="ordDetailLabel ' + N2NCSS.stockInfoLabel_color + '">' + languageFormat.getLanguage(20842, 'Trigger Price') + '</td>');
            htmlbuf.push('<td id="' + panelid + '_OrdDet_StopLimit"></td>');
            htmlbuf.push('</tr>');
            htmlbuf.push('<tr>');
            htmlbuf.push('<td class="ordDetailLabel ' + N2NCSS.stockInfoLabel_color + '">' + languageFormat.getLanguage(20181, 'Remarks') + '</td>');
            htmlbuf.push('<td id="' + panelid + '_OrdDet_Remarks" colspan="3"></td>');
        ...
        var defaultConfig = {
            ...
            style: 'padding: 5px;',
            html: htmlbuf.join(''),
            cls: 'fix_themebg font-style-applied font-size-max-applied',
            listeners: {
            ...
            if (success == true) {
                ...
                if (data != null) {
                    ...
                    for (var i = 0; i < totData; i++) {
                        ...
                        this.setOrdSts(sts);
                        if (parseFloat(ordprc) == 0 && (ordtype == 'Market' || ordtype == 'MarketToLimit')) {
                        	ordprc = '';
                        }
    ...
    setStopLimit: function(val) {
        ...
        val = this.formatCurrency(val);
        Ext.fly(this.id + '_OrdDet_StopLimit').update(val);
    },
