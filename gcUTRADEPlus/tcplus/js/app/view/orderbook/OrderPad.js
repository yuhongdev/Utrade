...
Ext.define('TCPlus.view.orderbook.OrderPad', {
    ...
    //Mutual fund test
    //amount test by price
    amountTest: /^[\d\,]*(\.[\d]{1,4})?$/,
    mfQtyTest: /^[0-9]|^[0-9]+$[0-9]/,
...
//    },
    ...
    basketList: null,
    isBasket: false,
    //Mutual fund check firstload
    afterFirstLoad: false,
    initComponent: function () {
        var panel = this;
        panel.pinTest = pinReg;
        panel._maxQtyLength = N2N_CONFIG.orderQtyMaxLength || 9;
        
        panel._labelWidth = panel._getLabelWidth();
...
//        }
        ...
        Ext.apply(Ext.form.field.VTypes, {
            ...
            roundQtyText: languageFormat.getLanguage(30244, 'Please input a valid quantity. You can use K, M, B with maximum 2 decimals. \neg. 1.55K'),
            roundQtyMask: /[0-9.\-KMB]/i,
            //Data validation during input
            amount: function (val, field) {
                    return panel.amountTest.test(val);
            },
            //Error message if fail valid
            amountText: languageFormat.getLanguage(33677, 'Not a valid price. Must be a numeric value.'),
            //What I can input
            amountMask: /[0-9.\b]/i,
        });
...
//            if (cfg.confirm) confirm = cfg.confirm;
            ...
            if (cfg.stkqty)
                stkqty = cfg.stkqty;
            if (cfg.stkprice)
                stkprice = cfg.stkprice;
            if (cfg.validity)
                validity = cfg.validity;
            
            //Mutual fund, can place in same place.
            if (cfg.netcashlimit)
                netcashlimit = cfg.netcashlimit;
            if (cfg.amount)
                amount = cfg.amount;
            if (cfg.mfqty)
                mfqty = cfg.mfqty;
        				...
        				load:function(thisStore, records){
        					...
        					if(records){
        						...
        						for(var i=0; i<records.length; i++){
        	                        ...
        	                        newobj.ac = rec.ac;
        	                        newobj.cc = rec.cc;
        	                        newobj.accno = rec.ac + global_AccountSeparator + rec.bc;
        	                        newobj.cliType = rec.cliType;
        ...
        panel.isBasic = jsutil.toBoolean(userPreference.get('opBasic', !N2N_CONFIG.orderPadAdvView));

        var stylebtnBasic = 'background:#27313C;margin:0px;padding:0px;margin-left:5px';
        var heightbtnBasic = 16;
        panel.btnBasic = Ext.create('Ext.button.Button', {
            hidden: !N2N_CONFIG.orderPadBasic,
            iconCls: panel.isBasic == true ? 'icon-up-first' : 'icon-down-first',
            ...
            handler: function () {
                ...
                userPreference.set('opBasic', panel.isBasic);
                userPreference.save();
        ...
        if (N2N_CONFIG.accTypesButtons.length > 0) {
        	panel.btnList = [];
        	panel.btnListWidth = 0;
        	var def = N2N_CONFIG.defAcc.split('|');
        	panel.lastSelectedAccType = def;
        	var btnlist = N2N_CONFIG.accTypesButtons.split(';');
        	for (var i=0; i<btnlist.length; i++) {
        		var btnType = btnlist[i].split('|');
        		panel['btn' + btnType[1]] = Ext.create('Ext.button.Button', {
        			text: btnType[1],
        			width: 35,
        			height: 20,
        			toggleGroup: 'acctypesbtn',
        			pressed: false,//btnType[1] == panel.lastSelectedAccType[1],
        			accType: btnType,
        			cls: 'accTypesButtons',
        			allowDepress: false,
        			handler: function(thisBtn) {
                                    panel.preserveQty = true;
                                    
        				panel.filterAccountList(thisBtn.accType);
        				panel.setPaymentList(); 
                        panel.callStkInfo();
                        panel.callAccBal();
                        
                        panel.disableCFDTradingLimit();
        			}
        		});
        		panel.btnList.push(panel['btn' + btnType[1]]);
        		panel.btnListWidth += panel['btn' + btnType[1]].width;
        	}
        }
        
        panel.btnAccTypesCt = new Ext.container.Container({
        	width: N2N_CONFIG.accTypesButtons.length > 0 ? (panel.btnListWidth + 20) : 0,//160,
        	items: panel.btnList
        });
        
        panel.btnBasketOrd = Ext.create('Ext.button.Button', {
        	iconCls: panel.isBasket ? 'icon-basket-on' : 'icon-basket-off',
            tooltip: panel.isBasket ? languageFormat.getLanguage(20915, 'Basket ON') : languageFormat.getLanguage(20916, 'OFF'),
            cls: isMobile ? 'menubtn' : '',
            height: isMobile ? '100%' : heightbtnBasic,
            width: isMobile ? 26 : 16,
            //isBasic: false,
            ...
            handler: function () {
                if (!panel.isBasket) {
                    panel.isBasket = true;
                    panel.btnBasketOrd.setIconCls('icon-basket-on');
                    panel.btnBasketOrd.setTooltip(languageFormat.getLanguage(20915, 'Basket ON'));
                    createBasketOrderPanel();
                } else {
                    panel.isBasket = false;
                    panel.btnBasketOrd.setTooltip(languageFormat.getLanguage(20916, 'OFF'));
                    panel.btnBasketOrd.setIconCls('icon-basket-off');
                }           
            }
        });
        ...
        panel.compRef.stkCb = Ext.widget('searchautobox', {
            itemId: "ordPad_stkname",
            emptyText: languageFormat.getLanguage(20875, 'Search stock here with name or code'),
            fieldLabel: languageFormat.getLanguage(20177, 'Stock'),
            labelStyle: 'white-space:nowrap;',
            style: 'padding-top:3px;padding-right:5px',
            labelPad: 2,
            labelWidth: panel._labelWidth,
            width: '100%',
            listConfig: {
                minWidth: 150,
            ...
            listeners: {
                select: function (thisCombo, records) {
                    if (records.length > 0) {
                        var rec = records[0];
                        ...
                        if (accno.length > 0) {
                            ...
                            var branchCode = accountName[accountName.length - 1].trim();
                            var accbranch = accno + global_AccountSeparator + branchCode;
                            obj.accbranchNo = accbranch;
                        }

                        createOrderPad(rec.get(fieldStkCode), rec.get(fieldStkName), panel.mode, obj, false);
                        thisCombo._itemSelected = true;
                    }
                }
            },
            onSearchEnterKey: function (records, e) {
                if (records.length > 0) {
                    var rec = records[0];
                    var accno = panel.getAccountNo();
                    var obj = {};
        ...
        panel.compRef.searchStockBt = Ext.widget('button', {
            iconCls: 'icon-center icon-search',
            width: 16,
            height: 16,
            iconAlign: 'top',
            padding: 0,
        ...
        var inputRow1 = new Ext.container.Container({
            ...
            border: false,
            layout: 'vbox',
            style: 'padding-left:5px;padding-top:0px;padding-bottom:3px;margin:0px;',
            defaults: {
                style: "padding-right:8px;"
            },
            items: [
                //only 1 net cash limit so
                panel.inputRow1Items = new Ext.container.Container({
                    width: '100%',
                    narrow: 350,
                    columnWidth: 0.5,
                    layout: {
...
//                            item.resumeLayouts(true);
                            if (formHeight) {
                                // fixed not able to show IDSS on IE
                                if (Ext.isIE) {
                                    formHeight += 21;
                                }

                                panel._setCtSize(formHeight, columnWidth);
                            }
                        }
                    },
                    items: [
                        Ext.create('Ext.form.field.Text', {
                            // ref: '../../dsqty',
                            itemId: 'accountno_label2',
                            baseCls: '',
                            border: false,
//                            columnWidth: !isMobile ? 0.20 : 0.20, //0.35,
                            editable: false,
                            fieldLabel: languageFormat.getLanguage(20834, 'Trading Limit'),
                            labelAlign: 'left',
                            labelPad: 2,
                            labelWidth: panel._labelWidth,
//                            fieldStyle: 'width:100%',
                            width: '100%',
                            name: 'accountno_label2',
                            inputType: this.fieldType,
                            listeners: {
                            },
                            allowBlank: true,
                            hidden: !N2N_CONFIG.tradingLimitVisible
                        }),
                        //create new field for net limit. take note TODO
                        Ext.create('Ext.form.field.Text', {
                            itemId: 'netcashlimit',
                            baseCls: '',
                            border: false,
//                            columnWidth: !isMobile ? 0.20 : 0.20, //0.35,
                            editable: false,
                            fieldLabel: languageFormat.getLanguage(20064, 'Cash'),
                            labelAlign: 'left',
//                            labelPad: 2,
//                            labelWidth: panel._labelWidth,
                            labelWidth: 35,
                            ...
                            name: 'netcashlimit',
                            inputType: this.fieldType,
                            hidden: !netcashlimit, //HIDE IF NOT MUTUAL FUNDS
                            listeners: {
                            },
                            allowBlank: true
                        })
                	...
                	layout: {
                     ...
                     items:[
			                Ext.create('Ext.form.field.ComboBox', {
			                    ...
			                    //labelSeparator: "",
			                    fieldLabel: languageFormat.getLanguage(20833, 'Account No.'),//"<table cellpadding='0' cellspacing='0' style='width:100%'><tr><td style='width:10px;white-space:nowrap'>" + languageFormat.getLanguage(20833, 'Account No.') + ":</td>", //For fixing error on Mozilla FireFox(Push down Trading Limit)
			                    msgTarget: 'none',
			                    //   boxLabel: '<span style="font-weight:normal">' + lblBal + '</span>',
			                    labelStyle: ' position: relative; white-space: nowrap;',
			                    style: isDealerRemisier ? 'padding-right:5px;' : '',
			                    name: 'accountno',
			...
			//                                        });
			                            	...
			                            	combo.getStore().on('load', function(thisStore, records) {
			                            		if (records) {
			                            			if(records.length == 1){
			                            				...
			                            				combo.collapse();
			                            				panel.setAccType(records[0].data.accno);
			                            ...
			                            if (!isMobile) {
			                                ...
			                                panel.btnAccTypesCt.render('btnacctypes');
			                            }
			                            
			                            panel.btnlimit = new Ext.button.Button({
			                                // ref: '../../btnlimit',
			                                itemId: 'btnlimit',
			                                iconCls: "icon-center icon-refresh-limit",
			                                iconAlign: 'top',   
			                                padding: 0,
			                                margin: 0,
			                                width: 18,
			                        ...
			                        select: function (thisComp, record) {
			                            ...
			                            if (panel.btnlimit.isDisabled()) {
			                                if (record[0].data.accno != '') {
			                                    ...
			                                    panel.setAccType(record[0].data.accno);
			                                }
			                            ...
			                            panel.disableCFDTradingLimit(record[0].data.accno);
			                        ...
			                        change: function(thisComp, newValue) {
			                        	if (!isDealerRemisier && newValue && N2N_CONFIG.defTradeAccFeature && defTrAccConf.getDefTrAccOpt(panel.exchangecode) == defAccValues.LAST) {
        ...
        panel.compRef.actionCt = Ext.create('Ext.form.field.ComboBox', {
            // ref: '../../cbAction',
//            xtype: 'combo',
            width: '100%',
            itemId: 'cbAction', 
           ...
           // labelStyle: ' position: relative; top: 4px;',
            //style: 'padding-right: 2px;padding-bottom:1px;',
...
//                            panel.setDefault();
        ...
        panel.compRef.trailTypeCT = Ext.create(panel._getExtInputCls(), {
            itemId: 'trailType',
            fieldLabel: languageFormat.getLanguage(10946, 'Step'),
            ...
            maxLengthText: languageFormat.getLanguage(30245, 'The maximum length for this field is {0}'),
            enforceMaxLength: true,
            mouseWheelEnabled: N2N_CONFIG.enablePriceQtyScroll,
            step: N2N_CONFIG.enableUserPrefQtyStep ? (userPreference.get('tqs') ? parseInt(userPreference.get('tqs'), 10) : N2N_CONFIG.qtyScrollStep) : N2N_CONFIG.qtyScrollStep,
            minValue: 0,
        ...
        panel.compRef.dsqtyPanel = Ext.create(panel._getExtInputCls(), {
            // ref: '../../dsqty',
            itemId: 'dsqty',
            baseCls: '',
            border: false,
...
//                                	}
            ...
            panel.compRef.priceCT = Ext.create('Ext.form.field.ComboBox', {
                ...
                autoSelect: N2N_CONFIG.ordPriceAutoSelect,
                ...
                onMouseWheel: function(e){
                	...
                	if (me.hasFocus) {
                		...
                		if (delta > 0) {
                			if (!me.readOnly) {
                	            var val = parseFloat(me.getValue())||0; // defaults to zero on parse failure
                	            me.setValue(panel.formatPrcDecimal(val + me.step), 3);
                	        }
                		} else if (delta < 0) {
                			if (!me.readOnly) {
                				var val = parseFloat(me.getValue())||0; // defaults to zero on parse failure
...
//                            vtype: 'qty',   			 bug - v1.3.14.9
            ...
            panel.compRef.orderTypeCT = Ext.create('Ext.form.field.ComboBox', {
                //   // ref: '../../otype',
                // xtype: 'combo',
                itemId: 'otype',
                queryMode: 'local',
               ...
               // labelStyle: ' position: relative; top: 4px;',
                ...
                listeners: {
                    ...
                    select: function (cbo, rec, idx) {
                        ...
                        // Mode sets to B when order pad is buying, S when order pad is selling.
                        if (mode == modeOrdBuy || mode == modeOrdSell) {
                            panel.setOrderPanelBuySellRule(panel.exchangecode);
...
//          labelStyle: ' position: relative; top: 4px;',
        ...
        panel.compRef.minQtyCt = Ext.create(panel._getExtInputCls(), {
            //itemId: 'minqtyPanel',
            itemId: 'minqty',
            baseCls: '',
            border: false,
        ...
        panel.compRef.setCurCt = Ext.create('Ext.form.field.ComboBox', {
            // ref: '../settcurrPanel',
        ...
        panel.compRef.paymentCt = Ext.create('Ext.form.field.ComboBox', {
            baseCls: '',
            border: false,
            // ref: '../paymentPanel',
            //itemId: 'paymentPanel',
        ...
        //Mutual fund field START
        //copy from qty
        panel.compRef.amountCT = Ext.create(panel._getExtInputCls(), {
            itemId: 'amount',
            fieldLabel: languageFormat.getLanguage(21273, 'Amount'),
            msgTarget: panel._elMsgTarget,
            labelAlign: 'left',
            labelPad: 2,
            labelWidth: panel._labelWidth,
            ...
            enforceMaxLength: true,
            mouseWheelEnabled: N2N_CONFIG.enablePriceQtyScroll,
            //Step is spin up or spin down amount you want, for example, scroll up 1 to add 100
            step: N2N_CONFIG.mfAmtScrollStep,
            minValue: 0,
            getSubmitValue: panel._submitValueFn(),
            vtype: N2N_CONFIG.orderRoundQty ? 'roundQty' : 'amount',
            maxLengthText: N2N_CONFIG.orderRoundQty ? '' : languageFormat.getLanguage(30245, 'The maximum length for this field is {0}'),
            hidden: !amount, //Follow atp config
            listeners: {
                render: function (thisComp) {
                    panel.initializeOrderPadDragZone(thisComp);
                    panel.initializeOrderPadDropZone(thisComp);
        ...
        panel.compRef.qtyMFCT = Ext.create(panel._getExtInputCls(), {
                itemId: 'quantityMF',
                fieldLabel: languageFormat.getLanguage(20836, 'Qty'),
                msgTarget: panel._elMsgTarget,
                labelAlign: 'left',
                ...
                labelWidth: panel._labelWidth,
                columnWidth: 0.50,
                hidden: !mfqty, //Follow atp config
                fieldCls: 'num-field',
                name: 'quantityMF',
                allowDecimals: false, //1.3.25.10 not allow decimal input
                inputType: this.fieldType,
                ...
                enforceMaxLength: true,
                mouseWheelEnabled: N2N_CONFIG.enablePriceQtyScroll,
                step: N2N_CONFIG.mfQtyScrollStep,
                minValue: 0,
                getSubmitValue: panel._submitValueFn(),
                vtype: N2N_CONFIG.orderRoundQty ? 'roundQty' : null,
                maxLengthText: N2N_CONFIG.orderRoundQty ? '' : languageFormat.getLanguage(30245, 'The maximum length for this field is {0}'),
        ...
        //Mutual fund field END

        panel.row2Items = [];
        panel.row3Items = [];
        panel.row5Items = [];
        //if (!isMobile) {
        //mutual fund field can mix with equity stock
        //when switch stock will hide field anyway
        panel.row2Items.push(panel.compRef.actionCt);
        panel.row2Items.push(panel.compRef.orderTypeCT);
        ...
        panel.row2Items.push(panel.compRef.trailTypeCT);
        panel.row2Items.push(panel.compRef.dsqtyPanel);
        panel.row2Items.push(panel.compRef.qtyCT);
        panel.row2Items.push(panel.compRef.minQtyCt);
        panel.row2Items.push(panel.compRef.priceCT);
        panel.row2Items.push(panel.compRef.setCurCt);
        //Mutual fund is before settlement currency
        panel.row2Items.push(panel.compRef.amountCT);
        panel.row2Items.push(panel.compRef.qtyMFCT);
        panel.row2Items.push(panel.compRef.validityCt);
        panel.row2Items.push(panel.compRef.paymentCt);
        
        //panel.row3Items.push(panel.compRef.stratCt);
        //}
        ...
        panel.inputRow2Container = new Ext.container.Container({
            baseCls: '',
            border: false,
            layout: 'vbox',
            width: '100%',
            items: [ panel.inputRow2 ]
                    ...
                    items: [
                        {
                            itemId: 'idssCb',
                            xtype: 'checkbox',
                            boxLabel: '<span style="font-weight:normal">' + languageFormat.getLanguage(20901, 'IDSS') + '</span>',
                            labelStyle: 'padding-left:5px;',
                            labelSeparator: '',
                            name: 'idss',
                            listeners: {
        ...
        var inPanelItems = new Array();
            inPanelItems.push({
                ...
                html: '<table border=0 style="width:100%;height:14px;" cellspacing="0px" cellpadding="0px"><tr><td id="trdLimitCol" style="text-align:left;overflow:hidden;white-space:nowrap;padding-left:4px;border-collapse:collapse;"></td>' +
                        '<td style="padding-right:8px;width:550px;text-align:right;padding-top:2px;padding-bottom:2px;"><span id="btnacctypes" style="background-position:0 0;display:block;float:left;margin-left:-11px;"></span></span><span id="btnbasic" style="background-position:0 0;display:block;float:right;"></span>&nbsp;<span id="sbtnlimit" style="background-position:0 0;display:block;float:right;"></span></td>' +
                        '</tr></table>'
            });
        inPanelItems.push(inputRow1);
        inPanelItems.push(panel.inputRow2Container);
//        inPanelItems.push(panel.inputRow3);
//        inPanelItems.push(panel.inputRow5);
        if(sCFDMenu){
            ...
            items: [
                {
                    // ref: '../../../../skipconfirm',
                    itemId: 'skipconfirm',
                    width: panel._getSkipConLabelWidth(),
                    xtype: 'checkbox',
                    hideLabel: true,
                            ...
                            items: [
                                {
                                    ...
                                    hidden: panel.checkIsHidePin(), //1.3.24.5 check category, if match then hide pin
                                    hideLabel: true,
                                    boxLabel: '<div class="icon-save-pin" style="width: 16px; height: 16px;"></div>',
                                    listeners: {
...
//            style: 'background-color:#fff;',
        ...
        //Mutual fund section
        var amount = Ext.create('widget.hiddenfield', {
            itemId: 'amount',
            name: 'amount',
            xtype: 'hiddenfield'
        });
        ...
        var innerMainCtConfItemsConf = {
            itemId: 'innerMainCtConfItemsConf',
            baseCls: '',
            border: false,
            layout: 'column',
            autoScroll: isMobile ? true : false,
        };
        if (isMobile) {
            ...
            var btnPanel = Ext.create('Ext.container.Container', {
                ...
                columnWidth: 0.35,
                layout: 'table',
                style: 'text-align:center;padding:3px 5px 5px 5px',
                items: {
                	layout: {
                	type: 'table',
                	columns: 2
...
//                    ordPad.gtd.setValue(Ext.Date.format(date,'Ymd'));
        ...
        if (n2nLayoutManager.isWindowLayout()) {
            var expand = cookies.readCookie('_OrdPad_Expand');
            ...
            //Create mutual fund info
            panel.createMutualFundInfo();
            panel.compRef.topInfo =  Ext.create("Ext.panel.Panel", {
                border: false,
                height: 119,
                collapsed: expand == "-1", 
                layout: "border",
                ...
                items: [ panel.compRef.marketDepth, panel.compRef.mFundInfo ],
                listeners: {
             ...
             confItems.push(panel.compRef.topInfo);
        }
        ...
        if (isMobile) {
            ...
            panel.compRef.formHeight = 300;
            panel.compRef.formDiffHeight = 125;
        } else {
        	panel.compRef.headerHeight = 23;
        	panel.compRef.formHeight = 300;
        	panel.compRef.mdHeight = 125;
        	panel.compRef.formDiffHeight = 120;
        }
        ...
        var defaultConfig = {
            ...
            listeners: {
                afterrender: function (p) {
                    ...
                    if (N2N_CONFIG.enablePriceQtyScroll) {
                        if (panel.compRef.priceCT.bodyEl) {
                            panel.compRef.priceCT.mon(panel.compRef.priceCT.bodyEl, 'mousewheel', panel.compRef.priceCT.onMouseWheel, panel.compRef.priceCT);
                        }

                        if (panel.compRef.stopLimitCt.bodyEl) {
                            panel.compRef.stopLimitCt.mon(panel.compRef.stopLimitCt.bodyEl, 'mousewheel', panel.compRef.stopLimitCt.onMouseWheel, panel.compRef.stopLimitCt);
                        }
                    }
                }
            }
...
//        panel.callAccBal();
    ...
    createMutualFundInfo: function() {
        //Mutual fund section START
        //just call function for mutual fund
        var panel = this;

//        var expand = cookies.readCookie('_OrdPad_Expand');
        panel.compRef.mFundInfo = Ext.widget('mfinfo', Ext.apply({
            collapsed: true,
            region: "center",
            layout: "fit",
            header: false,
        ...
        //console.log("test collapsed" + panel.compRef.mFundInfo.getCollapsed());
        //Mutual fund section END
    },
    adjustComps: function() {
        var panel = this;
        //Adjust after first load and switch dock or popup
        helper.runAfterCompRendered(panel, function() {
            panel.toggleTabPopup();
            panel._setCtSize();
            
        });
    },
    toggleTabPopup: function(){
        var panel = this;
        if (panel.mt === 'p') {
              var expand = cookies.readCookie('_OrdPad_Expand');
              if (panel.afterFirstLoad) {
              if (isMf(orderPad.exchangecode)) {
                ...
                if (expand != "-1") {
                    panel.compRef.topInfo.expand();
                    panel.compRef.marketDepth.expand();
                    panel.compRef.mFundInfo.collapse();
                } else {
                    panel.compRef.marketDepth.collapse();
                    panel.compRef.mFundInfo.collapse();
                    panel.compRef.topInfo.collapse();
                }
                  //On first load
                if (!panel.afterFirstLoad) {
                    panel.afterFirstLoad = true;
                }
             }
      } else {
          orderPad.compRef.marketDepth.collapse();
    ...
    //Mutual fund info
    updateMutualFundInfo: function (stkcode, stkname) {
        var panel = this;
        
        if (panel.compRef.mFundInfo) {
            panel.compRef.mFundInfo.searchMutualFundInfo(stkcode, stkname);
        }
    },
                ...
                if (enableRSSIndicator == 'TRUE') {
                    if (rec[fieldRSSIndicator] != null && rec[fieldRSSIndicator] == 'R') {
                        if (panel.mode == modeOrdSell || (panel.mode == modeOrdRevise && panel.prevAction.toLowerCase() == "sell")) {
                            input.down('#shortsellPanel').setVisible(true);
                        }
                    }
                }
            }
            ...
            if (dataObj[fieldStkCode] != null) {
                if (dataObj[fieldStkCode].indexOf(panel.stkcode) == 0) {
                    if (panel.lotsize == null || panel.lotsize == "") {
                        if (dataObj[fieldLotSize] != null && dataObj[fieldLotSize] != panel.lotsize) {
                    ...
                    panel.setStock(panel.stkcode, panel.stkname);
                    ...
                    if (panel.updPrc) {
                        ...
                        if (global_useBidSize) {
                        	...
                        	if(N2N_CONFIG.enablePriceQtyScroll){
                        		panel._sector = dataObj[fieldSectorCode];
                        		var bidSize = panel.getBidSize(panel.stkcode, panel.getExCode(), panel._sector, panel.price);
                        		panel.compRef.priceCT.step = bidSize;
                        		panel.compRef.stopLimitCt.step = bidSize;
                        	}
                        ...
                        // make sure no duplicate price
                        bidRec = jsutil.arrayUnique(bidRec);
                        askRec = jsutil.arrayUnique(askRec);
                    ...
                    if(global_cfddelta != null){
        	        	if (global_cfddelta.len != null && global_cfddelta.scpt != null) {
        	        		for (var i = 0; i < global_cfddelta.len; i++) { // verify from main.jsp
        	        			var V = global_cfddelta.scpt[i];
        	        			var cfdV = V.value.split(',');
        	        			
        	        			if (V.key == panel.stkcode) {
        	        				panel.cfdTradable = cfdV[4];
        	        				panel.cfdShortSell = cfdV[2];
        	        				dataObj[fieldCFDMarginLS_06] = cfdV[1];
        	        				dataObj[fieldCFDMarginPerc_06] = cfdV[0];
        	        				dataObj[fieldCFDLastUpdTime_06] = cfdV[5];
        	        				dataObj[fieldCFDMapValue_06] = null;
        	        				break;
        	        			}
        	        		}
        	        	}
                    ...
                    if(dataObj[fieldCFDMapValue_06]){
                    	...
                    	panel.cfdTradable = cfdValue.tradable;
                    	panel.cfdShortSell = cfdValue.shortsell;
                    }
                    
                    if(dataObj[fieldCFDMarginPerc_06]){
                    	panel.cfdMarginPerc = dataObj[fieldCFDMarginPerc_06];
                    }
                    
                    if(dataObj[fieldCFDMarginLS_06]){
                    	panel.cfdMarginLS = dataObj[fieldCFDMarginLS_06];
                    }
                    
                    
                    if(panel.cfdTradable == 'Y' && panel['btnCFD'] && panel['btnCFD'].pressed){
                    	if(panel._isQtyLotSize()){
                    		panel.setLotSize(panel.cfdMarginLS);
                    	}
                    }
...
//    //},
    resetPanel: function () {
        ...
        this.trdForm.down('#OrdPad').down('#inPanel').down('#idssPanel').setVisible(false);
        this.trdForm.down('#OrdPad').down('#inPanel').down('#idssPanel').setVisible(false);
        this.trdForm.down('#OrdPad').down('#inPanel').down('#idssCb').setValue(false);
        
        // reset cached account obj for remisier
        if (isDealerRemisier) {
            this.dealerAcc = null;
        }
    ...
    //For populate the action combobox
    setActionList: function (mode) {
        
        var panel = this;
        try {
            if (isMf(panel.exchangecode)) {
                panel.arActionList = new Array();
                if (mode) {
                    panel.arActionList.push([modeOrdBuy, languageFormat.getLanguage(33670, 'Subscribe')]);
                    panel.arActionList.push([modeOrdSell, languageFormat.getLanguage(33671, 'Redeem')]);
                    if (mode == modeOrdRevise || mode == modeOrdCancel) {
        ...
        try {
            ...
            if (reset) {
                ...
                if (panel.canTrade && panel.isReady) {
                    try {
                        if (isMf(panel.exchangecode)) {
                            //Separate equity stock and mutual fund
                            msgTxt = panel.getTrdMsgMF();
                        } else {
        ...
        if (!Ext.isEmpty(stkcode) && !Ext.isEmpty(stkname) && !Ext.isEmpty(act) && !Ext.isEmpty(otype)) {
            ...
            if (input.down('#minqty').isVisible()) {
                if (input.down('#minqty').readOnly == false) {
                    if (input.down('#minqty').getValue() && input.down('#minqty').getValue() != '') {
                        var minQtyStr = '';
                        if (N2N_CONFIG.orderRoundQty) {
            ...
            if (act == modeOrdBuy || act == modeOrdSell) {
            	if(panel.cfdTradable == 'Y' && panel['btnCFD'] && panel['btnCFD'].pressed){
            		//panel.compRef.btnSubmit.enable();
            		var initialMarginVal = totVal * (parseFloat(panel.cfdMarginPerc) / 100);
            		if(isNaN(initialMarginVal)){
            			initialMarginVal = 0;
            		}else{
            			initialMarginVal = formatutils.getCFDCurrConversionValue(initialMarginVal, panel.cdfCurrency, act, panel.currency);
            		}
            		initialMarginVal = panel.formatNumberComa(panel.formatPrcDecimal(initialMarginVal, 2))
            		
                    tempCurrencyMsg += languageFormat.getLanguage(20902, '<br/>ShortSell : [PARAM0]<br/>Initial Margin(%) : [PARAM1]<br/>Margin LotSize : [PARAM2]<br/>Initial Margin Est. : [PARAM3]<br/>',  panel.cfdShortSell+'|'+panel.cfdMarginPerc+'|'+panel.cfdMarginLS+'|'+initialMarginVal);
            	}/*else if(panel.cfdTradable == 'N' && panel['btnCFD'] && panel['btnCFD'].pressed){
                ...
                if (showTrxFees == '2') {
                    ...
                    var trxFees = 0;
                    var commRate = 1;
                    var amount = totUnits * price;

                    for (var i = 0; i < panel.arTrxFeeFormulaList.length; i++) {
                    	if(panel.arTrxFeeFormulaList[i].formulaName == 'CommRate'){
                    		panel.arTrxFeeFormulaList[i].thisComm = false;
                    		var commObj = panel.arTrxFeeFormulaList[i].formula[0];
                    		
                    		if(panel.isMatchedPrice(amount, commObj.opfr, commObj.frval) && panel.isMatchedPrice(amount, commObj.opto, commObj.toval)){
                    			panel.arTrxFeeFormulaList[i].value = commObj.cr;
                    			panel.arTrxFeeFormulaList[i].crRate = commRate;
                    			panel.arTrxFeeFormulaList[i].thisComm = true;;
                    		}
                    		commRate++;
                    	}else{
                    ...
                    if (OrdQty > 0) {
                        panel.trxFeesObj = trxFeesCalculator.calcTrxFee(panel.arTrxFeeFormulaList, act, amount);
                        trxFees = panel.trxFeesObj.trxFee;
                    }
                    /*
                    if (N2N_CONFIG.orderRoundQty) {
                        tempCurrencyMsg += '<br>' + languageFormat.getLanguage(20862, 'Trx Fees=[PARAM0]', formatutils.displayRound(trxFees));
                    } else {
                        tempCurrencyMsg += '<br>' + languageFormat.getLanguage(20862, 'Trx Fees=[PARAM0]', formatutils.formatNumber(trxFees));
                    }
                    */
                    
                    tempCurrencyMsg += '<br>' + languageFormat.getLanguage(20862, 'Trx Fees=[PARAM0]', formatutils.formatNumber(trxFees, null, false, 2));
            ...
            // amfraser SIP
            if (showSIPMessage == "TRUE" && accRet && !panel.isCFDAcc()) {
...
//        	console.log( '[OrderPad][getTrdMsg] otype ---> ' + otype );
                ...
                if (mode == 0) {
                    ...
                    if (N2N_CONFIG.defTradeAccFeature) {
                        if (trAction == modeOrdBuy || trAction == modeOrdSell) {
                            ...
                            if (!isDealerRemisier && defTrAccOpt == defAccValues.LAST) {
                                var defTrAccLast = defTrAccConf.getDefTrAccLast(panel.exchangecode);
                                if (isDealerRemisier) { // obsolete
                                     panel.searchAccount(defTrAccLast, true); // disable auto display account list
                                } else {
                                    el.setValue(defTrAccLast);
                                    panel.setAccType(defTrAccLast);
                                    
                                    panel.disableCFDTradingLimit();
                                }
                            } else if (!isDealerRemisier && defTrAccOpt == defAccValues.SPECIFIC && defTrAccVal != '') {
                                if (isDealerRemisier) { // obsolete
                                     panel.searchAccount(defTrAccVal, true); // disable auto display account list
                                } else {
                                    // get first match
                                    var firstMatch = panel._getFirstMatch(panel.arAccList, defTrAccVal);
                                    if (firstMatch) {
                                        el.setValue(firstMatch);
                                        panel.setAccType(firstMatch);
        ...
        try {
            ...
            if (accRet) {
                ...
                for (var i = 0; i < accList.length; i++) {
                    ...
                    if (acc.ac && accno == acc.ac) {
                        // Should update trading limit only
                        // Other info is not necessary. It can be used from initial object accRet (client) or panel.accs (remisier)
                        /*
                        ordPad.down('#ac').setValue(acc.ac);
                        ordPad.down('#cc').setValue(acc.cc);
                        ordPad.down('#ex').setValue(acc.ex);
                        ordPad.down('#branchcode').setValue(acc.bc);
                        ordPad.down('#brokercode').setValue(acc.bkc);
                        */
            ...
            if (isMf(panel.exchangecode)) {
                panel.setBalanceMF(acc);
            }
        } catch (e) {
            console.log('[OrderPad][setCreditLimit] Exception ---> ' + e);
        }
    },
    setBalanceMF: function (acc) {
        var panel = this;
        var ncl = acc.ncl;
        var elNCL = panel.trdForm.down("#netcashlimit");
        
        elNCL.setValue(processDecimal(ncl, 2));
    },
    ...
    setTradingLimitBal: function (acc) {
        ...
        if (acc.cliType == 'D') {
            blTxt.push(languageFormat.getLanguage(20903, 'Free Equity') + ': ' + acc.occ + ' ');
        	panel.cdfCurrency = acc.occ;
        }else{
        	if (panel.getPayment() == 'CUT') {
        ...
        if (panel.getPayment() == 'CUT') {
        	...
        	if (acc.cliType == 'D') {
        		var trdlimit = acc.ncl;
        		trdlimit = formatutils.getCFDCurrConversionValue(trdlimit, acc.occ, panel.mode);
        		blTxt.push(panel.formatNumberComa(panel.formatTradingLimitDecimal(trdlimit)));
        	}else{
    ...
    callAccBal: function () {
        ...
        try {
            ...
            panel.setAccMeta(panel.getAccNo());
            ...
            Ext.Ajax.request({
                ...
                success: function (response) {
                    ...
                    if (obj && obj.s == true) {
                        ...
                        if (!panel.isCFDAcc() && panel.getPayment() != 'CUT') {
                            if (isDealerRemisier) {
                                // cache account obj for later use
                                panel.dealerAcc = obj;
                            }
                            ...
                            if (isShowLimit == 'true') {
                                ...
                                var trdLimitLabel = Ext.get('trdLimitLabel');
...
//        }
        ...
        if (otype == 'Market' || otype == 'MarketToLimit' || otype == 'TrailStopPercent' || otype == 'TrailStopSteps') {
...
//            }
        ...
        if (!panel.pinTest.test(pin) && !panel.isBasket) {
...
//        }
        ...
        //MF field
        //amount follow qty
        var amount = panel.getAmount();
        var qtyMF = panel.getQtyMF();
        ...
        if (isMf(panel.exchangecode)) {
            if (panel.action == "Buy") {
                if (N2N_CONFIG.orderRoundQty) {
                    amount = jsutil.getNumFromRound(amount);
                }
                // [start - quantity validation]
                isnan = isNaN(amount);
                if (isnan || !amount || amount < 1 || !panel.amountTest.test(amount)) {
                    msgutil.alert(languageFormat.getLanguage(33723, 'Please enter a valid order amount.'), function (btn, txt) {
                        if (btn == 'ok') {
                            input.down('#amount').focus();
                        }
                    });
                    return false;
                }
                ...
                if (amount != orgQty) {
                    ...
                    if (revQty != null || revQty != '') {
                        ...
                        if (parseFloat(amount) > parseFloat(orgQty)) {
                            if (revQty.indexOf('+') == -1) {
                                msgutil.alert(languageFormat.getLanguage(30217, 'New order amount cannot be more than original order amount.'), function (btn, txt) {
                                    if (btn == 'ok') {
                                        input.down('#amount').focus();
                                    }
                                });
                        ...
                        } else if (parseFloat(amount) < parseFloat(orgQty)) {
                            if (revQty.indexOf('-') == -1) {
                                msgutil.alert(languageFormat.getLanguage(30218, 'New order amount cannot be less than original order amount.'), function (btn, txt) {
                                    if (btn == 'ok') {
                                        input.down('#amount').focus();
                                    }
                                });
            ...
            //Mutual fund sell
            else if (panel.action == "Sell") {
                if (N2N_CONFIG.orderRoundQty) {
                    qtyMF = jsutil.getNumFromRound(qtyMF);
                }
                // [start - quantity validation]
                isnan = isNaN(qtyMF);
                if (isnan || !qtyMF || qtyMF < 1 || !panel.mfQtyTest.test(qtyMF)) {
                    msgutil.alert(languageFormat.getLanguage(30207, 'Please enter a valid order quantity.'), function (btn, txt) {
                        if (btn == 'ok') {
                            input.down('#quantityMF').focus();
                        }
                    });
                    return false;
                }
                ...
                if (qtyMF != orgQty) {
                    ...
                    if (revQty != null || revQty != '') {
                        ...
                        if (parseFloat(qtyMF) > parseFloat(orgQty)) {
                            if (revQty.indexOf('+') == -1) {
                        ...
                        } else if (parseFloat(qtyMF) < parseFloat(orgQty)) {
                            if (revQty.indexOf('-') == -1) {
        ...
        if (!isRev) {
            msgutil.alert(languageFormat.getLanguage(30230, 'You did not make any changes for the order.'), function (btn, txt) {
                if (btn == 'ok') {
                    //input.down('#otype').focus();
                }
            });
    ...
    validateBSMF: function () {
        ...
        if (act == modeOrdBuy) {
            ...
            if (isnan) {
                ...
                if (!amount || amount <= 0 || !panel.amountTest.test(amount)) {
                    displayAlert = true;
                }
            }
            if (displayAlert) {
                if (isnan || !amount || amount <= 0 || !panel.amountTest.test(amount)) {
                    msgutil.alert(languageFormat.getLanguage(30257, 'Please enter a valid order amount.'), function (btn, txt) {
                        if (btn == 'ok') {
                            input.down('#amount').focus();
                        }
                    });
            ...
            //check mutual fund quantity
            isnan = isNaN(qtyMF);
            var displayAlert = false;
            if (isnan) {
                displayAlert = true;
            } else {
                if (!qtyMF || qtyMF <= 0 || !panel.mfQtyTest.test(qtyMF)) {
                    displayAlert = true;
                }
            }
            if (displayAlert) {
                if (qtyMF || !qtyMF || qtyMF <= 0 || !panel.mfQtyTest.test(qtyMF)) {
                    msgutil.alert(languageFormat.getLanguage(30207, 'Please enter a valid order quantity.'), function (btn, txt) {
                        if (btn == 'ok') {
                            input.down('#quantityMF').focus();
                        }
        ...
        //Checkbox section
        //1.3.25.3 Set a fix pin when trading pin is hidden
        //huh? is it even possible that trading pin is hidden
        //And why???????
        if (panel.checkIsHidePin()) {
            ordPad.down('#pin').setValue("123456");
            pin = "123456";
...
//            }
    ...
    setPaymentList: function (mode, payment) {
            ...
            try {
                ...
                if (el) {
                    ...
                    if (mode == 0) { // reset
                        ...
                        var prevPayment = panel.getPreviousPayment(panel.arPaymentList, payment);
                        
                        if (prevPayment) {
                            el.setValue(prevPayment);
                        } else {
    ...
    initForm: function (mode) {
        ...
        try {
            ...
            if (mode == 0) {
                ...
                panel.disableCmp(ordPad.down('#trailType'));
        ...
        if (quantityCmp != null && quantityCmp.labelEl != null) {
            ...
            if (size != null || !isNaN(size)) {
                 ...
                 */
                	...
                	if (size == "") {
                        quantityCmp.labelEl.update("<div style= 'margin-top:0px;margin-bottom:0px;'>" + this.getQtyTxt() + ':</div>');
                    } else {
                        quantityCmp.labelEl.update("<div style= 'margin-top:0px;margin-bottom:0px;'>" + this.getQtyTxt() + "x" + size + ':<div>');
                    }
                //}
            }
        }
    ...
    selectActionMode: function (act) {
        ...
        try {
            ...
            if(isMf(panel.exchangecode)){
                switch (act) {
                    case modeOrdBuy:
                        panel.compRef.btnSubmit.setText(languageFormat.getLanguage(33670, 'Subscribe').toUpperCase());
                        panel.compRef.btnSubmit.removeCls(["ordPad-SellBtn"]);
                        panel.compRef.btnSubmit.addCls("ordPad-BuyBtn");
                        //Increase the submit width
                        panel.compRef.btnSubmit.setWidth(100);
                        panel.enableCmp(input.down('#amount'));
                        panel.initBuySell();
                        break;
                    case modeOrdSell:
                        panel.compRef.btnSubmit.setText(languageFormat.getLanguage(33671, 'Redeem').toUpperCase());
                        panel.compRef.btnSubmit.removeCls(["ordPad-BuyBtn"]);
                        panel.compRef.btnSubmit.addCls("ordPad-SellBtn");
                        panel.compRef.btnSubmit.setWidth(75);
                        panel.enableCmp(input.down('#quantityMF'));
                        panel.initBuySell();
                        break;
                    case modeOrdCancel:
...
//        panel.setSetting();
        ...
        try {
            ...
            if (valid) {
                ...
                if (!skip && panel.allowDup != "1") {
                    ...
                    var trailType = input.down('#trailType').getValue();
                    ...
                    var st1 = '<span style="width:110px;display:inline-block;font-weight:bold;">';
                    var title = '<span style="display:block; padding-bottom:5px; padding-left:3px"><b style="font-size:1.3em">' + stkname + '</b>' + '&nbsp;<span id="ordConfBuyText" style="font-size:1.1em;">' + stkcode + '</span></span>';
                    if(N2N_CONFIG.orderChargesConfirmation && panel.exchangecode == 'PH'){
                    	msgBuf.push('<table style="float: left;border-right: 2px solid grey;width:40%"><td>');
                    }else{
                    	msgBuf.push('<table style="float: left;width:100%"><td>');
                    }
                    msgBuf.push('<span style="font-size:1.2em;display:inline-block;font-weight:bold;">' + languageFormat.getLanguage(20877, 'Order Details') + '</span>');
                    //msgBuf.push('<span style="font-size:1.1em;display:inline-block;">' + stkcode + '</span>');
                    msgBuf.push(st1 + languageFormat.getLanguage(20833, 'Account No.') + '</span>: ' + ac);
                    msgBuf.push(st1 + languageFormat.getLanguage(20837, 'Order Type') + '</span>: <span id="ordConfBuyText">' + languageFormat.getLanguage(10001, 'Buy') + ', </span>' + type);
                    msgBuf.push(st1 + languageFormat.getLanguage(20838, 'Validity') + '</span>: ' + validity);
                    msgBuf.push(st1 + languageFormat.getLanguage(20183, 'Order Price') + '(' + panel.currency + ')</span>: ' + price);
                    if (exchangecode == "MY") {
                        msgBuf.push(st1 + languageFormat.getLanguage(20851, 'Contract') + '</span>: ' + panel.formatNumberComa(qty));
                    } else {
                        msgBuf.push(st1 + languageFormat.getLanguage(20184, 'Order Qty') + '</span>: ' + panel.formatNumberComa(qty));
                    }

                    if (showExtraOrdMsg == 'TRUE') {
                        msgBuf.push(st1 + languageFormat.getLanguage(20853, 'Payment Mode') + '</span>: ' + panel.getPayment());
                        //msgBuf.push(st+ 'Total Order Value:  </span>'+ input.settcurr.getValue() + ' ' + (price * qty).toFixed(3));
                        msgBuf.push(st1 + languageFormat.getLanguage(20854, 'Total Order Value') + '</span>: ' + input.down('#settcurr').getValue() + ' ' + panel.totalValue);
                    }

                    if (input.down('#stoplimit').isVisible()) {
                        if (input.down('#stoplimit').readOnly == false) {
                            if (input.down('#stoplimit').getValue() != '') {
                                msgBuf.push(st1 + panel._getTriggerLabel() + '</span>: ' + tggrprice);
                            }
                        }
                    }
                    ...
                    if (input.down('#tptype').isVisible()) {
                        if (input.down('#tptype').readOnly == false) {
                            if (input.down('#tptype').getValue() != '') {
                                msgBuf.push(st1 + 'Trigger Type</span>: ' + tggrtype);
                            }
                        }
                    }
                    ...
                    if (input.down('#tpdirection').isVisible()) {
                        if (input.down('#tpdirection').readOnly == false) {
                            if (input.down('#tpdirection').getValue() != '') {
                                msgBuf.push(st1 + 'Trigger Direction</span>: ' + tggrdirection);
                            }
                        }
                    }
                    
                    if (input.down('#trailType').isVisible()) {
                        if (input.down('#trailType').readOnly == false) {
                            if (input.down('#trailType').getValue() != '') {
                                msgBuf.push(st1 + 'Step</span>: ' + trailType);
                            }
                        }
                    }
                    ...
                    if (input.down('#dsqty').isVisible()) {
                        if (input.down('#dsqty').readOnly == false) {
                            if (input.down('#dsqty').getValue() != '') {
                                ...
                                msgBuf.push(st1 + languageFormat.getLanguage(20839, 'Disclosed Qty') + '</span>: ' + panel.formatNumberComa(dsQtyVal));
                            }
                        }
                    }
                    if (input.down('#minqty').isVisible()) {
                        if (input.down('#minqty').readOnly == false) {
                            if (input.down('#minqty').getValue() != '') {
                                ...
                                msgBuf.push(st1 + languageFormat.getLanguage(20843, 'Min Qty') + '</span>: ' + panel.formatNumberComa(minQtyVal));
                            }
                        }
                    }
                    
                    var idssCb = input.down('#idssCb');
                    if (idssCb) {
                        if (idssCb.getValue()) {
                            msgBuf.push(st1 + ' ' + languageFormat.getLanguage(20901, 'IDSS') + ' </span>: <b>' + languageFormat.getLanguage(10011, 'Yes') + '</b>');
                        }
                    }
                    
                    if (input.down('#privateOrd')) {
                        if (input.down('#privateOrd').getValue()) {
                            msgBuf.push(st1 + msgutil.join(languageFormat.getLanguage(20904,' Private Order') ,'</span>: ', languageFormat.getLanguage(10011, 'Yes')));
                        }
                    }

                    for (var i = 1; i < msgBuf.length; i++) {
                    	if(i == 1){
                    		msgBuf[i] = '<span id="ordConfBuyHeader" style="color:#000;text-align:center;display:inline-block;width:100%;padding:3px 0px;">' + msgBuf[i] + '</span><br>';
                    	}else{
                    		if(i%2 != 0){
                    			msgBuf[i] = '<span id="ordConfAltRows" style="border-bottom: 1px solid grey;display:inline-block;width:100%;padding:3px 0px;">' + msgBuf[i] + '</span><br>';
                    		}else{
                    			msgBuf[i] = '<span style="border-bottom: 1px solid grey;display:inline-block;width:100%;padding:3px 0px;">' + msgBuf[i] + '</span><br>';
                    		}
                    	}
                    }
                    
                    msgBuf.push('</td></table>');
                    
                    var ocBuf = new Array();
                    if(N2N_CONFIG.orderChargesConfirmation && panel.exchangecode == 'PH'){
                    	var st2 = '<span style="width:100px;display:inline-block;font-weight:bold;">';
                    	ocBuf.push('<table style="float: left;width:60%"><td>');
                    	ocBuf.push('<span style="color:#000;font-size:1.2em;display:inline-block;font-weight:bold;">' + languageFormat.getLanguage(20878, 'Order Charges') + '</span>');
                    	ocBuf.push(st2 + languageFormat.getLanguage(20879, 'Commission') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.Commission));
                    	ocBuf.push(st2 + languageFormat.getLanguage(31647, 'PSE Fee') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.SECFee));
                    	ocBuf.push(st2 + languageFormat.getLanguage(20881, 'Commission VAT') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.VAT));
                    	ocBuf.push(st2 + languageFormat.getLanguage(20886, 'SCCP Charge') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.SCCPFee));
                    	//ocBuf.push(st2 + languageFormat.getLanguage(20884, 'STaxCharge') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.SalesTax));


                    	for (var i = 1; i < ocBuf.length; i++) {
                    		if(i == 1){
                    			ocBuf[i] = '<span id="ordConfBuyHeader" style="color:#000;text-align:center;display:inline-block;width:100%;padding:3px 0px;">' + ocBuf[i] + '</span><br>';
                    		}else{
                    			if(i%2 != 0){
                    				ocBuf[i] = '<span id="ordConfAltRows" style="border-bottom: 1px solid grey;display:inline-block;width:100%;padding:3px 0px;">' + ocBuf[i] + '</span><br>';
                    			}else{
                    				ocBuf[i] = '<span style="border-bottom: 1px solid grey;display:inline-block;width:100%;padding:3px 0px;">' + ocBuf[i] + '</span><br>';
                    			}
                    		}
                    	}

                    	ocBuf.push('<span style="display:inline-block;width:100%;padding-top:3px"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfBuyText style="font-weight:bold;">' + languageFormat.getLanguage(20887, 'Total Charges') + ' (' + input.down('#settcurr').getValue() + ')</span> : ' +  formatutils.formatNumber(panel.trxFeesObj.trxFee, null, null, 2) + '</span></span><br>');
                    	ocBuf.push('<span style="display:inline-block;width:100%;"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfBuyText style="font-weight:bold;">' + languageFormat.getLanguage(20854, 'Total Order Value') + ' (' + input.down('#settcurr').getValue() + ')</span> : ' + formatutils.formatNumber(panel.totalValue, null, null, 2) + '</span></span><br>');
                    	ocBuf.push('</td></table>');
                    }
                    
                    var msg = msgBuf.join('');
                    var othermsg = ocBuf.join('');
                    ...
                    if (isPromptPromoMsg) {
                        cutPromoMsg = "<br><br><span style='color:red;text-align:justify;display:inline-block;'><label>" + promoMessage + '</label></span>';
                    }
                    ...
                    var btnStyle = N2N_CONFIG.orderChargesConfirmation && panel.exchangecode == 'PH' ? "<div><span style='float:right;padding:3px 3px 5px 0px;'>" : "<div style='width:260px'><span style='float:right;position:relative;left:-20%;padding:3px 3px 3px 0px;'>";
                    
                    var mbox = Ext.Msg.show({
                        title: global_popUpMsgTitle,
                        msg: title + msg + othermsg + "<br><br>" + btnStyle + "<button style='width:70px;margin-right:5px;' onclick='orderPad.submitOrder();Ext.Msg.hide();'>" + okText + "</button><button style='width:70px' onclick='orderPad.allowUpdate=true;Ext.Msg.hide();'> " + cancelText + "</button></span>" + cutPromoMsg + '</div>',
                        minWidth: N2N_CONFIG.orderChargesConfirmation && panel.exchangecode == 'PH' ? 600 : 300,
                        cls: 'fix_msgfull'
                                /*
    ...
    getTrdInfoMsg: function (rs) {
        ...
        if (isMobile) { // for mobile
            ...
            try {
                if (rs.on) {
                    arMsg.push(languageFormat.getLanguage(20905, 'Ticket No: '));
                    arMsg.push(rs.on);
                    arMsg.push('<br>');
                }
                ...
                if (validity == 'GTD') {
                    validity = languageFormat.getLanguage(20906, 'Good Till ') + gtd;
                }

                arMsg.push(languageFormat.getLanguage(20907, 'Your order '));
                arMsg.push(' ');
                arMsg.push(stkcode);
                arMsg.push('/');
                arMsg.push(stkname);
                ...
                arMsg.push(' ');
                arMsg.push(prc);
                arMsg.push(languageFormat.getLanguage(20908, ', Validity for '));
                arMsg.push(validity);
                arMsg.push(languageFormat.getLanguage(20909, ' is submitted on '));
                arMsg.push(rs.tm);
                arMsg.push(msgutil.join('<br>', languageFormat.getLanguage(20910, 'For the status of submitted order, please go to order book')));
    ...
    getTrdAckMsg: function (rs) {
        ...
        try {
            ...
            arMsg.push(validity);
            arMsg.push(languageFormat.getLanguage(20911, ' accepted on '));
            arMsg.push(rs.tm);
            arMsg.push('<br>' + languageFormat.getLanguage(20910, 'For the status of submitted order, please go to order book'));
            //Your order <action> <stockcode>/<stockname> - <action> <qty> (units)@ <price>, Validity for <validity> is accepted on <order time stamp>.
            msg = arMsg.join('');
        } catch (e) {
    ...
    //Submit form/param to backend
    submitForm: function (basketOrdList, basketPin, fromBasket) {
        var panel = this;
        ...
        if (global_s2FARequire == 'true' && global_s2FABypass == 'YES' && global_s2FADeviceList.length == 0 && global_s2FAMobileDevice.length == 0 || global_s2FABypass == 'ALL') {
        ...
        ordPad.down('#ordsource').setValue(global_OrderSource);
        ...
        if (isMobile) {
            ...
            ordPad.down('#msgBox').update(panel.getTrdMsg() + '<br>' + panel.printMsg(languageFormat.getLanguage(10020, 'Processing...'), 'font-weight:bold; color:blue;' + bgcolor));
        }
        ...
        var data = null;
        var userConfirmDuplicate = userPreference.get('confirmDup', '0');
        if(userConfirmDuplicate == '0'){
            panel.allowDup = '1';
        }
        
        if(basketOrdList){
        	var params = {"selectedRecords": basketOrdList};
        	var jsonRec = Ext.encode(params);
        	data = {
            		jsonRec: jsonRec,
            		pin3: basketPin,
            		action: 'BS',
                        allowDup: panel.allowDup
            };
        }else{
            var basketList = panel.getTradeFormValues();
        	var params = {"selectedRecords": basketList};
        	var jsonRec = Ext.encode(params);
        	data = {
            		jsonRec: jsonRec,
                        allowDup: panel.allowDup
        ...
        panel.trdForm.getForm().submit({
            ...
            success: function (form, action) {
                if (action.result && action.result.duplicate) {
                    panel.processDuplicate(action.result.promptMsgBox,fromBasket);
                } else {
            	if(fromBasket){
                    basketOrderPanel.setLoading(false);
                    msgutil.info(languageFormat.getLanguage(32007, 'Order(s) submitted. Please check order book for details.'));
        	}
                ordPad.down('#pin').enable();  // enable back component
                panel.submitBasketResult(form, action);
                }
            },
        ...
        //Not allow duplicate, only after message box press ok then enable it
        panel.allowDup = "0";
        
    },
    processDuplicate: function (promptMsgBox,fromBasket) {
        var panel = this;
        var ordPad = panel.trdForm.down('#OrdPad');

        if (promptMsgBox) {
            //After N millisecond
            var duplicateMsg = msgutil.popup({
                title: global_popUpMsgTitle,
                width: 350,
                items: {
                    style: 'padding: 5px',
                    items: [
                        {
                            xtype: 'container',
                            html: languageFormat.getLanguage(20917, 'This order is similar to the last order you submitted. Would you like to proceed?'),
                        },
                        {
                          xtype: 'checkbox',
                          itemId: "skipDupCB",
                            ...
                            items: [
                                {
                                    ...
                                    handler: function () {
                                        ...
                                        //Save duplicate order checking
                                        if (duplicateMsg.down("#skipDupCB").getValue()) {
                                            userPreference.set('confirmDup', '0');
                                            userPreference.save();
                                        }
                                        
                                        if (fromBasket) {
                                            basketOrderPanel.processBasketOrd();
                                        } else {
                                            panel.proceedOrder();
                                        }
                                        duplicateMsg.close();
                                ...
                                },
                                {
                                    ...
                                    cls: 'fix_btn',
                                    handler: function () {
                                        panel.trdForm.down('#msgBox').update(panel.getTrdMsg());
                                        if (fromBasket) {
                                            basketOrderPanel.setLoading(false);
                                        }
                                        duplicateMsg.close();
            ...
            ordPad.down('#pin').enable();
            panel.compRef.btnSubmit.enable();
            // allowUpdate for fix unable to trade again after have duplicate order
            // allow update for allow user to change order pad stock
            panel.allowUpdate = true;
            // isReady for check if orderpad rdy for display msg or not
            //is false before submit form and true after submit
            // panel.isReady = true;
            if (fromBasket) {
                basketOrderPanel.setLoading(false);
            }
...
//                  	msgutil.alert(msg, null, 'Failure');
            ...
            if (resetPriceQty == true) {	// reset Price & Qty field
                ...
                input.down('#trailType').setValue('');
                
                // reset more fields
                // reset IDSS field
                var idssCb = input.down('#idssCb');
                if (idssCb) {
                    idssCb.setValue(false);
                }
            }
...
//            if (panel.fldCfg.confirm == true) confirm = '1';
        ...
        if (equityPrtfPanel != null) {
            equityPrtfPanel.callAccBal();
        }
        if (mfEquityPrtfPanel != null) {
            mfEquityPrtfPanel.callAccBal();
        }
        if (derivativePrtfPanel != null) {
            derivativePrtfPanel._procCallAccBalance();
        }
        if (equityPrtfRealizedPanel != null) {
            equityPrtfRealizedPanel.callAccBal();
        }
        ...
        try {
            ...
            if (valid) {
                ...
                if (!skip && panel.allowDup != "1") {
                    ...
                    var msgBuf = new Array();
                    var st1 = '<span style="width:100px;display:inline-block;font-weight:bold;">';
                    var title = '<span style="display:block; padding-bottom:5px; padding-left:3px"><b style="font-size:1.3em">' + stkname + '</b>' + '&nbsp;<span id="ordConfSellText" style="font-size:1.1em;">' + stkcode + '</span></span>';
                    if(N2N_CONFIG.orderChargesConfirmation && panel.exchangecode == 'PH'){
                    	msgBuf.push('<table style="float: left;border-right: 2px solid grey;width:40%"><td>');
                    }else{
                    	msgBuf.push('<table style="float: left;width:100%"><td>');
                    ...
                    msgBuf.push('<span style="font-size:1.2em;display:inline-block;font-weight:bold;">' + languageFormat.getLanguage(20877, 'Order Details') + '</span>');
                    msgBuf.push(st1 + languageFormat.getLanguage(20833, 'Account No.') + '</span>: ' + ac);
                    msgBuf.push(st1 + languageFormat.getLanguage(20837, 'Order Type') + '</span>: <span id="ordConfSellText">' + languageFormat.getLanguage(10002, 'Sell') + ', </span>' + type);
                    msgBuf.push(st1 + languageFormat.getLanguage(20838, 'Validity') + '</span>: ' + validity);
                    msgBuf.push(st1 + languageFormat.getLanguage(20183, 'Order Price') + '(' + panel.currency + ')</span>: ' + price);
                    if (exchangecode == "MY") {
                        msgBuf.push(st1 + languageFormat.getLanguage(20851, 'Contract') + '</span>: ' + panel.formatNumberComa(qty));
                    ...
                    if (input.down('#stoplimit').isVisible()) {
                        if (input.down('#stoplimit').readOnly == false) {
                            if (input.down('#stoplimit').getValue() != '') {
                                msgBuf.push(msgutil.join(st1, panel._getTriggerLabel(), '</span>: ', tggrprice));
                            }
                        }
                    }
                    ...
                    if (input.down('#tptype').isVisible()) {
                        if (input.down('#tptype').readOnly == false) {
                            if (input.down('#tptype').getValue() != '') {
                                msgBuf.push(msgutil.join(st1, languageFormat.getLanguage(20913, 'Trigger Type'), '</span>: ', tggrtype));
                            }
                        }
                    }
                    ...
                    if (input.down('#tpdirection').isVisible()) {
                        if (input.down('#tpdirection').readOnly == false) {
                            if (input.down('#tpdirection').getValue() != '') {
                                msgBuf.push(msgutil.join(st1, languageFormat.getLanguage(20914, 'Trigger Direction'), '</span>: ', tggrdirection));
                            }
                        }
                    }
                    ...
                    if (input.down('#trailType').isVisible()) {
                        if (input.down('#trailType').readOnly == false) {
                            if (input.down('#trailType').getValue() != '') {
                                msgBuf.push(msgutil.join(st1, languageFormat.getLanguage(10946, 'Step'), '</span>: ', trailType));
                            }
                        }
                    }
                    ...
                    if (input.down('#shortsell')) {
                        if (input.down('#shortsell').getValue()) {
                            msgBuf.push(st1 + ' ' + languageFormat.getLanguage(20848, 'Short Sell') + ': </span><b>' + languageFormat.getLanguage(10011, 'Yes') + '</b>');
                        }
                    }
                    ...
                    if (input.down('#privateOrd')) {
                        if (input.down('#privateOrd').getValue()) {
                            msgBuf.push(msgutil.join(st1, languageFormat.getLanguage(20904, ' Private Order'), '</span>: ', languageFormat.getLanguage(10011, 'Yes')));
                        }
                    }
                    
                    for (var i = 1; i < msgBuf.length; i++) {
                    	if(i == 1){
                    		msgBuf[i] = '<span id="ordConfSellHeader" style="color:#fff;text-align:center;display:inline-block;width:100%;padding:3px 0px;">' + msgBuf[i] + '</span><br>';
                    	}else{
                    		if(i%2 != 0){
                    			msgBuf[i] = '<span id="ordConfAltRows" style="border-bottom: 1px solid grey;display:inline-block;width:100%;padding:3px 0px;">' + msgBuf[i] + '</span><br>';
                    		}else{
                    ...
                    if(N2N_CONFIG.orderChargesConfirmation && panel.exchangecode == 'PH'){
                    	var st2 = '<span style="width:100px;display:inline-block;font-weight:bold;">';
                    	ocBuf.push('<table style="float: left;width:60%"><td>');
                    	ocBuf.push('<span style="color:#fff;font-size:1.2em;display:inline-block;font-weight:bold;">' + languageFormat.getLanguage(20878, 'Order Charges') + '</span>');
                    	ocBuf.push(st2 + languageFormat.getLanguage(20879, 'Commission') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.Commission));
                    	ocBuf.push(st2 + languageFormat.getLanguage(31647, 'PSE Fee') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.SECFee));
                    	ocBuf.push(st2 + languageFormat.getLanguage(20881, 'Commission VAT') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.VAT));
                    	ocBuf.push(st2 + languageFormat.getLanguage(20886, 'SCCP Charge') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.SCCPFee));
                    	ocBuf.push(st2 + languageFormat.getLanguage(20884, 'STax Charge') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.SalesTax));
                    	
                    	for (var i = 1; i < ocBuf.length; i++) {
                    		if(i == 1){
                    			ocBuf[i] = '<span id="ordConfSellHeader" style="color:#000;text-align:center;display:inline-block;width:100%;padding:3px 0px;">' + ocBuf[i] + '</span><br>';
                    		}else{
                    			if(i%2 != 0){
                    				ocBuf[i] = '<span id="ordConfAltRows" style="border-bottom: 1px solid grey;display:inline-block;width:100%;padding:3px 0px;">' + ocBuf[i] + '</span><br>';
                    			}else{
                    	...
                    	ocBuf.push('<span style="display:inline-block;width:100%;padding-top:3px"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfSellText style="font-weight:bold;">' + languageFormat.getLanguage(20887, 'Total Charges') + ' (' + input.down('#settcurr').getValue() + ')</span> : ' +  formatutils.formatNumber(panel.trxFeesObj.trxFee, null, null, 2) + '</span></span><br>');
                    	ocBuf.push('<span style="display:inline-block;width:100%;"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfSellText style="font-weight:bold;">' + languageFormat.getLanguage(20854, 'Total Order Value') + ' (' + input.down('#settcurr').getValue() + ')</span> : ' + formatutils.formatNumber(panel.totalValue, null, null, 2) + '</span></span><br>');
                    	ocBuf.push('</td></table>');
                    }
    ...
    //Mutual fund order process section START
    //After press subscribe
    submitBuyMF: function () {
        ...
        try {
            ...
            if (valid) {
                ...
                //Check if not skip and allowduplicate
                //easiest way to describe is following code will create a popup message
                if (!skip && panel.allowDup != "1") {
                    var currencyEl = input.down('#settcurr');
                    var defCurrency;
                    if (currencyEl && currencyEl.getValue()) {
                    ...
                    var stkname = panel.stkname;
                    var stkcode = ordPad.down('#stkcode').getValue();
                    var amount = input.down('#amount').getRawValue();
                    
                    //Use config from round qty and not price due to 1 reason
                    //Price are obtained from atp, and price are not formatted
                    //In thousand separator. But quantity is typed in my side
                    //So the formatted.
                    if (N2N_CONFIG.orderRoundQty) {
                        amount = jsutil.getNumFromRound(amount);
                    } else {
                    ...
                    msgBuf.push('<span style="font-size:1.2em;display:inline-block;font-weight:bold;">' + languageFormat.getLanguage(20877, 'Order Details') + '</span>');
                    msgBuf.push(st1 + languageFormat.getLanguage(20833, 'Account No.') + '</span>: ' + ac);
                    msgBuf.push(st1 + languageFormat.getLanguage(20837, 'Order Type') + '</span>: <span id="ordConfBuyText">' + languageFormat.getLanguage(33670, 'Subscribe') + '</span>');
                    msgBuf.push(st1 + languageFormat.getLanguage(20207, 'Order amount') + '(' + panel.currency + ')</span>: ' + panel.currency + ' ' + panel.formatNumberComa(amount));
                    ...
                    ocBuf.push('<table style="float: left;width:60%"><td>');
                    ocBuf.push('<span style="color:#000;font-size:1.2em;display:inline-block;font-weight:bold;">' + languageFormat.getLanguage(20878, 'Order Charges') + '</span>');
                    ocBuf.push(st2 + languageFormat.getLanguage(20818, 'Management Fee') + '</span>: ' + panel.formatNumberComa(panel.trxFeesObj.Commission)); //TODO CHANGE PARAM
                    ...
                    ocBuf.push('<span style="display:inline-block;width:100%;padding-top:3px"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfBuyText style="font-weight:bold;">' + languageFormat.getLanguage(20887, 'Total Charges') + '(' + input.down('#settcurr').getValue() + ')</span> : ' + panel.formatNumberComa(panel.trxFeesObj.trxFee) + '</span></span><br>');
                    ocBuf.push('<span style="display:inline-block;width:100%;"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfBuyText style="font-weight:bold;">' + languageFormat.getLanguage(20854, 'Total Order Value') + '(' + input.down('#settcurr').getValue() + ')</span> : ' + panel.currency + ' ' + panel.formatNumberComa(amount) + '</span></span><br>');
                    ocBuf.push('</td></table>');
                    //Other charges END
                    ...
                    var btnStyle = "<div><span style='float:right;padding:3px 3px 5px 0px;'>";
                    ...
                    //Msg section TODO END
                } else {
                    panel.showLoadMask(1);
                    panel.isReady = false;
                    //Format submit part by
                    panel.submitOrder();
                }
            } else {
                this.allowUpdate = true;
        ...
        try {
            ...
            if (valid) {
                ...
                if (!skip && panel.allowDup != "1") {
                    ...
                    var stkname = panel.stkname;
                    var stkcode = ordPad.down('#stkcode').getValue();
                    var quantityMF = input.down('#quantityMF').getRawValue();
                    
                    if (N2N_CONFIG.orderRoundQty) {
                        quantityMF = jsutil.getNumFromRound(quantityMF);
                    } else {
                        quantityMF = parseFloat(quantityMF);
                    }

                    //Msg section START
                    var msgBuf = new Array();
                    //1 small comment here, the confirmation message for sell for equity stock, width is 100px.
                    //I don't know why, and I will never know
                    var st1 = '<span style="width:110px;display:inline-block;font-weight:bold;">';
                    var title = '<span style="display:block; padding-bottom:5px; padding-left:3px"><b style="font-size:1.3em">' + stkname + '</b>' + '&nbsp;<span id="ordConfSellText" style="font-size:1.1em;">' + stkcode + '</span></span>';
                    //1 horizontal line	
                    msgBuf.push('<table style="float: left;border-right: 2px solid grey;width:40%"><td>');
                    msgBuf.push('<span style="font-size:1.2em;display:inline-block;font-weight:bold;">' + languageFormat.getLanguage(20877, 'Order Details') + '</span>');
                    msgBuf.push(st1 + languageFormat.getLanguage(20833, 'Account No.') + '</span>: ' + ac);
                    msgBuf.push(st1 + languageFormat.getLanguage(20837, 'Order Type') + '</span>: <span id="ordConfSellText">' + languageFormat.getLanguage(33671, 'Redeem') + '</span>');
                    msgBuf.push(st1 + languageFormat.getLanguage(20184, 'Order Qty') + '</span>: ' + panel.formatNumberComa(quantityMF));
                    ...
                    ocBuf.push('<span style="display:inline-block;width:100%;padding-top:3px"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfSellText style="font-weight:bold;">' + languageFormat.getLanguage(20887, 'Total Charges') + '</span> : ' + panel.formatNumberComa(panel.trxFeesObj.trxFee) + '</span></span><br>');
                    ocBuf.push('<span style="display:inline-block;width:100%;"><span id="ordConfAltRows" style="float:right;display:inline-block;width:100%;padding:3px 0px;"><span id=ordConfSellText style="font-weight:bold;">' + languageFormat.getLanguage(33672, 'Total Order QTY') + '</span> : ' + panel.formatNumberComa(quantityMF) + '</span></span><br>');
                    ocBuf.push('</td></table>');
                    //Other charges END
    ...
    //Submit form
    submitFormMF: function (basketOrdList, basketPin, fromBasket) {
        var panel = this;
...
//        }
        ...
        //2FA IS 2 FACTOR AUTHENTICATION SO....
        if (global_s2FARequire == 'true' && global_s2FABypass == 'YES' && global_s2FADeviceList.length == 0 && global_s2FAMobileDevice.length == 0 || global_s2FABypass == 'ALL') {
            if (!N2N1FA.return1FAValidation('ordPad')) {
                this.allowUpdate = true;
                return;
        ...
        if (isMobile) {
            ...
            ordPad.down('#msgBox').update(panel.getTrdMsgMF() + '<br>' + panel.printMsg(languageFormat.getLanguage(10020, 'Processing...'), 'font-weight:bold; color:blue;' + bgcolor));
        }
        
        panel.encrypt();
        ordPad.down('#pin').disable();
        
        var data = null;
        
        //Block duplicate order, can use the same as the equity buy sell
        var userConfirmDuplicate = userPreference.get('','0');
        if(userConfirmDuplicate == '0'){
            panel.allowDup = '1';
        }
        
        //check if basket order. but the word is still confusing.
        //basketlist????? for second part
        //a var call basketlist that save value from orderpad?????
        if (basketOrdList) {
            var params = {"selectedRecords": basketOrdList};
            var jsonRec = Ext.encode(params);
            data = {
    ...
    getTrdMsgMF: function () {
        ...
        //Field
        var act, amount;
        amount = input.down('#amount').getRawValue();
        quantityMF = input.down('#quantityMF').getRawValue();
        act = ordPad.down('#act').getValue();
        ...
        if (act == modeOrdBuy) {
            ...
            if (!Ext.isEmpty(stkcode) && !Ext.isEmpty(stkname) && !Ext.isEmpty(act)) {
                ...
                msgTxt = '<span style="font-weight: bold; color:' + color + '">';
                msgTxt += panel.getAction(act) + lineBreak;
                ...
                if (!isNaN(amount) && amount > 0) {
                    msgTxt += languageFormat.getLanguage(20871, 'Value') + " = " + panel.currency + ' ' + panel.formatNumberComa(amount);
...
//                    //
//                    msgTxt += '<br/><br/>';
//                    msgTxt += languageFormat.getLanguage(20918, 'Management Fee') + " = " + panel.currency + ' ' + panel.formatNumberComa(amount);
//                }
                return msgTxt;
            ...
            if (!Ext.isEmpty(stkcode) && !Ext.isEmpty(stkname) && !Ext.isEmpty(act)) {
                ...
                if (!isNaN(quantityMF) && quantityMF > 0) {
                    msgTxt += languageFormat.getLanguage(20871, 'Value') + " = " + panel.currency + ' ' + panel.formatNumberComa(quantityMF);
...
//                    //
//                    msgTxt += '<br/><br/>';
//                    msgTxt += languageFormat.getLanguage(20918, 'Management Fee') + " = " + panel.currency + ' ' + panel.formatNumberComa(quantityMF);
//                }
                return msgTxt;
            ...
            if(panel.action == "Buy"){
                ...
                if (!Ext.isEmpty(stkcode) && !Ext.isEmpty(stkname) && !Ext.isEmpty(act)) {
                    ...
                    msgTxt = '<span style="font-weight: bold;">';
                    msgTxt += panel.getAction(act) + lineBreak;
    ...
    //Mutual fund order process section END
...
//                        	panel.setDefaultQuantity(1);
    ...
    setDefaultQuantity: function (qty) {
        ...
        if (panel.preserveQty) { // to skip clear qty when selecting some dropdowns (account type, account no, payment)
            panel.preserveQty = false;
            return;
        }
...
//        var input = panel.trdForm.down('#inPanel');
    ...
    //On first load, load field visibility from config
    initFldCfg: function (ex) {
        ...
        try {
            ...
            //Not every broker have mf
            //Here to set initial field visibility
            //by insert config into fldCfg
            if (isMf(ex)) {
                panel.setFldConfigMF(ex);
                panel.updateScrollAmountMF();
            } else {
                panel.setFldConfig(ex);
            }
            //think of how to set default value
    ...
    setExUICfg: function (ex) {
        ...
        if (ex) {
            ...
            if (N2N_CONFIG.orderPadBasic) {
                ...
                input.down('#trailType').setVisible(cfg.trailType);
...
//            input.creditlimitPanel.setVisible(panel.fldCfg.showlimit);
                ...
                input.down('#otype').setVisible(cfg.ordertype);
                input.down('#qty').setVisible(cfg.stkqty);
                input.down('#price').setVisible(cfg.stkprice);
                input.down('#validity').setVisible(cfg.validity);
                //mutual fund
                input.down('#netcashlimit').setVisible(cfg.netcashlimit);
                input.down('#amount').setVisible(cfg.amount);
                panel.resumeLayouts();
            }
            //var confirm = '0';
    ...
    initBuySell: function () {
        ...
        try {
            ...
            if (ex) {
                ...
                if(panel.btnList.length > 0){
                	panel.setAccType(null, false);
                }                
            }
        } catch (e) {
...
//                        }
                ...
                panel.disableCmp(input.down('#cbValidity'));
                panel.disableCmp(input.down('#stoplimit'));
                panel.disableCmp(input.down('#trailType'));
                ...
                panel.disableCmp(input.down('#ordPad_stkname'));
                ...
                if (input.down('#idssCb')) {
                    input.down('#idssCb').setValue(false);
                }
                
                panel.disableCmp(input.down('#privateOrd'));

                if (isMf(ex)) {
                    panel.disableCmp(input.down('#amount'));
                    panel.disableCmp(input.down('#quantityMF'));
                }
...
//  			}
                ...
                if (isMf(exchg)) {
                    //If not mf, don't need to care, because hide already
                    if (panel.action == "Buy") {
                        panel.disableCmp(input.down('#quantityMF'));
                        panel.enableCmp(input.down('#amount'));
                    } else if (panel.action == "Sell") {
...
//      			grab enable value element
                    ...
                    if (panel.fldCfg.trailType) {
                        if (blnTrailType) {
                            panel.enableCmp(panel.compRef.trailTypeCT);
                        } else {
                            panel.disableCmp(panel.compRef.trailTypeCT);
                        }
                    }
...
//        var mode = panel.mode;	// BUY || SELL || CANCEL || REVISE
        ...
        panel.enableCmp(input.down('#ordPad_stkname'));
        panel.enableCmp(input.down('#price'));
        panel.enableCmp(input.down('#quantity'));
        panel.enableCmp(input.down('#otype'));
        panel.enableCmp(input.down('#cbValidity'));
                    ...
                    if (ordCtrl.type == currentOrderType) {
                        ...
                        if (panel.fldCfg.trailType) {
                            if (blnTrailType) {
                                ...
                                panel.disableCmp(panel.compRef.trailTypeCT);
                                panel.compRef.trailTypeCT.setValue('');
                            }
                        }
                        ...
                        // The label of field "Trigger Price" will be changed to Trigger Value/% based on the above order type respectively.
                        panel.compRef.stopLimitCt.setFieldLabel(panel._getTriggerLabel());
    ...
    //Equity stock field initial config
    setFldConfig: function (ex) {
        ...
        var fldCfg = panel.createInitFldConfig();
        ...
        if (ex && panel.arFldCfg && panel.arFldCfg.length > 0) {
            for (var i = 0; i < panel.arFldCfg.length; i++) {
                ...
                if (key && key == tempExchange) {
                    ...
                    if (cfg.ordertype && cfg.ordertype == 'Y')
                        fldCfg.ordertype = true;
                    if (cfg.stkqty && cfg.stkqty == 'Y')
                        fldCfg.stkqty = true;
                    if (cfg.stkprice && cfg.stkprice == 'Y')
                        fldCfg.stkprice = true;
                    if (cfg.validity && cfg.validity == 'Y')
                        fldCfg.validity = true;
                    if (cfg.trailType && cfg.trailType == 'Y')
                        fldCfg.trailType = true;
    ...
    //Mutual fund field initial config
    setFldConfigMF: function (ex) {
        ...
        if (ex && panel.arFldCfgMF && panel.arFldCfgMF.length > 0) {
            for (var i = 0; i < panel.arFldCfgMF.length; i++) {
                var cfg = panel.arFldCfgMF[i];
                var key = cfg.ex;
                var tempExchange = formatutils.removeDelayExchangeChar(ex);
                ...
                if (key && key == tempExchange) {
                    ...
                    if (cfg.settcurr && cfg.settcurr == 'Y')
                        fldCfg.settcurr = true;
                    //Mutual fund section
                    if (cfg.netcashlimit && cfg.netcashlimit == 'Y')
                        fldCfg.netcashlimit = true;
                    if (cfg.amount && cfg.amount == 'Y')
                        fldCfg.amount = true;
                    if (cfg.mfqty && cfg.mfqty == 'Y')
                        fldCfg.mfqty = true;
    ...
    createInitFldConfig: function () {
        ...
        //Additional field
        fldCfg.ordertype = false;
        fldCfg.stkqty = false;
        fldCfg.stkprice = false;
        fldCfg.validity = false;
        
        //Mutual fund section
        fldCfg.netcashlimit = false;
        fldCfg.amount = false;
        fldCfg.mfqty = false;
    ...
    setBalance: function (val) {
        ...
        var el = this.trdForm.down("#accountno_label2");
        
        if (el) {
            var cfg = this.fldCfg;
            if (me.exchangecode == '') {
                el.setValue('');
                el.setVisible(N2N_CONFIG.tradingLimitVisible);
                
                if (me.btnlimit.isVisible()) {
                    me.btnlimit.setVisible(false);
                }
            } else if (N2N_CONFIG.hideLimitRow && me.exchangecode == 'MY') { // totally hide trading limit row
                el.setVisible(false);
                el.setValue('');
                ...
                var trimVal = val.substring(trimValIndex + 2);
                var tradinglimit = val.split(':');
                if(tradinglimit.length > 1){
                	el.setFieldLabel(tradinglimit[0])
                	el.setValue(tradinglimit[1].trim());
                }else{
                	el.setFieldLabel(languageFormat.getLanguage(20834, 'Trading Limit'));
                	el.setValue(tradinglimit[0]);
                }
...
//        var input = this.trdForm.OrdPad.inPanel;
    ...
    setIDSS: function(val) {
        var input = this.trdForm.down('#inPanel');
        var el = input.down('#idssCb');
        if (el)
            el.setValue(val);
    },
    getIDSS: function() {
...
//  				el.setValue(val);
    ...
    setTrailType: function (val) {
        var panel = this;

        panel.compRef.trailTypeCT.setValue(val);
    },
    ...
    //Mutual fund getter and setter
    setNetCashLimit: function (val) {
        var input = this.trdForm.down('#inPanel');
        var el = input.down('#netcashlimit');
        if (el)
            el.setValue(val);
    },
    getNetCashLimit: function () {
        var input = this.trdForm.down('#inPanel');
        var el = input.down('#netcashlimit');
        if (el)
            return el.getValue();
    ...
    disableCmp: function (comp) {
        try {
            ...
            if (comp.disableCmp === false || comp.disableCmp == undefined) {
                ...
                var bgcolor = "background-color: #505050;";
...
//            return languageFormat.getLanguage(20852, 'Units');
    ...
    setAmount: function(val) {
        var el = this.trdForm.down('#amount');
        if (el) {
            if (N2N_CONFIG.orderRoundQty) {
                val = formatutils.formatNumber(val);
            }
    ...
    getAmount: function () {
        var input = this.trdForm.down('#inPanel');
        var el = input.down('#amount');
        if (el)
            return el.getValue();
        else
            return '';
    },
    setQtyMF: function(val) {
        var el = this.trdForm.down('#quantityMF');
        if (el) {
            if (N2N_CONFIG.orderRoundQty) {
                val = formatutils.formatNumber(val);
            }
...
//    },
    ...
    _openOTPwindow: function () {
        ...
        panel.tFieldOTPPin = new Ext.form.TextField({
            ...
            width: 200,
            //inputType	: 'password',
            emptyText:  languageFormat.getLanguage(30253,'Key in password to validation')
        });
            ...
            success: function (response) {
                try {
                    ...
                    if (obj.s) {
                        ...
                        msgutil.info(languageFormat.getLanguage(30254, "Success to log in."), function () {
                            panel.OTPwindow.destroy();
                        });
                    } else {
                        panel.OTPwindow.setTitle(panel.OTPwindow.title + " : " + obj.s);
    ...
    _getLotSize: function (value) {
        ...
        if(!N2N_CONFIG.enableUserPrefQtyStep){
    		panel.compRef.minQtyCt.step = parseInt(N2N_CONFIG.qtyScrollStep, 10);
    		panel.compRef.qtyCT.step = parseInt(N2N_CONFIG.qtyScrollStep, 10);
    		panel.compRef.dsqtyPanel.step = parseInt(N2N_CONFIG.qtyScrollStep, 10); 
    	}
        ...
        if(!panel._isQtyLotSize()){
        	...
        	if(!N2N_CONFIG.enableUserPrefQtyStep){
        		panel.compRef.minQtyCt.step = parseInt(value, 10);
        		panel.compRef.qtyCT.step = parseInt(value, 10);
        		panel.compRef.dsqtyPanel.step = parseInt(value, 10);
        	}
        }

        return lotSize;
    },
    _isQtyLotSize: function(){
    	var panel = this;
    	var isLotSize = true;
            ...
            if (accRet) {
                ...
                for (var i = 0; i < accList.length; i++) {
                    ...
                    if (acc.ac && accno == acc.ac) {
                        ...
                        if (acc.cliType == 'D') {
                        	al = panel.formatTradingLimitDecimal(acc.ncl); //cfd trading limit
                        }
    ...
    advBasicComponent: function () {
        ...
        //Section for visibility for field in advance view
        if (panel.fldCfg) {
            var cfg = panel.fldCfg;
            ...
            if (cfg.trailType)
                compo.push(panel.compRef.trailTypeCT);
            if (cfg.minqty)
            ...
            //If is mutual fund, don't hide when basic mode
            if (cfg.settcurr && !isMf(orderPad.exchangecode))
            ...
            if (global_sIDSSEnabled === '1')
                compo.push(panel.trdForm.down('#idssCb'));
            if (cfg.forceord)
        ...
        input.down('#stoplimit').setVisible(cfg.stoplimit);
        panel.compRef.trailTypeCT.setVisible(cfg.trailType);
        input.down('#minqty').setVisible(cfg.minqty);
        input.down('#dsqty').setVisible(cfg.dsqty);
        input.down('#settcurr').setVisible(cfg.settcurr);
        input.down('#payment').setVisible(cfg.payment);
        ...
        //Additional fund
        input.down('#otype').setVisible(cfg.ordertype);
        input.down('#quantity').setVisible(cfg.stkqty);
        input.down('#price').setVisible(cfg.stkprice);
        input.down('#cbValidity').setVisible(cfg.validity);
        
        //Mutual fund session
        input.down('#netcashlimit').setVisible(cfg.netcashlimit);
        
        if (panel.mode == modeOrdSell) {
            input.down('#amount').setVisible(false);
        } else {
            input.down('#amount').setVisible(cfg.amount);
        }
        
        if (panel.mode == modeOrdBuy) {
            input.down('#quantityMF').setVisible(false);
        } else {
            input.down('#quantityMF').setVisible(cfg.mfqty);
        }
        ...
        panel.trdForm.down('#skipconfirm').setVisible(cfg.skipconfm);
        
        panel.activateIDSS();
        
        //If no extra button in advance mode
        if (compo.length == 0) {
            panel.btnBasic.hide();
        } else {
            panel.btnBasic.show();
            for (var i = 0; i < compo.length; i++) {
                ...
                if (basic) {
                    ...
                    if (i == 0) {
                        ...
                        panel.btnBasic.setIconCls('icon-down-first');
                        //}
                    }
                } else {
                    compo[i].show();
                    if (i == 0) {
                        ...
                        panel.btnBasic.setIconCls('icon-up-first');
                        //}
                    }
                }
...
//  			Insert dragged node and target node to their specific index position.
    ...
    _setCtSize: function(formHeight, columnWidth) {
        tLog('call _setCtSize');
        ...
        var configItemBtn = me.trdForm.down('#innerMainCtConfItemsConf').getHeight();
        configItemBtn = configItemBtn == 0 || !configItemBtn ? 20: configItemBtn;

        if (!isMobile) {
            me.compRef.innerMainCt.setHeight(configItemBtn);
        }
        if (me.trdForm.down('#inPanel').getHeight() == 0) {
            if (formHeight){
                tLog('||| formhight from 0 ');
                me.compRef.formHeight = formHeight;
            }
        } else {
            me.compRef.formHeight = me.trdForm.down('#inPanel').getHeight();

            if (!isMobile) {
                me.compRef.formHeight += configItemBtn;
            }
        }
        if (columnWidth) {
            if (me.trdForm.down('#OrdPad').down('#btnPanel')) {
                me.trdForm.down('#OrdPad').down('#btnPanel').setStyle({
        ...
        var bodyHeight = me.compRef.formHeight;

        tLog('row4Cnt :' + me.trdForm.down('#row4Cnt').getHeight());
        tLog('trdForm :' + me.trdForm.getHeight());
        tLog('mainCt :' + me.compRef.innerMainCt.getHeight());
        tLog('north :' + me.trdForm.down('#northCt').getHeight());
        tLog('inPanel :' + me.trdForm.down('#inPanel').getHeight());

        if (!isMobile)
            tLog('msgPanel :' + me.trdForm.down('#msgPanel').getHeight());

        if (!isMobile && me.compRef.innerMainCt)
            tLog('innerMainCt :' + me.compRef.innerMainCt.getHeight());

        if (!isMobile && me.trdForm.down('#innerMainCtConfItemsConf'))
            tLog('innerMainCtConfItemsConf :' + me.trdForm.down('#innerMainCtConfItemsConf').getHeight());

        var topInputHeight = me.trdForm.down('#inPanel').getHeight() == 0 ?
                me.compRef.formHeight : me.trdForm.down('#inPanel').getHeight();

        if (!isMobile) {
            me.compRef.mainCt.down('#msgBox').setHeight(topInputHeight);
        }

        var winMinHeight = me.compRef.headerHeight + bodyHeight + winPadding;
        var winHeight = winMinHeight;

        tLog('bodyHeight :' + bodyHeight);
       
        if (!isMobile) {
            me.compRef.mainCt.setHeight(topInputHeight + configItemBtn - 5);
            me.compRef.mainCt.down('#northCt').setHeight(topInputHeight);
            me.trdForm.setHeight(me.compRef.headerHeight + topInputHeight + winPadding);
        }

        if (isMobile) {
            me.compRef.mainCt.setHeight(me.trdForm.down('#inPanel').getHeight() + me.trdForm.down('#optPanel').getHeight());
            me.trdForm.setHeight(me.compRef.mainCt.getHeight());

            var padd = !me.compRef.topInfo.getCollapsed() ? 40 : 68;
            var popHeight = me.compRef.mainCt.getHeight() + padd;

            if (!me.compRef.topInfo.getCollapsed()) {
                popHeight += me.compRef.topInfo.getHeight()
            }

            var appMainSize = n2nLayoutManager.getAppMain().getSize();
            if (popHeight > appMainSize.height) {
                var diff = popHeight - appMainSize.height;
                popHeight = appMainSize.height;

                // prevent scrolling in mainCt
                me.compRef.mainCt.setHeight(me.compRef.mainCt.getHeight() - diff);
            }

            pcomp.setHeight(popHeight);
            me._runningSetCtSize = false;
            me.doLayout(); // fix empty space when orderpad docked
        ...
        if (isWin) {
            ...
            if (winMinHeight > appMainSize.height) {
                if (!me.compRef.topInfo.getCollapsed()) {
                    winHeight +=  me.compRef.topInfo.getHeight();
                }
            } else {
                if (!me.compRef.topInfo.getCollapsed()) {
                    winHeight +=   me.compRef.topInfo.getHeight();
            ...
            pcomp.setHeight(winHeight + configItemBtn - 28);  ///  split hieght: 5
        }
...
//        tLog('innerMainCtConfItemsConf :' + me.trdForm.down('#innerMainCtConfItemsConf').getHeight());
    ...
    setHelpTxt: function (orderType, act) {
        ...
        if (act == modeOrdBuy) {
            if (orderType == 'StopLimit' || orderType == 'Stop') {
                helpTxt = '<br><i>' + languageFormat.getLanguage(30249, 'To submit a Buy Stop Order, Trigger Price should be > current market price (LastTrade, BestBid, BestOffer) of the counter.') + '</i>';
            } else if (orderType == 'MarketIfTouched' || orderType == 'LimitIfTouched') {
                helpTxt = '<br><i>' + languageFormat.getLanguage(30250, 'To submit a Buy If-Touch Order, Trigger Price should be < current market price (LastTrade, BestBid, BestOffer) of the counter.') + '</i>';

            }
        } else if (act == modeOrdSell) {
            if (orderType == 'StopLimit' || orderType == 'Stop') {
                helpTxt = '<br><i>' + languageFormat.getLanguage(30251, 'To submit a Sell Stop Order, Trigger Price should be < current market price (LastTrade, BestBid, BestOffer) of the counter.') + '</i>';
            } else if (orderType == 'MarketIfTouched' || orderType == 'LimitIfTouched') {
                helpTxt = '<br><i>' + languageFormat.getLanguage(30252, 'To submit a Sell If-Touch Order, Trigger Price should be > current market price (LastTrade, BestBid, BestOffer) of the counter.') + '</i>';
            }
        } else if (act == modeOrdRevise) {
            if (panel.prevAction.toLowerCase() == "buy") {
                if (orderType == 'StopLimit' || orderType == 'Stop') {
    ...
    runSearchAccount: function() {
        ...
        } else if (panel.getAccountName().trim().length < N2N_CONFIG.constDRMinChars) {
    ...
    searchAccount: function (searchValue, selectMatch) {
        ...
        if (searchValue) {
            panel.trdForm.down("#accountno").doQuery(searchValue, true);
        }
        /*
    ...
    _getFirstMatch: function (list, searchKey) {
        ...
        for (var i = 0; i < list.length; i++) {
            var accStr = list[i][0].toLowerCase();
            ...
            if (bidSize) {
                ...
                for (var i = 0; i < bidSize.length; i++) {
                    ...
                    if (
                        ...
                        me.compRef.priceCT.step = bidInt;
                        me.compRef.stopLimitCt.step = bidInt;
    ...
    getBidSize: function(stkCode, exch, sector, price) {
        var me = this;
    	var bidInt = 0;
            ...
            if (bidSize) {
                ...
                for (var i = 0; i < bidSize.length; i++) {
                    ...
                    if (
                    	...
                    	bidInt = parseFloat(bidRule.bint);
                    }
                }
            }
        }
    ...
    _getExtInputCls: function() {
        return N2N_CONFIG.orderRoundQty ? (N2N_CONFIG.enablePriceQtyScroll ? 'widget.customspinner' : 'Ext.form.field.Text') : 'Ext.form.field.Number';
    },
 ...
 _getBlurHandler: function() {
    ...
    stockNameFormat: function(val, rec) {
        var newString = "";
        var temp = val.split(".");
    ...
    filterAccountList: function(accType){
    	var panel = this;
    	panel.lastSelectedAccType = accType;
    	if (accRet) {
    		var accList = !isDealerRemisier ? accRet.ai : panel.accs;
    		var el = panel.trdForm.down("#accountno");
			if (el) {
				el.setValue('');
			}
    		for (var i = 0; i < accList.length; i++) {
    			var acc = accList[i];
    			if (acc.ac) {
    				if(accType[0].indexOf(acc.cliType) != -1){
    					
    					if (el) {
    						el.setValue(acc.accno);
    						break;
    					}
    				}
    			}
    ...
    setAccType: function(accno, isReset){
    	var panel = this;
    	var accList = !isDealerRemisier ? accRet.ai : panel.accs;
    	if(panel.btnList.length > 0){
    		if(accno){
    			if (accList && accList.length > 0) {
    				for (var i = 0; i < accList.length; i++) {
    					var acc = accList[i];
    					if (acc.accno == accno) {
    						for(var j=0; j<panel.btnList.length; j++){
    							var btn = panel.btnList[j];
    							var accType = btn.accType;
    							btn.enable();
    							if(accType[0].indexOf(acc.cliType) != -1){
    								btn.setPressed(true);
    							}else{
    			...
    			if(isReset){
    				for(var i=0; i<panel.btnList.length; i++){
    					var btn = panel.btnList[i];
    					btn.disable();
    					btn.setPressed(false);
    				}
    			}else{
    ...
    setAccMeta: function(accno) {
        tLog('setAccMeta', accno);
    ...
    clearAccMeta: function() {
        var panel = this;
        var ordPad = panel.trdForm.down('#OrdPad');

        ordPad.down('#ac').setValue('');
        ordPad.down('#cc').setValue('');
        ordPad.down('#ex').setValue('');
        ordPad.down('#branchcode').setValue('');
        ordPad.down('#brokercode').setValue('');

    },
    isCFDAcc: function(accountNo){
    	var panel = this;
    	var isCFD = false;
    	var accno = '';
    	...
    	if(accRet){
    		var accList = accRet.ai;
    		if(accountNo){
    			accno = accountNo.split(global_AccountSeparator)[0];
    		}else{
    			accno = panel.getAccountNo();
    		}
    ...
    _getLabelWidth: function() {
        // get width of the longest label
        var lWidth = ofmx.getWidth('Trading Limit');

        if (lWidth == 0) {
            return 75; // default if the returned width is wrong
        }

        return lWidth + 10; // with some padding

    },
    _getSkipConLabelWidth: function() {
        var lWidth = ofmx.getWidth(languageFormat.getLanguage(20846, 'Skip Confirmation'));

        if (lWidth == 0) {
            return 130; // default if the returned width is wrong
        }

        return lWidth + 20; // width some padding
    },
    ajdustAllLabels: function() {
        var me = this;
        var inPanel = me.trdForm.down('#inPanel');
        var lWidth = me._getLabelWidth();
        var lWidth2 = me._getSkipConLabelWidth();

        me._setLabelWidth(inPanel, '#accountno_label2', lWidth);
        me._setLabelWidth(inPanel, '#accountno', lWidth);
        me._setLabelWidth(inPanel, '#ordPad_stkname', lWidth);
        me._setLabelWidth(inPanel, '#cbAction', lWidth);
        me._setLabelWidth(inPanel, '#dsqty', lWidth);
        me._setLabelWidth(inPanel, '#cbValidity', lWidth);
        me._setLabelWidth(inPanel, '#price', lWidth);
        me._setLabelWidth(inPanel, '#quantity', lWidth);
        me._setLabelWidth(inPanel, '#otype', lWidth);
        me._setLabelWidth(inPanel, '#tptype', lWidth);
        me._setLabelWidth(inPanel, '#tpdirection', lWidth);
        me._setLabelWidth(inPanel, '#stoplimit', lWidth);
        me._setLabelWidth(inPanel, '#trailType', lWidth);
        me._setLabelWidth(inPanel, '#minqty', lWidth);
        me._setLabelWidth(inPanel, '#settcurr', lWidth);
        me._setLabelWidth(inPanel, '#payment', lWidth);

        var skipCon = me.down('#skipconfirm');
        if (skipCon) {
            skipCon.setWidth(lWidth2);
        }

    },
    _setLabelWidth: function(inPanel, labelId, width) {
        var tradeLimit = inPanel.down(labelId);
        if (tradeLimit && tradeLimit.labelEl) {
            tradeLimit.labelEl.setWidth(width);
        }
    },
    startEdit: function() {
        var me = this;
        ...
        // focus on qty field
        me.compRef.qtyCT.focus();
    },
    setStock: function(stkCode, stkName) {
        var me = this;
        var dataRow = {};
        dataRow[fieldStkCode] = stkCode;
        dataRow[fieldStkName] = stkName;

        me.compRef.stkCb.store.loadData([dataRow]);
        me.compRef.stkCb.setValue(stkCode);
    },
    disableCFDTradingLimit: function(accno){
    	var panel = this;
    	if(!N2N_CONFIG.showCFDLimit){
    		if(panel.isCFDAcc(accno)){
    			panel.disableCmp(panel.trdForm.down("#accountno_label2"));
    		}else{
    			panel.enableCmp(panel.trdForm.down("#accountno_label2"));
    		}
    	}
    },
    focusSearchBox: function() {
        focusManager.focusSearchbox(this.compRef.stkCb);
    },
    //New way of display result
    submitBasketResult: function (form, action) {
        var panel = this;

        console.log('[OrderPad][submitBasketResult] start *** ');
        var bgcolor = "";
        ...
        try {
            ...
            if (!isMobile) {
                if (isMf(panel.exchangecode)) {
                    msgTxt = panel.getTrdMsgMF() + '<br>';
                } else {
                    msgTxt = panel.getTrdMsg() + '<br>';
                }
            }
            var msg = '';
            var resultList = action.result.d;
            if(!resultList){
...
//                          msgutil.alert('Ajax communication failed', null, 'Failure');
                        ...
                        case Ext.form.Action.SERVER_INVALID:
                        {
                            if (result.t == 'trd' || result.t == 'ro' || result.t == 'co') {
                                resetPin = false;
                                msg = result.em;
                                if ((result.ec && result.ec == '502') || (result.ec && result.ec == '509')) {
                                	if(resultList.length == 1){
                                		Ext.Msg.show({
                                			...
                                			fn: function (btn, text) {
                                				if (btn == 'ok') {
                                                                    if (result.jObj && result.jObj.TktNo) {
                                                                        panel.setConfirmation('1');
                                                                        ordPad.down('#tktno').setValue(result.jObj.TktNo);
                                                                        panel.submitForm();
                                                                    } else {
                                                                        tLog('result.jObj', result.jObj);
                                                                    }
                                				}
                                			},
                                			icon: Ext.Msg.QUESTION
                                		...
                                		var rec = result.jObj;
                                		var stkcode = result.sc;
                                		var obj = {sc: stkcode, msg: msg, rec: rec, confirm: false};
                                		actionConfirmationList.push(obj);
                                	}
                                } else if ((result.ec && result.ec == '510')) {
                                    //for CIMBSG CUT 'Error 510  50||There is insufficient cash balance to proceed with this order. Please use Cash Top-up service to top-up your Investment Power|InsufficientValue=xxx'
                                    var tempMsg = result.em.split('|');
                                    var tempKeyArray;
                                    var key = '';
...
//                                            myRef.focus(); // focus popup.
//                                        });
                                } else if ((result.ec && result.ec == '513')) {
                                    //for CIMBSG CUT 'Error 513  50||You do not have sufficient funds to place an order. Click on Convert button to perform FX conversion to increase your investment power.
                                    var tempMsg = result.em.split('|');
                                    var tempKeyArray;
                                    var key = '';
                                	...
                                	if (result.em)
                                    	...
                                    	msg = languageFormat.getLanguage(30238, 'Please try again.');
                                }
                            } else if (result.t == 'a') {
                                if (result.m) {
                                    msg = result.m;
...
//                              msgutil.alert(msg, null, 'Failure');
            ...
            if(actionConfirmationList.length > 0){
            	var buttonList = [];

            	buttonList.push({
            		text: 'OK',
            		handler: function() {
            			var ls = actGrid.getStore().getData();
                        var n = ls.length;
                        var basketList = [];
                        for (var i = 0; i < n; i++) {
                        	var r = ls.items[i].data.rec;
                        	if(r.Confirm == '1'){
                            	basketList.push(r);
                        	}
                        }

                        var pin = basketOrderPanel.down('#pinw').getValue();
                        var pin2 = encryptedString(panel.keyPair, pin);
                        orderPad.submitForm(basketList, pin2);
                        
                        msgutil.info('Order(s) Submitted. Please check order book for details.');

                        panel.actConfWin.close();
                        panel.actConfWin = null;
            		}
            	});
            	...
            	var actGrid = Ext.create('Ext.grid.Panel', {
                    store: {
                        fields: ['sc', 'msg', 'rec', 'confirm']
                    },
                    viewConfig: {
                        stripeRows: true,
                        trackOver: true
                    ...
                    columns: [
                        {
                        	...
                        	width: 45,
                        	listeners:{ 
                        		checkchange: function(thisCheckCol, rowIndex, checked) {
                        			var store = actGrid.getStore();
                        			var rec = store.getAt(rowIndex);
                        			var val = checked;

                        			if(checked){
                        				var jObj = rec.get('rec');
                        				jObj.Confirm = '1';
                        				rec.set('rec', jObj);
                                    	store.commitChanges();
                        			}else{
                        ...
                        {text: "Code", dataIndex: 'sc', width: 130, sortable: true, menuDisabled: true},
                        {text: "Message", dataIndex: 'msg', width: 335, flex: 1, sortable: false, menuDisabled: true}
                    ],
                    buttons: buttonList
                });
            	
            	actGrid.store.loadData(actionConfirmationList);
            	//actGrid.store.sort('datetime', 'DESC');

                if (!panel.actConfWin) {
                    panel.actConfWin = msgutil.popup({
...
//            if (panel.fldCfg.confirm == true) confirm = '1';
            ...
            console.log('[OrderPad][submitBasketResult] Exception ---> ' + e);
        }
        panel.allowUpdate = true;
        panel.isReady = true;
        panel.callAccBal();
        //Refresh component if it is opened
        if (orderStatusPanel != null) {
            orderStatusPanel.search(true);
        }
        if (mfOrderStatusPanel != null) {
    ...
    //get hidden form from orderpad
    getTradeFormValues: function () {
        var panel = this;
        var selectedRecords = [];
        var r = panel.trdForm.getValues();
        var a = new Object();
        if (basketOrderPanel) {
            var datetime = new Date().getTime();
            a.uId = basketOrderPanel.cbBasket.getDisplayValue() + '_' + datetime;
        }
        a.TktNo = r.tktno;
        a.OrdNo = r.ordno;
        a.SubOrdNo = r.subordno;
        a.StkCode = r.stkcode;
        a.AccNo = r.ac;
        a.BCode = r.branchcode;
        a.Act = r.act;
        a.PrevAct = r.prevaction;
        a.Validity = r.validity;
        if(isMf(panel.exchangecode)){
            if (r.act == modeOrdBuy) {
                a.OrdQty = 0;
            } else if (r.act == modeOrdSell) {
                a.OrdQty = r.quantityMF;
            } else if (r.act == modeOrdRevise || r.act == modeOrdCancel) {
                if (panel.action == "Buy") {
                    a.OrdQty = 0;
                } else if (panel.action == "Sell") {
                    a.OrdQty = r.quantityMF;
            ...
            a.OrdQty = r.quantity * r.lotsize;
        }
        a.OrdPrc = r.price;
        a.ExpDate = r.gtd;
        a.Currency = r.settcurr;
        a.OrdType = r.otype;
        a.LotSize = r.lotsize;
        a.StopLimit = r.stoplimit;
        a.MinQty = r.minqty;
        a.DsQty = r.dsqty;
        a.Confirm = r.confirm;
        a.TPType = r.tptype;
        a.TPDirection = r.tpdirection;
        a.SettOpt = r.payment;
        a.ClientCode = r.cc;
        a.AccExchCode = r.ex;
        a.BrokerCode = r.brokercode;
        a.MtQty = r.mtqty;
        a.UnMtQty = r.unmtqty;
        a.TrailType = r.trailType;
        
        //Mutual fund section
        if (r.act == modeOrdSell) {
            a.Amount = 0;
        } else if (r.act == modeOrdBuy) {
            a.Amount = r.amount;
        } else if (r.act == modeOrdRevise || r.act == modeOrdCancel) {
                if (panel.action == "Buy") {
                    a.Amount = r.amount;
        ...
        if (r.shortsell && r.act == 'S') {
            a.ShortSell = '262144';
        }

        if (r.idss === 'on' && (r.act == 'B' || r.act == 'S')) {
            a.IDSS = '262144';
        }
    ...
    submitOrder: function () {
        ...
        if (!panel.isBasket) {
            ...
            var isAdd = false;
            if (basketOrderPanel) {
                //check basket order panel
                if (basketOrderPanel.store.getCount() >= N2N_CONFIG.maxOrdersinBasket) {
                    msgutil.alert(languageFormat.getLanguage(30256,'We regret to inform that we are unable to add order to your basket as you have reached your maximum number of orders.'));
                } else {
                    var records = panel.getTradeFormValues();
                    basketOrderPanel.store.add(records);
                    basketOrderPanel.getView().refresh();
                    basketOrderPanel.isAddBasket = true;
                    isAdd = true;
                    msgutil.alert(languageFormat.getLanguage(30255, 'Order has been added to basket.'), null, 'Success');
                }
            } else {
                var basketList = localStorage.getItem(loginId + '_BasketList');
                if (basketList || basketList == "") {
                    // get record and stringify it                  
                    var records = panel.getTradeFormValues();
                    records = JSON.stringify(records[0]);

                    //Get basket order record and split it, then add records
                    var basketListArray = basketList == '' ? [] : basketList.split('|');
                    basketListArray.push(records);

                    //Stringify the whole basketListArray array to add into storage
                    var recordList = '';
                    for (var i = 0; i < basketListArray.length; i++) {
                        recordList += basketListArray[i];
                        if (i != (basketListArray.length - 1)) {
                            recordList += '|';
                        }
                    }
                    msgutil.alert(languageFormat.getLanguage(30255, 'Order has been added to basket.'), null, 'Success');
            ...
            if (isAdd) {
                panel.resetStockOrder();
            }
            panel.allowUpdate = true;
        }
    },
    resetStockOrder: function() {
        var panel = this;
        var ordPad = panel.trdForm.down('#OrdPad');
        if (!ordPad.down('#chkSavePin').getValue())
            ordPad.down('#pin').setValue('');
        var input = panel.trdForm.down('#inPanel');
        input.down('#price').setValue('');
        input.down('#quantity').setValue('');
    ...
    reupdateLotSize: function() {
        var me = this;
        var lotSize = null;

        if (!me.lotsize || me.lotsize == 0) {
            if (me._isQtyLotSize()) {
                lotSize = lotSizeArchives.getLotSize(me.stkcode);
            } else {
                lotSize = 1;
            }

            if (lotSize) {
                tLog('reupdateLotSize', lotSize);
    ...
    getPreviousPayment: function(paymentList, payment) {

        if (paymentList && payment) {
            for (var i = 0; i < paymentList.length; i++) {
                var pmn = paymentList[i];

                if (payment.toLowerCase() === pmn[0].toLowerCase()) {
                    return pmn[0]; // found payment in the list so it's a valid previous payment
                }
            }
        }
    ...
    activateIDSS: function () {
        ...
        ex = stockutil.getExchange(stkcode);
        board = getStkBoard(stkcode);

        if (N2N_CONFIG.orderPadExchgIdss.length > 0 && N2N_CONFIG.orderPadExchgIdss.indexOf(ex) != -1 && N2N_CONFIG.orderPadBoardIdss.indexOf(board) != -1) {
            // follow atp flags
            if (global_sIDSSEnabled === '1') {
                if (global_sIDSSOption === '0') { // allowed both
                    if (panel.mode == modeOrdBuy || panel.mode == modeOrdSell) {
                        input.down('#idssPanel').setVisible(true);
                    } else {
                        input.down('#idssPanel').setVisible(false);
                    }
                } else if (global_sIDSSOption === '1') { // allowed buy
                    if (panel.mode == modeOrdBuy) {
                        input.down('#idssPanel').setVisible(true);
                    } else {
                        input.down('#idssPanel').setVisible(false);
                    }
                } else if (global_sIDSSOption === '2') { // allowed sell
                    if (panel.mode == modeOrdSell) {
                        input.down('#idssPanel').setVisible(true);
                    } else {
                        input.down('#idssPanel').setVisible(false);
    ...
    proceedOnEnter: function() {
        var me = this;

        if (N2N_CONFIG.orderPadTradeOnEnter) {
            me.compRef.btnSubmit.handler();
        }
    },
    //Update scroll amount
    updateScrollAmountMF: function () {
        var panel = this;
        var input = panel.trdForm.down('#inPanel');
        
        if (panel.minInvest) {
            input.down('#amount').step = parseInt(panel.minInvest);
        } else {
            input.down('#amount').step = parseInt(N2N_CONFIG.mfAmtScrollStep);
        }
        
        if (panel.minRedeem) {
            input.down('#quantityMF').step = parseInt(panel.minRedeem);
        } else {
            input.down('#quantityMF').step = parseInt(N2N_CONFIG.mfQtyScrollStep);
        }
    },
    //Copy from the confirm message
    popupMsgMF: function() {
        
        var tradeMFMsg = msgutil.popup({
            title: global_popUpMsgTitle,
            //Submit orderMF
            width: 350,
            items: {
                ...
                items: [
                    {
                        xtype: 'container',
                        html: languageFormat.getLanguage(20208, 'Your account is not allowed to trade these funds yet. You need to complete your investor profile first.')
                    },
                    {
                            xtype: 'container',
                            ...
                            items: [
                                {
                                    xtype: 'button',
                                    text: languageFormat.getLanguage(20210, 'Do Now'),
                                    width: 75,
                                    style: 'margin-right: 6px',
                                    cls: 'fix_btn',
                                    handler: function () {
                                ...
                                {
                                    xtype: 'button',
                                    text: languageFormat.getLanguage(20211, 'Later'),
                                    width: 75,
                                    cls: 'fix_btn',
                                    handler: function () {
                                        //Close this window
    ...
    _getTriggerLabel: function () {
        var panel = this;
        var orderType = panel.compRef.orderTypeCT.getValue();

        if (orderType === 'TrailStopPercent') {
            return languageFormat.getLanguage(20919, 'Trigger %');
        } else if (orderType === 'TrailStopSteps') {
            return languageFormat.getLanguage(20920, 'Trigger Value');
        } else {
            return languageFormat.getLanguage(20842, 'Trigger Price');
        }
    }
});
