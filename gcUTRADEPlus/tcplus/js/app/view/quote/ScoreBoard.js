...
//        } else {
    ...
    _setCookieId: function() {
        var me = this;
        me.cookieKey = columnWidthSaveManager.getCookieColKey('scoreboard');
        me.paramKey = N2N_CONFIG.constKey + 'SB';
        me.colWidthKey = 'sb';
    },
    _createMainPanel: function() {
        ...
        var topBarItems = panel._getSectorUI();
        // top toolbar
        panel.compRef.topBar = Ext.create('Ext.toolbar.Toolbar', {
            items: topBarItems,
            hidden: topBarItems.length < 2 // hide toolbar when there are less than two items
        });
        ...
        panel.gridPanel = Ext.create('Ext.grid.Panel', {
            ...
            listeners: {
                ...
                columnresize: function(ct, column, newWidth) {
                    ...
                    helper.updateColWidthCache(panel, panel.cookieKey, column, newWidth, column.dataIndex);
                    
                    if (N2N_CONFIG.autoQtyRound) {
                        helper.refreshView(panel.gridPanel);
                    }
                },
                itemdblclick: function(gridView, record) {
                    if (record) {
                        // filter quotescreen
                        var secList = record.get(fieldIndexCode);
                        var sec = record.get(fieldSectorCode);

                        if (secList && sec && quoteScreen) {
                            n2nLayoutManager.activateItem(quoteScreen);
                            quoteScreen.loadQuote(secList, sec);
                        }
                    }
                },
                cellclick: function(thisView, td, cellIndex, record) {
                    panel._updatePieChart(cellIndex, record);
                },
                click: {
                    element: 'body',
                    delegate: '.x-grid-row-summary',
                    fn: function() {
                        panel._updatePieChart(0, Ext.create('TCPlus.model.ScoreBoard', panel.boardRec));
                    }
                },
                dblclick: {
                    element: 'body',
                    delegate: '.x-grid-row-summary',
                    fn: function() {
                        if (panel.sectorId && quoteScreen) {
                            n2nLayoutManager.activateItem(quoteScreen);
                            quoteScreen.loadQuote(panel.sectorId);
                        }
                    }
                }
            }
...
//                }],
    ...
    getColConfig: function() {
        ...
        return [{
                ...
                summaryType: function(records) {
                    var label = panel.sectorId;
                    var meta = {};
                    newSetting(meta, null, "string");
                    
                    panel.boardRec[fieldSbId] = panel.sectorId;
                    
                    return Ext.String.format('<div class="{0}">' + label + ' ({1})</div>', meta.css, records.length);
                },
                ...
                summaryType: function(records) {
                    var label = panel.sectorText || languageFormat.getLanguage(11035, 'Total');
                    var meta = {};
                    newSetting(meta, null, "string");
                    
                    panel.boardRec[fieldSbName] = label;
                ...
                summaryRenderer: function(value) {
                    ...
                    panel.boardRec[fieldSbVol] = value;

                    return Ext.String.format('<div class="{0}">{1}</div>', meta.css, formatutils.formatNumber(value, panel.getWidth(fieldSbVol)));
                }
            }, {
                ...
                summaryRenderer: function(value) {
                    var meta = {};
                    newSetting(meta, value, "up");
                    panel.boardRec[fieldSbUp] = value;

                    return Ext.String.format('<div class="{0}">{1}</div>', meta.css, formatutils.formatNumber(value));
                }
            }, {
                ...
                summaryRenderer: function(value) {
                    var meta = {};
                    newSetting(meta, value, "down");
                    panel.boardRec[fieldSbDown] = value;
                ...
                summaryRenderer: function(value) {
                    var meta = {};
                    newSetting(meta, value, "string");
                    panel.boardRec[fieldSbUnchg] = value;
                ...
                summaryRenderer: function(value) {
                    var meta = {};
                    newSetting(meta, value, "string");
                    panel.boardRec[fieldSbUntrd] = value;
                ...
                summaryRenderer: function(value) {
                    var meta = {};
                    newSetting(meta, value, "string");
                    panel.boardRec[fieldSbVal] = value;

                    return Ext.String.format('<div class="{0}">{1}</div>', meta.css, formatutils.formatNumber(value, panel.getWidth(fieldSbVal)));
                }
            }
        ];
    },
    genChart: function(up, down, unchg, untrd) {
        ...
        var upLabel = languageFormat.getLanguage(11031, 'Up') + ' (' + up + ')';
        var dwnLabel = languageFormat.getLanguage(11032, 'Down') + ' (' + down + ')';
        var unchgLabel = languageFormat.getLanguage(11036, 'Unchange') + ' (' + unchg + ')';
        var untrdLabel = languageFormat.getLanguage(11037, 'Untraded') + ' (' + untrd + ')';
            ...
            for (var j = 0; j < bi.length; j++) {
                ...
                if (item['bid'] == arr[i].id) {
                    ...
                    */
                    if (insert) {
                        if (!panel.sectorId) {
                            panel.sectorText = sbtbItem;
                            panel.sectorId = sbtbItemId;
                        }
                        ...
                        var menuItem = {
                            ...
                            text: sbtbItem,
                            handler: function() {
                                panel.sectorText = this.text;
                                panel.sectorId = this.secId;
...
//            "enablesmartlabels":"0"
    ...
    _updatePieChart: function(cellIndex, record) {
        var panel = this;

        if (isMobile) {
            if (cellIndex === 0) {
                if (!panel.chartPanel) {
                    panel.chartPanel = msgutil.popup({
                        title: "Pie Chart",
                        width: 300,
                        height: 300,
