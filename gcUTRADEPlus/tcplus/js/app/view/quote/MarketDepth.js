...
//    },
    ...
    initComponent: function() {
        ...
        panel.showHideInfobar = userMarketDepthInfoBar;
                ...
                listeners: {
                    select: function(thisCb, records) {
                        if (records.length > 0) {
                            ...
                            panel.setCode(rec.get(fieldStkCode), rec.get(fieldStkName));
                            n2nLayoutManager.updateKey(panel);
                            panel.refresh();
                            
                            if (N2N_CONFIG.syncSibling) {
                                syncGroupManager.syncAllComps(panel, rec.get(fieldStkCode), rec.get(fieldStkName));
                            }                            
                        }
                    }
                },
        ...
        this.topToolbar = Ext.create('Ext.toolbar.Toolbar', {
            ...
            items: [
                panel.tField_Search,
                {
                    ...
                    //  xtype: 'tbbutton',
                    iconCls: 'icon-search',
                    hidden: N2N_CONFIG.searchAutoComplete,
                    handler: function() {
                        panel._procCallSearch();
                    }
                },
        ...
        var lStyle = 'font-weight:bold;line-height:19px;font-size: ' + fontsize + ';display: inline-block;width: 100%;text-align:center;';
        
        this.tOpen = new Ext.form.Label({
            html: getPriceString(null, null, languageFormat.getLanguage(10512, 'O') + ': '),
            style: lStyle
        });

        this.tHigh = new Ext.form.Label({
            html: getPriceString(null, null, languageFormat.getLanguage(10513, 'H') + ': '),
            style: lStyle
        });

        this.tLow = new Ext.form.Label({
            html: getPriceString(null, null, languageFormat.getLanguage(10514, 'L') + ': '),
            style: lStyle
        });

        this.tLastDone = new Ext.form.Label({
            html: getPriceString(null, null, languageFormat.getLanguage(10515, 'LD') + ': '),
            style: lStyle
        });

        this.toolbarInfo = new Ext.container.Container({
            dock: 'top',
            xtype: 'toolbar',
            width: '100%',
            hidden: !(N2N_CONFIG.marketDepthInfoBar && panel.showHideInfobar),
            layout: {
                type: 'table',
                columns: 4,
                tableAttrs: {
        ...
        Ext.apply(this, {
            ...
            cls: 'fix_width',
            keyEnabled: N2N_CONFIG.keyEnabled && !panel.isMatrix && panel.type !== 'depthorderpad',
            style: panel.selectionHandler ? 'border: 1px solid transparent': '',
            listeners: {
                ...
                afterrender: function (thisComp) {
                    ...
                    if (thisComp.el) {
                        ...
                        if (typeof thisComp.selectionHandler === 'function') {
                            thisComp.el.on('click', function() {
                                thisComp.selectionHandler(thisComp);
                                
                                // sync group
                                if (thisComp.stkcode && thisComp.stkname) {
                                    syncGroupManager.syncAllComps(thisComp.cardComp._grid, thisComp.stkcode, thisComp.stkname);
                                }
                            });
                        }

                        // add context menu
                        if (thisComp.enableContextMenu) {
                            thisComp.el.on('contextmenu', function(e) {
                                // select card on right click
                                thisComp.selectionHandler(thisComp);
                                if (panel.cardComp && panel.cardComp._grid && panel.cardComp._grid.alias[0] == 'widget.watchlist') {
                                    panel.cardComp._grid.showAt(thisComp.stkcode, thisComp.stkname, e);
                                } else {
                                    mainContextMenu.showAt(thisComp.stkcode, thisComp.stkname, e);
                                }
                            });
                        }
                    }
                ...
                columnresize: function (ct, column, newWidth, eOpts) {
                    if (global_allowCookies == "TRUE" && column.width != null) {
                        ...
                        panel.tempWidth = cookies.procTempCookies(panel, panel.cookieKey, panel._idMap[colId], newWidth);
            ...
            dockedItems: [this.topToolbar,this.toolbarInfo],
    ...
    procColumnWidth: function () {
        ...
        if (cookies.isCookiesExist(panel.cookieKey)) {
            var temp = cookies.getCookies(panel.cookieKey);
            columnID = temp[0];
            columnWidth = temp[1];
    ...
    _procColumnModel: function (type) {
        ...
        try {
            ...
            if (panel.isGradient != "gradient") {
                if (panel.body.hasCls("marketdepth")) {
                    panel.body.removeCls("marketdepth");
                }
            } else if (!isMobile) {
                if (!panel.body.hasCls("marketdepth")) {
                    panel.body.addCls("marketdepth");
                }
            }
            ...
            success: function (res) {
                if (res.s && res.d && res.d.length > 0) {
                    ...
                    if (lotSize && global_displayUnit.toLowerCase() == 'lot') {
                        var mdQtyFid = normalMDFidManager.getQtyFid(panel.normalLevel);
                        
                        for (var i = 0; i < mdQtyFid.length; i++) {
                            if (dataObj[mdQtyFid[i]]) {
                                dataObj[mdQtyFid[i]] /= lotSize;
                            }
                        }
                    }
                    panel.setStkName(dataObj[fieldStkName]);
                    panel._updateNormalType(dataObj);
                    // if (!panel.isMatrix || panel.fromSearch) {
                        panel.fromSearch = false;
     ...
     * 
     */
    _updateInfoBar: function(dataObj) {
        var panel = this;
        ...
        if (!N2N_CONFIG.marketDepthInfoBar) { // need to update belows if infobar disabled
            return;
        }
        
        var open = dataObj[fieldOpen];
        if (open != null) {
            helper.setHtml(panel.tOpen, getPriceString(open, panel.lacp, languageFormat.getLanguage(10512, 'O') + ': '));
        }

        var high = dataObj[fieldHigh];
        if (high != null) {
            helper.setHtml(panel.tHigh, getPriceString(high, panel.lacp, languageFormat.getLanguage(10513, 'H') + ': '));
        }

        var low = dataObj[fieldLow];
        if (low != null) {
            helper.setHtml(panel.tLow, getPriceString(low, panel.lacp, languageFormat.getLanguage(10514, 'L')  + ': '));
        }

        var lastDone = dataObj[fieldLast];
        if (lastDone != null) {
            helper.setHtml(panel.tLastDone, getPriceString(lastDone, panel.lacp, languageFormat.getLanguage(10515, 'LD') + ': '));
        }
    },
    /**
     * Description <br/>
     *
     * 		load normal level market depth result into Ext.Store / Ext.grid
     *
    ...
    _updateNormalType: function (dataObj) {
        ...
        if (dataObj && dataObj[fieldStkCode] == panel.stkcode) {
            try {
                ...
                var finalTotalObj = {};
                finalTotalObj.level = 'normal_' + panel.normalLevel;
                finalTotalObj[ cmap_mdNo ] = '###';
                var bcum = 0;
                var scum = 0;
                for (var i = 0; i < panel.normalLevel; i++) {
    ...
    getFieldList: function () {
        var panel = this;
        var fieldList = normalMDFidManager.getFid(panel.normalLevel);
        ...
        if (N2N_CONFIG.marketDepthInfoBar) {
    ...
    _procCallMultiTypeAjax: function () {
        ...
        var infoFids = [fieldStkCode, fieldLotSize, fieldLacp];
        if (N2N_CONFIG.marketDepthInfoBar) {
            infoFids.push(fieldOpen, fieldHigh, fieldLow, fieldLast);
        }

        if (panel._infoExists(tempRecord, infoFids)) {
            panel.lotSize = tempRecord[fieldLotSize];
            panel._updateInfoBar(tempRecord);
            ...
            conn.getStockInfo({
                ...
                success: function(res) {
                    if (res.s && res.d.length > 0) {
                        var dObj = res.d[0];
                        panel.lotSize = dObj[fieldLotSize];
                        panel._updateInfoBar(dObj);
                        conn.getMDL(requestObj);
                    }
                }
            });
    ...
    _infoExists: function(rec, fids) {
        if (rec) {
            for (var i = 1; i < fids.length; i++) { // skip the first fid (stock code)
                var f = fids[i];
                if (!rec[f]) {
                    return false;
                }
     ...
     * Description <br/>
     *
     * 		update normal market depth record
     *
     * @param {object} dataObj
     */
    ...
    _updateNormalTotal: function () {
        ...
        if (panel.showTotalBidAsk) {
            ...
            var normalLevel = panel.normalLevel;
            var storeRecord = panel.store.getAt(normalLevel);
            
            if (storeRecord.get(cmap_mdBSplit) != column_1) { // bid : split
                panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdBSplit), column_1, 'up');
            }
            if (storeRecord.get(cmap_mdBidQty) != column_3) { // bid : qty
                panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdBidQty), column_3, 'up');
            ...
            if (storeRecord.get(cmap_mdBidPrice) != tempResultBid) { // bid : price
                if (isNaN(tempResultBid)) {
                    panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdBidPrice), parseFloat(0).toFixed(decimalPlace), 'up');
                } else {
                    panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdBidPrice), parseFloat(tempResultBid).toFixed(decimalPlace), 'up');
                }
            }
            ...
            if (storeRecord.get(cmap_mdAskPrice) != tempResultAsk) { // ask : price
                if (isNaN(tempResultAsk)) {
                    panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdAskPrice), parseFloat(0).toFixed(decimalPlace), 'up');
                } else {
                    panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdAskPrice), parseFloat(tempResultAsk).toFixed(decimalPlace), 'up');
                }
            }
            if (storeRecord.get(cmap_mdAskQty) != column_6) { // ask : qty
                panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdAskQty), column_6, 'up');
            }
            if (storeRecord.get(cmap_mdSSplit) != column_8) { // ask : split
                panel._updateGridCell(normalLevel, panel.allColumn.indexOf(cmap_mdSSplit), column_8, 'up');
            }
            /*
    ...
    _updateMultiRecord: function (dataObj) {
        ...
        try {
            ...
            if (dataObj[ '105' ] == '1') { // add
                ...
                var baQty = dataObj['58'];
                // fix display as lot
                if (global_displayUnit.toLowerCase() == "lot" && panel.lotSize > 1 && baQty) {
                    baQty = baQty / panel.lotSize;
                }

                if (dataObj[ '106' ] == '0') { // both column
                    panel._updateGridCell(dataObj[ '107' ], '4', dataObj[ '68' ], 'up'); // bid price
                    panel._updateGridCell(dataObj[ '107' ], '3', baQty, 'up'); // bid volume
                    panel._updateGridCell(dataObj[ '107' ], '1', dataObj[ '170' ], 'up'); // bid split

                    panel._updateGridCell(dataObj[ '107' ], '5', dataObj[ '68' ], 'up'); // ask price
                    panel._updateGridCell(dataObj[ '107' ], '6', baQty, 'up'); // ask volume
    ...
    refresh: function(themeName) {
        ...
        if (N2N_CONFIG.marketDepthInfoBar) {
            if (panel.showHideInfobar) {
                helper.setHtml(panel.tOpen, getPriceString(null, null, languageFormat.getLanguage(10512, 'O') + ': '));
                helper.setHtml(panel.tHigh, getPriceString(null, null, languageFormat.getLanguage(10513, 'H') + ': '));
                helper.setHtml(panel.tLow, getPriceString(null, null, languageFormat.getLanguage(10514, 'L') + ': '));
                helper.setHtml(panel.tLastDone, getPriceString(null, null, languageFormat.getLanguage(10515, 'LD') + ': '));
                panel.toolbarInfo.show();
            } else {
                panel.toolbarInfo.hide();
            }
        }
...
//                                }
     ...
     * Description <br/>
     *
     * 		return normal level market depth cell info
     *
     * @param {integer} 	row
     * @param {integer} 	colum
    ...
    _returnCellInfo: function (row, colum, fieldId) {
        ...
        if (normalLevel > 5) {
            fieldList.push([fieldBSplit6, '', fieldBqty6, fieldBuy6, fieldSell6, fieldSqty6, '', fieldSSplit6]);
        }
        if (normalLevel > 6) {
            fieldList.push([fieldBSplit7, '', fieldBqty7, fieldBuy7, fieldSell7, fieldSqty7, '', fieldSSplit7]);
        }
        if (normalLevel > 7) {
            fieldList.push([fieldBSplit8, '', fieldBqty8, fieldBuy8, fieldSell8, fieldSqty8, '', fieldSSplit8]);
        }
        if (normalLevel > 8) {
            fieldList.push([fieldBSplit9, '', fieldBqty9, fieldBuy9, fieldSell9, fieldSqty9, '', fieldSSplit9]);
        }
        if (normalLevel > 9) {
            fieldList.push([fieldBSplit10, '', fieldBqty10, fieldBuy10, fieldSell10, fieldSqty10, '', fieldSSplit10]);
        }
    ...
    setting: function () {
        ...
        if (mdSetting == null) {
            ...
            var mkDepthCfgInfobarItem = new Ext.container.Container({
                layout: 'table',
                padding: '0 5 0 5',
                border: false,    
                hidden: !N2N_CONFIG.marketDepthInfoBar,
                defaults: {
                    baseCls: ''
                },
                showHideInfobar: panel.showHideInfobar,
                items: [
                    new Ext.form.field.Radio({
                        name: "mkDepthCfgInfobarRadio",
                        width: 170,
                        labelWidth: 110,
                        boxLabel: languageFormat.getLanguage(21107, "Show"),
                        fieldLabel: languageFormat.getLanguage(10516, 'Show/Hide Infobar'),
                        labelSeparator: '',
                        checked: panel.showHideInfobar,
                        hidden: !N2N_CONFIG.marketDepthInfoBar,
                        handler: function (thisObj, checked) {
                            if (checked) {
                            	mkDepthCfgInfobarItem.showHideInfobar = true;
                                // panel.toolbarInfo.show();
                            }
                        }
                    ...
                    new Ext.form.field.Radio({
                        ...
                        handler: function (thisObj, checked) {
                            if (checked) {
                            	mkDepthCfgInfobarItem.showHideInfobar = false;
                                // panel.toolbarInfo.hide();
                            }
                        }
                    })
            ...
            var saveBtn = new Ext.Button({
                ...
                handler: function () {
                    ...
                    panel.showHideInfobar = mkDepthCfgInfobarItem.showHideInfobar;
                    
                    // save column setting
                    colutils.saveColumn('md', col);
                    ...
                    var params = [
                                  ...
                                  jsutil.boolToStr(panel.showHideInfobar, '1', '0')
                                  ];
                    ...
                    Ext.Ajax.request({
                    	...
                    	success: function(response) {
                    		...
                    		if (panel.showSplit !== split || panel.showBcum !== cum) {
                    			...
                    			cookies.eraseCookie(panel.cookieKey);
                    		}
                            ...
                            if (newMarketDepth) {
                                ...
                                newMarketDepth.showHideInfobar = mkDepthCfgInfobarItem.showHideInfobar;
                            ...
                            if (activeWatchlistArr && activeWatchlistArr.length > 0) {
                                for (var i = 0; i < activeWatchlistArr.length; i++) {
                                    var deptPn = activeWatchlistArr[i].wlpanel;
                                    if (deptPn && helper.inView(deptPn) && deptPn.isCardView && deptPn.cardComp) {
                                        deptPn.cardComp.mkDeptShowHideInfobar = mkDepthCfgInfobarItem.showHideInfobar;
                                        deptPn.cardComp.refresh();
                                    }
                                }
                            }
                            ...
                            if (marketDepthMatrixPanel) {
                                if (helper.inView(marketDepthMatrixPanel)) {
                                    marketDepthMatrixPanel.refresh();
                                } else {
                                    marketDepthMatrixPanel.updateGradientStatus(marketDepthGradient);
                                }
                            }

                            if (recentQuotePanel && helper.activeView(recentQuotePanel) && recentQuotePanel.isCardView && recentQuotePanel.cardComp) {
                                recentQuotePanel.cardComp.mkDeptShowHideInfobar = mkDepthCfgInfobarItem.showHideInfobar;
                                recentQuotePanel.cardComp.refresh();
                            }

                            if (quoteScreen && helper.inView(quoteScreen) && quoteScreen.isCardView && quoteScreen.cardComp) {
                                // quoteScreen.cardComp.updateGradientStatus(marketDepthGradient);
                                quoteScreen.cardComp.mkDeptShowHideInfobar = mkDepthCfgInfobarItem.showHideInfobar;
                            ...
                            if (orderPad && orderPad.compRef.marketDepth) {
                                ...
                                md2.showHideInfobar = mkDepthCfgInfobarItem.showHideInfobar;
                                md2.refresh();
                            }
            ...
            var btnCont = [];
            var btnHeader = [saveBtn];
            
            mdSetting = msgutil.popup({
                modal: false,
    ...
    getMktDepthLevel: function () {
        ...
        this.normalLevel = 5; // reset to 5
        var hasLmsMdl = false;
        this.showTotalBidAsk = jsutil.toBoolean(showMktDepthTotal);
                    ...
                    for (var i = 0; i < scpts.length; i++) {
                        ...
                        if (this.exchangecode == scpt.ex) {
                            if (scpt.bts2mktdptlvl != null) {
                                // check overwritten config here
                                var mdConf = overwriteMDManager.getMDInfo(this.exchangecode);
                                
                                if (mdConf) {
                                    if (mdConf.mdType === 'mdl') {
                                        if (mdConf.level < scpt.bts2mktdptlvl) {
                                            this.bts2NewMktDepthLvl = mdConf.level;
                                        }
                                        tempPanelType = 'multi';
                                    } else {
                                        if (mdConf.level < scpt.bts2mktdptlvl) {
                                            this.normalLevel = mdConf.level;
                                        } else {
                                            this.normalLevel = scpt.bts2mktdptlvl;
                                        }
                                    }
                                } else {
        ...
        // checking forcing md config
        if (!hasLmsMdl) {
            var mdConf = forcingMDManager.getMDInfo(this.exchangecode);
            ...
            if (mdConf) {
                if (mdConf.mdType === 'mdl') {
                    ...
                    if (mdConf.level) {
                        this.normalLevel = mdConf.level;
                        tempPanelType = 'normal';
                    }
                }
            }
        }
        
        // no need to show total for 1 level
        if ((tempPanelType == 'multi' && this.bts2NewMktDepthLvl < 2) || (tempPanelType == 'normal' && this.normalLevel < 2)) {
            this.showTotalBidAsk = false;
        }
    ...
    setVisibleColumn: function (id) {
        ...
        var isHidden = true;
        var mdCol = layoutProfileManager.getCol('md');
        ...
        try {
            if (panel.onViewType == 'search') {
                if (record != null) {
                    ...
                    if (N2N_CONFIG.syncSibling) {
                        syncGroupManager.syncAllComps(panel, stkCode, stkName);
                    }
                }
    ...
    setShowFormatQtyLot: function() {
        ...
        if (mdCol == '') {
            ...
            if (mdCol.indexOf('Qty') != -1) {
                var index = mdCol.indexOf('Qty');
                var tempString = mdCol.substring(index);
    ...
    _setCookieId: function () {
        var me = this;
        me.cookieKey = columnWidthSaveManager.getCookieColKey('marketDepth' + me.exchangecode);
    },
