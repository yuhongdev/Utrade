...
//                }
        ...
        panel.tempLabelPanel = new Ext.container.Container({
            ...
            listeners: {
                afterrender: function() {
                    ...
                    panel.tLabel_UpDown = Ext.widget('box', {
            ...
            items: [
                {
                    ...
                    layout: {
                        ...
                        tableAttrs: {
                            ...
                            cellspacing: '2',
                            cls: 'stkinfo_tbl font-style-applied font-size-max-applied'
                        }
                    },
                    items: itemList
                }
    ...
    _procCallChart: function() {
        ...
        if (!isStockChart) {
                ...
                success: function(response) {
                    ...
                    try {
                        ...
                        if (obj.s) {
                            ...
                            panel.tPanel_Chart.update(msgutil.parseMsg(obj.d));
                        }
            ...
            var url = helper.addUrlParams(embeddedStockChartURL, "code=" + stockcode + '&Name=' + encodeURIComponent(stockutil.getStockName(panel.stkname)) + "&exchg=" + exchangecode + "&amount=" + panel.historyDay 
            			+ "&color=" + formatutils.procThemeColor() + '&lang=' + global_Language + '&View=' + (stockutil.isDelayStock(panel.stkcode) ? 'D' : 'R') +
                                '&type=si&newOpen=' + (jsutil.boolToStr(panel.newOpen, '1', '0'))) + '&loginId=' + md5UniqueLoginId;
    ...
    updateFeedRecord: function(dataObj) {
        ...
        if (dataObj[fieldChange] != null) {
            ...
            if (parseFloat(dataObj[fieldChange]) > 0) {
                cssClass = N2NCSS.FontUp;
                arrowCls = 'icon-arrow-up';

            } else if (parseFloat(dataObj[fieldChange]) < 0) {
                cssClass = N2NCSS.FontDown;
                arrowCls = 'icon-arrow-down';
            ...
            panel.tLabel_UpDown.removeCls(['icon-arrow-up', 'icon-arrow-down']);
            panel.tLabel_UpDown.addCls(arrowCls);
        }
