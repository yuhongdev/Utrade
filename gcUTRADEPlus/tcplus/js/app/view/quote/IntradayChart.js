...
//                                    panel.switchButtonStyle();
        ...
        if (N2N_CONFIG.searchAutoComplete) {
            this.tfSearch = Ext.widget('searchautobox', {
                listeners: {
                    select: function(thisCb, records) {
                        if (records.length > 0) {
                            panel.selectSearchResult(null, records[0]);
                        }
                    }
                },
...
//        this.stkname = null;
    ...
    selectSearchResult: function(dtview, record) {
        ...
        if (N2N_CONFIG.syncSibling) {
            syncGroupManager.syncAllComps(this, record.data[fieldStkCode], record.data[fieldStkName]);
        }        
    },
    loadChart: function() {
        var panelid = this.id;
        ...
        if (!isStockChart) {
                ...
                success: function(response) {
                    ...
                    if (obj != null) {
                        if (obj.s == true) {
                            ...
                            panel.setErrMsg(msgutil.parseMsg(obj.d));
                        }
                    } else {
                        debug(response);
            ...
            var charturl = helper.addUrlParams(embeddedStockChartURL, "code=" + stkParts.code + '&Name=' + encodeURIComponent(stockutil.getStockName(this.stkname)) + "&exchg=" + stkParts.exch + "&mode=d&color=" + formatutils.procThemeColor() 
            				+ '&lang=' + global_Language + '&View=' + (stockutil.isDelayStock(panel.stkcode) ? 'D' : 'R') + '&type=ic&newOpen=' + (jsutil.boolToStr(panel.newOpen, '1', '0'))) + '&loginId=' + md5UniqueLoginId;
