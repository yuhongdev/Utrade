...
Ext.define('TCPlus.view.quote.Indices', {
    ...
    _procCallChart: function() {
        ...
        if (!isStockChart) {
            ...
            var charturl = helper.addUrlParams(embeddedStockChartURL, "code=" + stockcode + '&Name=' + encodeURIComponent(stockutil.getStockName(panel.chartStockName)) + "&exchg=" + exchg + "&isstock=N&mode=d&color=" 
            				+ formatutils.procThemeColor() + '&lang=' + global_Language + '&View=' + (stockutil.isDelayStock(panel.chartStockCode) ? 'D' : 'R')
                                        + '&type=in&newOpen=' + (jsutil.boolToStr(panel.newOpen, '1', '0'))) + '&loginId=' + md5UniqueLoginId;
    ...
    refresh: function() {
        ...
        panel.tGrid_Indices.setTitle(panel._getIndexTitle());

        panel._procCallIndices();
    },
    switchRefresh: function(silent) {
        // fixed issue which indices tab activated and use default chart setting
        this.newOpen = false;
...
//        console.log(dataObj);
    ...
    _getIndexTitle: function() {
        var title = languageFormat.getLanguage(20029, 'INDICES').toUpperCase();

        if (isDelayedIndex(exchangecode)) {
            title += ' [Delayed]';
        }
