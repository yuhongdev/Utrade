...
//	tImageFlag				: null, //v1.3.33.13
    ...
    tLabelUnchange: null,
    tLabelUntrade: null,
    //new label for trading phase
    tLabelTPValue: null,
    //Market foreign value
    tLabelMktFrVal: null,
    tLabelDateTime: null,
    tLabelIndexDelay: null,
    tLabelDisclaimer: null,
    _roundValue: false,
    /**
     * Description <br />
    ...
    initComponent: function() {
        ...
        */
        ...
        panel.tImageUnchange = Ext.create('widget.box', {
            autoEl: {
                tag: 'div',
                cls: 'icon icon-unchange'
            }
        }),
        panel.tImageUntrade = Ext.create('widget.box', {
            autoEl: {
                tag: 'div',
                cls: 'icon icon-untrade'
            }
        });
        ...
        panel.tImageSignal = Ext.create('widget.box', {
            autoEl: {
                tag: 'div',
                cls: 'icon icon-connecting',
        ...
        if (N2N_CONFIG.displayMktFrVal) {
            panel.tImageMktFrVal = Ext.create('widget.box', {
                autoEl: {
                    tag: 'div',
                    cls: 'icon icon-mktfrval'
                }
            });
        }
        // layouts
        ...
        if (N2N_CONFIG.enableLayoutProfile) {
            for (var l in layoutProfiles) {
                panel['lp' + l] = Ext.create('Ext.button.Button', {
                    ...
                    iconCls: 'icon-layout-btn-' + l,
                    tooltip: languageFormat.getLanguage(20605, 'Layout') + ' ' + l,
                    lp: l,
            ...
            // fix sorting issue on Safari
            layoutItems.sort(jsutil.getArraySorter('lp'));

            panel.saveCurrentLayoutBtn = Ext.create('Ext.button.Button', {
                cls: 'plain-btn',
                iconCls: 'icon-save-layout',
        ...
        panel.autoFitBt = createAutoFitButton(panel);
        ...
        panel.exchNameBt = Ext.create('Ext.button.Button', {
            baseCls: 'ex-box',
            hidden: true,
            handler: function() {
                menuHandler.marketSummary();
            }
        });
        ...
        panel.tLabelSummaryUpDown = Ext.create('widget.box', {
            autoEl: {
                tag: 'div',
                cls: 'icon',
        ...
        //Value for display trading phase
        panel.tLabelTPValue = new Ext.form.Label({
            style: 'margin-left: 15px; font-weight: bold;',
            width: 160,
            height: lblHeight
        });        
        
        //Market Foreign Value label
        if (N2N_CONFIG.displayMktFrVal) {
            panel.tLabelMktFrVal = new Ext.form.Label({
                style: adjustStyle,
                width: 70,
                height: lblHeight
            });
        ...
        //Disclaimer
        if (N2N_CONFIG.exchDisclaimerList.length > 0) {

            panel.tLabelDisclaimer = new Ext.form.Label({
                itemID: 'ExchgDisclaimer',
                text: 'Disclaimer', //Will be change later
                width: 65,
                height: lblHeight,
                style: 'text-align: right;color: blue;font-weight: bold;text-decoration: underline;cursor: pointer;',
                hidden: N2N_CONFIG.exchDisclaimerList.indexOf(exchangecode) == -1,
                listeners: {
                    render: function () {
                        panel.tLabelDisclaimer.getEl().on('click', function () {
                            //Some message box here, get the message box from cimb
                            N2NDisclaimer.exDisclaimerValidation();
                        }
                        );
                    }
                }
        ...
        if (panel.tLabelDisclaimer)
            sumItems.push(panel.tLabelDisclaimer);

        if (N2N_CONFIG.displayMktFrVal) {
            sumItems.push(panel.tImageMktFrVal);
            sumItems.push(panel.tLabelMktFrVal);
        }

        if (N2N_CONFIG.mktSumDisplayTPStatus) {
            sumItems.push(panel.tLabelTPValue);
        }
        ...
        panel.summaryPanel = {
            ...
            listeners: {
                ...
                afterrender: function(thisComp) {
                    ...
                    Ext.create('Ext.tip.ToolTip', {
                        target: panel.tLabelTPValue.getId(),
                        html: languageFormat.getLanguage(21033, 'Trading phase')
                    });
                    //Foreign flow
                    if (N2N_CONFIG.displayMktFrVal) {
                        Ext.create('Ext.tip.ToolTip', {
                            target: panel.tImageMktFrVal.getId(),
                            html: languageFormat.getLanguage(21036, 'Market Foreign Value')
                        });
                        Ext.create('Ext.tip.ToolTip', {
                            target: panel.tLabelMktFrVal.getId(),
                            html: languageFormat.getLanguage(21036, 'Market Foreign Value')
                        });
                    }
                }
    ...
    setSignal: function(stage) {
        ...
        if (signalLabel) {
            signalLabel.removeCls(['icon-online', 'icon-offline', 'connecting']);
            signalLabel.addCls(signal);
        }
    },
    /**
     * Description <br/>
...
//				console.log("dataobj[36]:"+dataobj[fieldSectorCode]+", dataobj[33]:"+dataobj[fieldStkCode]);
                if (dataobj[fieldSectorCode] == '10000' || (dataobj[fieldSectorCode] == '2000'))
                {
                    if (dataobj[fieldSectorCode] == '10000')
                    {
                        ...
                        //Market foreign value
                        if (N2N_CONFIG.displayMktFrVal) {
                            var frbuy = dataobj[fieldSbFrBuy];
                            var frsell = dataobj[fieldSbFrSell];
                        }
                        ...
                        if (!Ext.isEmpty(up)) {
                            ...
                            if (upEl != null) {
                                ...
                                upEl.update(upStr);
                                upEl.setStyle('color', sumFontColorReader.get('up', cFUp));
                            }
                        }
                        if (!Ext.isEmpty(down)) {
                            ...
                            if (downEl != null) {
                                ...
                                downEl.setStyle('color', sumFontColorReader.get('down', cFDown));
                            }
                        }
                        ...
                        if (N2N_CONFIG.displayMktFrVal) {
                            ...
                            if ((frbuy || frsell) && N2N_CONFIG.displayMktFrVal) {
                                var mktfrvalEl = panel.tLabelMktFrVal.getEl();
                                if (mktfrvalEl != null) {
                                    var netVal = panel._frbuy - panel._frsell;

//                            netVal = formatutils.formatNumber(netVal, null, null, 0);
                                    netVal = panel.formatNumber(netVal);
                                    panel._autoWidth(panel.tLabelMktFrVal, netVal);
                                    mktfrvalEl.update(netVal);
                                }
                            }
                        }
...
//                     console.log("fieldSectorCode: "+dataobj[fieldSectorCode]);
    ...
    setLast: function(last) {
        ...
        try {
            ...
            if (label != null) {
                ...
                label.removeCls(allColorCls);
                img.removeCls(['icon-up', 'icon-down']);
                
                if (change > 0) {
                    label.addCls(colorUpCls);
                    img.addCls('icon-up');
                } else if (change < 0) {
                    label.addCls(colorDownCls);
                    img.addCls('icon-down');
                } else {
                    label.addCls(colorUnChgCls);
                    img.update('');
                }
            }
    ...
    setChange: function(change, chgpc) {
        ...
        if (label != null) {
            ...
            if (change > 0) {
                label.setStyle('color', sumFontColorReader.get('up', cFUp));
            } else if (change < 0) {
                label.setStyle('color', sumFontColorReader.get('down', cFDown));
            } else {
                label.setStyle('color', sumFontColorReader.get('unchg', FUnchanged));
            }
            panel.change = change;
        }
    },
    ...
    clearTextField: function() {
        ...
        panel.tLabelSummaryUpDown.el.removeCls(['icon-up', 'icon-down']);
    ...
    clockStart: function() {
        ...
        panel.loopTime = setInterval(function() {
            ...
            if (panel.tLabelDateTime != null) {
                helper.setHtml(panel.tLabelDateTime, stringTime);
            }
    ...
    setExchangeFlag: function() {
        ...
        if (panel.exchangeCode) {
            // panel.tButtonFlagSummaryGraph.setIconCls('icon-exch-' + panel.exchangeCode);

            helper.show(panel.exchNameBt);
            panel.exchNameBt.setText(getExchangeShortName(panel.exchangeCode));
            panel.exchNameBt.el.set({
                'title': getExchangeFullName(panel.exchangeCode)
            });
        } else {
            helper.hide(panel.exchNameBt);
        }
    },
    /**
     * Description <br/>
...
//		}
                ...
                success: function(response) {
                    ...
                    if (imageUrl != null && imageUrl.length > 0) {
                        ...
                        object = new Ext.form.Label({
                            text: languageFormat.getLanguage(21034, 'Chart Link not available')
                        });
                    }
            	...
            	success: function(obj2) {
            		if (obj2 != null) {
            			if(obj2.s && obj2.d.length > 0){
            				...
            				var url = helper.addUrlParams(embeddedStockChartURL, "code=" + stkcode + '&Name=' + encodeURIComponent(stockutil.getStockName(name)) + "&exchg=" + excode + "&isstock=N&mode=d&color=" + formatutils.procThemeColor() + '&lang=' + global_Language + '&View=' + (stockutil.isDelayStock(stkcode) ? 'D' : 'R'));
    ...
    _disableAllLabel: function(visible) {
        ...
        if (N2N_CONFIG.displayMktFrVal) {
            panel.tImageMktFrVal.setVisible(visible);
            panel.tLabelMktFrVal.setVisible(visible);
        }        
        if (N2N_CONFIG.mktSumDisplayTPStatus) {
            panel.tLabelTPValue.setVisible(visible);
        }        
    ...
    setDelayIndexLabel: function() {
        ...
        if (isDelayedIndex(panel.exchangeCode)) {
            panel.tLabelIndexDelay.setText(languageFormat.getLanguage(21035, '[Delayed]'));
            panel.tLabelIndexDelay.setVisible(true);
            helper.show(panel.tLabelIndexDelay);
        } else {
            panel.tLabelIndexDelay.setText('');
            panel.tLabelIndexDelay.setVisible(false);
            //Hide label if not delay index
            helper.hide(panel.tLabelIndexDelay);
        }
    }
});
