...
//					console.log("stkcode 06:"+stkcode);
					...
					if(arrTemp[fieldCFDMapValue_06] != null){
						var cfdValue = formatutils.getCFDValues(arrTemp[fieldCFDMapValue_06]);
    					
    					arrTemp[fieldCFDTradable_06] = cfdValue.tradable;
    					arrTemp[fieldCFDShortSell_06] = cfdValue.shortsell;
					}
					
					if(!arrTemp['fromCFDFile']){
							...
							if (global_cfddelta.len != null && global_cfddelta.scpt != null) {
								...
								for (var i = 0; i < global_cfddelta.len; i++) { // verify from main.jsp
									...
									if (V.key == stkcode) {
										...
										arrTemp[fieldCFDTradable_06] = cfdV[4];
										arrTemp[fieldCFDShortSell_06] = cfdV[2];
										arrTemp[fieldCFDMarginLS_06] = cfdV[1];
										arrTemp[fieldCFDMarginPerc_06] = cfdV[0];
										arrTemp[fieldCFDLastUpdTime_06] = cfdV[5];
										arrTemp[fieldCFDMapValue_06] = null;
										break;
									}
								}
							}
					...
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldBqty6] != null) {
						var bqty6 = formatutils.formatLot( arrTemp[fieldBqty6], htsub.items );
						htsub.put(fieldBqty6, bqty6);
						arrTemp[fieldBqty6] = bqty6;
					}
                                        
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldBqty7] != null) {
						var bqty7 = formatutils.formatLot( arrTemp[fieldBqty7], htsub.items );
						htsub.put(fieldBqty7, bqty7);
						arrTemp[fieldBqty7] = bqty7;
					}
                                        
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldBqty8] != null) {
						var bqty8 = formatutils.formatLot( arrTemp[fieldBqty8], htsub.items );
						htsub.put(fieldBqty8, bqty8);
						arrTemp[fieldBqty8] = bqty8;
					}
                                        
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldBqty9] != null) {
						var bqty9 = formatutils.formatLot( arrTemp[fieldBqty9], htsub.items );
						htsub.put(fieldBqty9, bqty9);
						arrTemp[fieldBqty9] = bqty9;
					}
                                        
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldBqty10] != null) {
						var bqty10 = formatutils.formatLot( arrTemp[fieldBqty10], htsub.items );
						htsub.put(fieldBqty10, bqty10);
						arrTemp[fieldBqty10] = bqty10;
					}
					...
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldSqty6] != null) {
						var sqty6 = formatutils.formatLot( arrTemp[fieldSqty6], htsub.items );
						htsub.put(fieldSqty6, sqty6);
						arrTemp[fieldSqty6] = sqty6;
					}
                                        
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldSqty7] != null) {
						var sqty7 = formatutils.formatLot( arrTemp[fieldSqty7], htsub.items );
						htsub.put(fieldSqty7, sqty7);
						arrTemp[fieldSqty7] = sqty7;
					}
                                        
                                        if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldSqty8] != null) {
						var sqty8 = formatutils.formatLot( arrTemp[fieldSqty8], htsub.items );
						htsub.put(fieldSqty8, sqty8);
						arrTemp[fieldSqty8] = sqty8;
					}
                                        
                                        if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldSqty9] != null) {
						var sqty9 = formatutils.formatLot( arrTemp[fieldSqty9], htsub.items );
						htsub.put(fieldSqty9, sqty9);
						arrTemp[fieldSqty9] = sqty9;
					}
                                        
                                        if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldSqty10] != null) {
						var sqty10 = formatutils.formatLot( arrTemp[fieldSqty10], htsub.items );
						htsub.put(fieldSqty10, sqty10);
						arrTemp[fieldSqty10] = sqty10;
					}
					...
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldVol] != null) {
						...
						// v1.3.34.68
                                        ...
                                        if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldIDSSTolVol_06]) {
                                            var idsstolvol = formatutils.formatLot(arrTemp[fieldIDSSTolVol_06], htsub.items);
                                            htsub.put(fieldIDSSTolVol_06, idsstolvol);
                                            arrTemp[fieldIDSSTolVol_06] = idsstolvol;
                                        }
		...
		try {
			...
			if ( dataObj.d != null ) {
				...
				for ( var i = 0; i < dataObj.d.length; i ++ ) {
					...
					if(arrTemp[fieldStatus] != null){
						var statusValue = arrTemp[fieldStatus].charAt(1);
						var newsValue = arrTemp[fieldStatus].charAt(5) + arrTemp[fieldStatus].charAt(4);
						var oriRSSIValue = arrTemp[fieldStatus].charAt(0);
						...
						arrTemp[fieldStkStatus] = statusValue;
						arrTemp[fieldNews] = newsValue;
						arrTemp[fieldRSSIndicator] = oriRSSIValue;
					}
					...
					if (global_displayUnit.toLowerCase() == "lot" && arrTemp[fieldVol] != null) {
						...
						// v1.3.34.68
                                        ...
                                        if (htsub.items && !arrTemp[fieldEPS]) { // get eps from storage
                                            arrTemp[fieldEPS] = htsub.items[fieldEPS];
                                        }

                                        // calculate PE ratio
                                        if (N2N_CONFIG.calculatePERatio) {  
                                            
                                            // ignore PE Ratio from feed
                                            if (arrTemp[fieldPERatio]) {
                                                delete arrTemp[fieldPERatio];
                                            }
                                            
                                            var peRatio = stockutil.getPERatio(arrTemp[fieldEPS], arrTemp[fieldLast], arrTemp[fieldLacp]);
                                            
                                            if (peRatio) {
                                                htsub.put(fieldPERatio, peRatio);
                                                arrTemp[fieldPERatio] = peRatio;
                                            }
                                        }
                                        
                                        if (htsub.items && !arrTemp[fieldLacp]) { // get LAPC from storage
                                            arrTemp[fieldLacp] = htsub.items[fieldLacp];
                                        }
                                        
                                        var ytdChgPer = stockutil.getYtd(arrTemp[fieldLacp], arrTemp[fieldYTD]);
                                        if (ytdChgPer) {
                                            htsub.put(fieldYTDChgPer, ytdChgPer);
                                            arrTemp[fieldYTDChgPer] = ytdChgPer;
                                        }
                                        var ytd1ChgPer = stockutil.getYtd(arrTemp[fieldLacp], arrTemp[field1YTD]);
                                        if (ytd1ChgPer) {
                                            htsub.put(fieldYTD1ChgPer, ytd1ChgPer);
                                            arrTemp[fieldYTD1ChgPer] = ytd1ChgPer;
                                        }
                                        var ytd3ChgPer = stockutil.getYtd(arrTemp[fieldLacp], arrTemp[field3YTD]);
                                        if (ytd3ChgPer) {
                                            htsub.put(fieldYTD3ChgPer, ytd3ChgPer);
                                            arrTemp[fieldYTD3ChgPer] = ytd3ChgPer;
                                        }
                                        var ytd5ChgPer = stockutil.getYtd(arrTemp[fieldLacp], arrTemp[field5YTD]);
                                        if (ytd5ChgPer) {
                                            htsub.put(fieldYTD5ChgPer, ytd5ChgPer);
                                            arrTemp[fieldYTD5ChgPer] = ytd5ChgPer;
                                        }
                                        
                                        var mktAvgPrice = stockutil.getMktAvgPrice(htsub.get(fieldVol), htsub.get(fieldValue), htsub.get(fieldLotSize));
                                        if (mktAvgPrice) {
                                            htsub.put(fieldMktAvgPrice, mktAvgPrice);
                                            arrTemp[fieldMktAvgPrice] = mktAvgPrice;
                                        }
                                                                                
                                        // ignore existing VWAP
                                        if (arrTemp[fieldVWAP]) {
                                            delete arrTemp[fieldVWAP];
                                        }

                                        // calculate VWAP
                                        var VWAP = stockutil.getVWAP(arrTemp[fieldValue], arrTemp[fieldVol]);

                                        if (VWAP) {
                                            htsub.put(fieldVWAP, VWAP);
                                            arrTemp[fieldVWAP] = VWAP;
                                        }
