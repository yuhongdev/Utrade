...
var timer = (function() {
	...
	var cfdFileTimer = null;
        
	if ( global_showCountDownTimer.toLowerCase() == "true" ){ //from main.jsp
		
		/* Disable listening to mousemove event. Only reset timer upon mouse click and key press events.
		...
		*/
		...
		function showAlert(){
			...
			var tempAlert = Ext.Msg.show( { 
				...
				msg 	: languageFormat.getLanguage(33400, 'Your session will be expired on [PARAM0] seconds.' ,global_countdownTime), 
			...
			timeClockExit = setInterval( function() {
				...
				if ( exitCounting >= 0 ) {
                    tempAlert.updateText(languageFormat.getLanguage(33401, " Your session will be expired in  [PARAM0] seconds. <br/> Press OK to continue, or Cancel to logout.", exitCounting));
...
//		};
	...
	function callCFDDeltaFile(){
		if(N2N_CONFIG.showCFDDeltaFile){
			var url = addPath + 'tcplus/CFDDelta';
			Ext.Ajax.request({
				url: url,
				method: 'POST',
				timeout: 5000,
				success: function (response) {
					try {
						global_cfddelta = Ext.decode(response.responseText);
						...
						if(global_cfddelta != null){
							if (global_cfddelta.len != null && global_cfddelta.scpt != null) {
								...
								for (var i = 0; i < global_cfddelta.len; i++) { // verify from main.jsp
									...
									arrTemp['fromCFDFile'] = true;

									arrTempList.push(arrTemp);
								}

								dataObj.d = arrTempList;
								dataObj.s = true;
								dataObj.c = arrTempList.length;

								Storage.procJson06(dataObj);
							}
						}

						console.log('[timer][cfddelta] result : success *** ');
					} catch (e) {
						console.log('[timer][cfddelta] Exception ---> ' + e);
						setRefreshTask();
					}
				},
				failure: function (response) {
					console.log('[timer][cfddelta] failure ---> ' + response.responseText);
					setRefreshTask();
				},
				callback: function(){
					setRefreshTask();
	...
	function setRefreshTask() {
        // clears previous task if any
        stopRefreshTask();
        cfdFileTimer = setTimeout(function () {
        	callCFDDeltaFile();
        }, 300000);
    }
	...
	return{
		callCFDDeltaFile : callCFDDeltaFile,
                stopIdleTimer: _stopIdleTimer
	};
}());