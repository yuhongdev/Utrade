var N2NDisclaimer = (function() {
    ...
    function exDisclaimerValidation() {
        var returnValue = false;

        if (N2N_CONFIG.exchDisclaimerList.indexOf(exchangecode) > -1) {
            // avoid multi disclaimers
            if (!tExWindow_Window) {
                //create new function and var for button type
                _procButtonTypeEx();
                //open disclaimer window
                _openExDisclaimerWindow();
            }
            returnValue = true;
        }
        return returnValue;
    ...
    function _procButtonTypeEx() {
        var btnType = N2N_CONFIG.exchDisclaimerFile[1];
        exButtonList = [];
        ...
        } else if (btnType == "ACCEPT/DECLINE") {
            exButtonList.push({
                text: 'ACCEPT',
                handler: function() {
                    _procConfirmationEx();
    ...
    function _openExDisclaimerWindow() {
        var htmlDesign = '<iframe width="100%" height="100%" frameborder="0" scrolling="auto" src="ex_disclaimer.jsp?' + new Date().getTime() + '"> </iframe> ';

        tExWindow_Window = msgutil.popup({
            title: "Disclaimer",
            width: parseInt(N2N_CONFIG.exchDisclaimerFile[2] || 150),
            height: parseInt(N2N_CONFIG.exchDisclaimerFile[3] || 70),
            plain: true,
            closable: true,
            resizable: false,
            dislayAtClick: false,
            ...
            buttons: exButtonList,
            listeners: {
                destroy: function() {
                    tExWindow_Window = null;
                }
            }
        });
    }
    ...
    function _procConfirmationEx() {
            tExWindow_Window.destroy();
    }
    ...
    return {
        ...
        exDisclaimerValidation: exDisclaimerValidation
    };
