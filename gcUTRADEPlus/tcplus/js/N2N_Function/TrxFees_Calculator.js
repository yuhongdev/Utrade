...
//ABACUS
//[TrxFeeFormula]=PH,Amount,<OrdQty>*<OrdPrice>
//[TrxFeeFormula]=PH,CommRate,<=,9000,-,-,0.015
//[TrxFeeFormula]=PH,CommRate,-,-,>,9000,0.0025
//[TrxFeeFormula]=PH,Commission,{Amount}*{CommRate},Min=20~135
//[TrxFeeFormula]=PH,VAT,{Commission}*0.12
//[TrxFeeFormula]=PH,SCCPFee,{Amount}*0.0001
//[TrxFeeFormula]=PH,BuyTrxFee,{Commission}+{SCCPFee}+{VAT}
//[TrxFeeFormula]=PH,SalesTax,{Amount}*0.005
//[TrxFeeFormula]=PH,SellTrxFee,{BuyTrxFee}+{SalesTax}
// =====Formula======
...
var trxFeesCalculator = (function() {
    ...
    function calcTrxFee(arTrxFeeFormulaList, act, amount) {
        var trxFee = 0;
        var crType = 0;
        var obj = {};
            ...
            if (arTrxFeeFormulaList[i].formula.indexOf('{') != -1) {
                ...
                if (formulaVariables != null) {
                    for (var j = 0; j < formulaVariables.length; j++) {
                        for (var k = 0; k < arTrxFeeFormulaList.length; k++) {
                            if (formulaVariables[j] == arTrxFeeFormulaList[k].formulaName) {
                            	if(arTrxFeeFormulaList[k].formulaName == 'CommRate'){
                            		if(arTrxFeeFormulaList[k].thisComm){
                            			crType = arTrxFeeFormulaList[k].crRate;
                    ...
                    if (arTrxFeeFormulaList[i].min != null) {
                    	if(arTrxFeeFormulaList[i].min.indexOf('~') != -1){
                    		var min = arTrxFeeFormulaList[i].min.split('~');
                    		if(arTrxFeeFormulaList[i].formulaName == 'Commission'){
                    			if(crType == '1'){
                    				if(value < parseFloat(min[0])){
                    					value = parseFloat(min[0]);
                    				}
                    			}else{
                    				if(value < parseFloat(min[1])){
                    					value = parseFloat(min[1]);
                    				}
                    			}
                    		}
                    	}else{
                    		if (value < parseFloat(arTrxFeeFormulaList[i].min)) {
                    			value = parseFloat(arTrxFeeFormulaList[i].min);
                    		}
                    	}
                    }

                    if (arTrxFeeFormulaList[i].max != null) {
                        if (value > parseFloat(arTrxFeeFormulaList[i].max)) {
                            value = parseFloat(arTrxFeeFormulaList[i].max);
                        }
                    }

                    arTrxFeeFormulaList[i].value = value;
                    obj[arTrxFeeFormulaList[i].formulaName] = arTrxFeeFormulaList[i].value;
