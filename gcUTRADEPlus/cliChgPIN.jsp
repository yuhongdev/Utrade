...
try {
	...
	if (!validSession) {
		...
		if (sSecure.compareToIgnoreCase("L") == 0) {
			...
			out.println("	this.location.href='"+oN2NSession.getSetting("HTMLRoot")+oN2NSession.getSetting("projectFolder")+"/cliChgPIN.jsp'");		
		} else {
		...
		if ( (!sOldPIN.equals("")) && (!sNewPIN.equals("")) ) {
			...
			errorMessage = atp.mfChangeExistPIN(g_sCliCode, sLoginID, sOldPIN, sNewPIN, session, oN2NSession.getSetting("BHCode"));
	...
	<tr><td>
		<table border=1 cellpadding=0 cellspacing=0 width='100%' height=100%>
			<tr><td>
				<table width='100%'>
					<tr><td width='10%'>&nbsp;</td>
						<td width='80%' class=fntInfoMsg colspan=2>Please enter your old and new PIN into the system.<br/><br/></td>
						<td width='10%'>&nbsp;</td></tr>
					<tr><td width='10%'>&nbsp;</td>
						<td width='45%' class=fntInputMsg>Old PIN</td>
						<td width='35%'><form id=frmChgPIN name=frmChgPIN action="cliChgPIN.jsp" method=post onsubmit="return entryVerify(this);">
							<input type=password id=txtOldPIN name=txtOldPIN class=inputField maxlength=6 size=15 value="" /></td>
						<td width='10%'>&nbsp;</td></tr>
					<tr><td>&nbsp;</td>
						<td class=fntInputMsg>New PIN</td>
						<td colspan=2><input type=password id=txtNewPIN name=txtNewPIN class=inputField maxlength=6 size=15' value="" /></td>
						<td>&nbsp;</td></tr>
					<tr height=30><td>&nbsp;</td>
						<td class=fntInputMsg>Confirm New PIN</td>
						<td colspan=2><input type=password id=txtNewCfmPIN name=txtNewCfmPIN class=inputField maxlength=6 size=15' value="" /></td>
						</tr>
					<tr><td>&nbsp;</td>
						<td colspan=2><hr width='100%'></td>
						<td>&nbsp;</td></tr>
					<tr height=25><td>&nbsp;</td>
						<td colspan="2"><input type=submit value='Submit'>&nbsp;&nbsp;<input type="button" name="Cancel" value="Cancel" onclick="javascript:btnCancel_onClick();"></td>
						<td>&nbsp;</td></tr></form>
					...
					<tr><td>&nbsp;</td>
						...
						<td>&nbsp;</td></tr>
					<tr><td>&nbsp;</td>
						<td colspan=2><span style='color:red'>click CANCEL button to continue.</span></td>
						<td>&nbsp;</td></tr>
					<%}%>							
				</table>
				</td></tr>
...
{
	//window.resizeTo(305,430);
<% 
/**
	  	switch(nRetCode) {
	     	case 0:
	     		out.print("alert('Your PIN have been changed successfully.');");
	     		out.println("	this.location.href='"+oN2NSession.getSetting("HTMLRoot")+"/n2n_gc/cliChgPIN.jsp?secure=C'");
	     		break;
	     	case 2:
...
<% 
		...
		//out.println("if((parent.location.href).indexOf('cliChgPIN.jsp') != -1) {");
		out.println("if((this.location.href).indexOf('cliChgPIN.jsp') != -1) {");
...
//	this.location.href = '< %=oN2NSession.getSetting("HTMLRoot") + g_sPath% >/cliChgPIN.jsp?secure=C';
	parent.close();
}
</script>
<%
