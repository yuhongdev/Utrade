...
<tr id=tblDOB height=20>
	...
	<td class=clsCliDtlInput colspan=3>
		...
		<%
		String [] sdtTemps = null;
		String dt_years = "";
		
		String dt_wordss = clibean.getDob();
		
		if ((dt_wordss!=null)&&(dt_wordss.length()==10)) {
				sdtTemps = dt_wordss.split("-");
				dt_years = sdtTemps[0];
				if (dt_years.length()==4)
						dt_wordss = sdtTemps[2] + "-" + sdtTemps[1] + "-" + sdtTemps[0];
				else
						dt_wordss = clibean.getDob();
		} else {
				dt_wordss = clibean.getDob();
		}
		%>
		<html:N2N_HtmlObject type="dobCalendar" objectName="frmUserReg.dob" defaultValue="<%=dt_wordss%>"/>		
