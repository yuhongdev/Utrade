...
function JSformatNumber_Old(p_Number, nDecimal)
{
	var sNumber;
	var nDecPower;
	var pos;
...
function JSformatNumber(num, decimalNum)
{
	JSformatNumber(num, decimalNum, true, false, true)
}
...
function JSsetInnerText(oData, vsValue) {
	if (oData == null || vsValue == null) {
		return "";
	}

	if (document.all) {
		oData.innerText = vsValue;
	} else {
		oData.textContent = vsValue;
	}
