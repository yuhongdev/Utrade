...
var stockutil = {
    ...
    filterCols: function(colList, skipMDColCheck) {
        var col05 = new Array();
        var col06 = new Array();
        ...
        for (var i = 0; i < colList.length; i++) {
            ...
            } else if (skipMDColCheck || (MD_COLS.indexOf(col) === -1 && (!isNaN(col) || col === fieldBuyRate))) {
                col05.push(col);
            }
        }
    ...
    getPERatio: function(last, EPS) {
        var lastValue = parseFloat(last);
        var EPSValue = parseFloat(EPS);

        if (lastValue != NaN && EPSValue != NaN && EPSValue != 0) {
            var PERatio = lastValue / EPSValue;

            return Ext.util.Format.number(PERatio, '0.0000');
        }
    ...
    getStockNameFromList: function(stkCode, exch) {
        if (fullList[exch]) {
            return fullList[exch][stkCode] || stkCode;
        } else {
            return stkCode;
        }
