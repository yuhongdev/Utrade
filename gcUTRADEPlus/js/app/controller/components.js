                        ...
                        mdToolBtn,
                        {
                            ...
                            handler: function (thisBtn, a, b) {
                                ...
                                if (pcomp.getCollapsed()) {
                                    thisBtn.setIcon(ICON.EXPAND);
                                        mdToolBtn.hide();
                                } else {
                                    thisBtn.setIcon(ICON.COLLAPSE);
                                        mdToolBtn.show();
                                    }
                                }
                        },
    ...
    if (stkcode && stkcode.length > 0) {
        ...
        try {
            if (orderPad.allowUpdate === true) {
                ...
                orderPad.updateTitle(stkcode, stockutil.getStockPart(stkname, ''));
                                    ...
                                    if (obj.s == true) {
                                        ...
                                        if (obj.m) {
                                            ...
                                            if (obj.e == -2002) {		// if its error code is -2002
                                            	...
                                            	msgutil.alert(languageFormat.getLanguage(30133, 'Maximum \'[PARAM0]\' stocks per watchlist has been reach.', 
    ...
    ordWinTools.push({
        ...
        listeners: {
            afterrender: function(thisBtn) {
                thisBtn.toggle(!getMDCollapsed(), true);
            }
        }
    });
...
function toggleMarketDepth() {
    //orderPad.compRef.marketDepth.toggleCollapse();
	var collapsedMD = orderPad.compRef.marketDepth.getCollapsed() ? true : false;
...
function getMDCollapsed() {
    return jsutil.toBoolean(cookies.readCookie('_OrdPad_Collapsed_MD_'));
}
...
function viewWatchlist(wlName, portlet_col, index1, wlConf, ctTab) {
    ...
    // checks maximum watchlists open
    if (N2N_CONFIG.maxOpenWatchlist !== -1 && (activeWatchlistArr.length + 1 > N2N_CONFIG.maxOpenWatchlist)) {
        msgutil.alert(languageFormat.getLanguage(30130, 'Maximum of [PARAM0] watchlists are allowed to open. Kindly close some watchlists and try again.', N2N_CONFIG.maxOpenWatchlist.toString()));
        return;
    }
    ...
    if (idx != -1) {
        ...
        for (var i = 0; i < totWL; i++) {
            ...
            var viewWl = {
                ...
                wlName: list[i],
                icon: N2N_CONFIG.menuType == MENU_TYPE.TOOL ? '' : icoBtnViewWlist24,
        ...
        if (showWatchListHeader == "TRUE") {
            ...
            if (N2N_CONFIG.recentQuote) {
                tbWacthListItem.push({
                    ...
                    icon: N2N_CONFIG.menuType == MENU_TYPE.TOOL ? '' : icoBtnAddWlist24,
                    cls: N2N_CONFIG.menuType == MENU_TYPE.TOOL ? 'toolbar-menu-item-medium' : 'x-menu-item-medium',
            ...
            if (showWatchListRename == "TRUE") {
                tbWacthListItem.push({
                    ...
                    icon: N2N_CONFIG.menuType == MENU_TYPE.TOOL ? '' : iconBtnRenameWlist24,
            ...
            if (showWatchListDelete == "TRUE") {
                tbWacthListItem.push({
                    ...
                    icon: N2N_CONFIG.menuType == MENU_TYPE.TOOL ? '' : icoBtnRmvWlist24,
            ...
            if (global_personalizationTheme.indexOf("wh") == -1) {
                iconWL = icoBtnWlist24;
            } else if (N2N_CONFIG.menuType != MENU_TYPE.TOOL) {
                iconWL = icoWBBtnWlist;
            }
...
function createEquityPortfolioPanel(accNo, portlet_col, index, epConf, tabCt) {
    ...
    if (equityPrtfPanel == null) {
        ...
        funcObj.func = function () {
            ...
            activateBuySellMenu(modeOrdBuy, objRec);
        };
        funcs.push(funcObj);
        ...
        funcObj.func = function () {
            ...
            equityPrtfPanel.accbranchNo = accbranch;
            objRec.accbranchNo = accbranch;
            activateBuySellMenu(modeOrdSell, objRec);
        };
        funcs.push(funcObj);
...
function createEquityManualPortFolioPanel(accNo, portlet_col, index1) {
    ...
    if (equityManualPrtfPanel == null) {
        ...
        if (!N2N2FA.return2FAValidate('manualPrtf', arguments)) {
            return;
        }

        equityManualPrtfPanel = Ext.widget('equityprtf', {
            exchangecode: ex,
            slcomp: 'emp',
            portalType: 'large',
        ...
        equityManualPrtfPanel.on('itemdblclick', function (thisView, record, item, index, evt) {
            if (!touchMode && singleClickMode == "FALSE") {
                // updateActivePanel(grid, index);
                // createCorporateNewsAction(grid, index);
            }
        });

        equityManualPrtfPanel.on('cellclick', function (thisView, td, cidx, record, tr, ridx, e) {
            if (touchMode || singleClickMode == "TRUE") {
                // updateActivePanel(thisView, ridx, cidx);
                // createCorporateNewsAction(thisView, ridx);
            }
        });
        ...
        n2nLayoutManager.addItem(equityManualPrtfPanel, portlet_col, index1);
...
//            var tempPanel = tabPanel1.getComponent(i).getComponent(0);
//
//            if (tempPanel.id == equityManualPrtfPanel.id) {
//                tabPanel1.getComponent(i).on({
//                    "activate": {
//                        fn: function(thisPanel) {
//                            thisPanel.getComponent(0).search();
...
//        }
        ...
        funcObj.func = function () {
            /**
             * Apply Buy Logic, To Get selected StkCode get using
             * orderStatusPanel.contextMenu.stkCode
             */
            var ordRec = equityManualPrtfPanel.cMenuObject;
            var stk = getActiveTabStkPrice(modeOrdBuy);
            var prc = stk.price;
            if (prc && !isNaN(prc)) {
                ordRec.price = prc;
            }
            if (ordRec != null) {
                createOrderPad(equityManualPrtfPanel.cMenuStkCode, equityManualPrtfPanel.cMenuStkName, false, modeOrdBuy, ordRec);
            }
        };
        funcs.push(funcObj);
        ...
        funcObj.func = function () {
            var ordRec = equityManualPrtfPanel.cMenuObject;
            var stk = getActiveTabStkPrice(modeOrdSell);
            var prc = stk.price;
            if (prc && !isNaN(prc))
                ordRec.price = prc;
            createOrderPad(equityManualPrtfPanel.cMenuStkCode, equityManualPrtfPanel.cMenuStkName, false, modeOrdSell, ordRec);
        };
        funcs.push(funcObj);
...
function createFundamentalCPIQWin(stkcode, portlet_col, index) {
    ...
    if (stkcode != '') {
        ...
        if (jsutil.toBoolean(showUISettingItem[2]) && jsutil.toBoolean(global_NewWindow_News)) {
            ...
            if (fundCpIq == null) {
                fundCpIq = Ext.create('Ext.ux.IFrame', {
                ...
                n2nLayoutManager.addItem(fundCpIq, portlet_col, index);
...
function createFundamentalScreenerCPIQWin(portlet_col, index) {
...
function createSettlementSubMenu(settSubMenus, settSetting, isMetroMenu, portlet_col, index) {
    ...
    switch (settSetting) {
        case webESettlementURL:
            ...
            mobileMetroIcon = ICON.SSETTLEMENT;
            break;
...
function changeFontSize(fontsize, isSaved, isFirst) {
    if (jsutil.toBoolean(showUISettingItem[5]) && !isMobile) {
        if (!isSaved || isFirst) {
            ...
            changeClsStyle(".n2n .x-grid-row .x-grid-cell", "font-size:" + fontsize + "px !important;");
            changeClsStyle(".N2N_stockNameStyle div", "font-size:" + fontsize + "px !important;");
            
            var opFont = fontsize > 14 ? 14 : fontsize;
            changeClsStyle(".orderpad .x-form-item-label", "font-size:" + opFont + "px !important;line-height:" + lineHeight + 'px!important');
...
function adjustAllColumnWidthsSettings(adjustWidth) {
    if (adjustWidth !== 0) {
        ...
        for (var i = 0; i < viewExCodes.length; i++) {
            var qck = cookieKey + '_Quote_' + viewExCodes[i];
            cookies.adjustAllColWidths(qck, adjustWidth);
        }
        
        // watchlist
        for (var i = 0; i < watchListArr.length; i++) {
            var wlck = cookieKey + '_Watchlist_' + watchListArr[i];
            cookies.adjustAllColWidths(wlck, adjustWidth);
        }
        
        // recent quote
        cookies.adjustAllColWidths(cookieKey + '_RecentQuote', adjustWidth);
        
        // order status
        cookies.adjustAllColWidths(cookieKey + '_OrdSts', adjustWidth);
        
        cookies.adjustAllColWidths(cookieKey + '_OrdHistory', adjustWidth);
        cookies.adjustAllColWidths(cookieKey + '_Portfolio', adjustWidth);
        cookies.adjustAllColWidths(cookieKey + '_RealizeGL', adjustWidth);
        
        cookies.adjustAllColWidths2(N2N_CONFIG.trackerTSColStateId, adjustWidth);
        cookies.adjustAllColWidths2(N2N_CONFIG.trackerBSColStateId, adjustWidth);
        
        cookies.adjustAllColWidths(N2N_CONFIG.scoreColStateId, adjustWidth);
        cookies.adjustAllColWidths2(N2N_CONFIG.dpColStateId, adjustWidth);
    }
...
function restoreAllColumnWidthSettings() {
    ...
    // recent quote
    cookies.eraseCookie(cookieKey + '_RecentQuote');

    cookies.eraseCookie(cookieKey + '_OrdSts');
    cookies.eraseCookie(cookieKey + '_OrdHistory');
    cookies.eraseCookie(cookieKey + '_Portfolio');
    cookies.eraseCookie(cookieKey + '_RealizeGL');
    cookies.eraseCookie(N2N_CONFIG.trackerTSColStateId);
    cookies.eraseCookie(N2N_CONFIG.trackerBSColStateId);
    cookies.eraseCookie(N2N_CONFIG.scoreColStateId);
    cookies.eraseCookie(N2N_CONFIG.dpColStateId);
}
...
function setFontStyle() {
    ...
    if (jsutil.toBoolean(showUISettingItem[7]) && !isMobile) {
        changeClsStyle(".N2N_FontUp", "color:#" + gl_up + " !important;");
        changeClsStyle(".N2N_FontDown", "color:#" + gl_down + " !important;");
        changeClsStyle(".N2N_FontUnchange", "color:#" + gl_unchg + " !important;");
        changeClsStyle(".N2N_FontUnchange_yellow", "color:#" + gl_yel + " !important;");
    }
...
function createAutoWidthButton(grid, gridItem) {
