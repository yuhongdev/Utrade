    ...
    initComponent: function() {
        ...
        if (defaultMenu) {
            ...
            me.compRef.menuBtn = Ext.create('Ext.button.Button', {
                ...
                style: "background-color: "+bgMbMenu+";",
                enableToggle: true,
                icon: ICON.MENU,
                cls: 'menubtn'
            });
...
//            		bodyCls:'mainmenuTool',
    ...
    prepareLayouts: function() {
        ...
        if (me.isWindowLayout()) {
            if (isMobile) {
                ...
                if (N2N_CONFIG.splitScreen) {
                    ...
                    me.compRef.mainCt1 = Ext.create('Ext.container.Container', {
                        ...
                        flex: 1 - me.lyConf.getScreenSize('mt2', true) - me.lyConf.getScreenSize('mt5', true) - me.lyConf.getScreenSize('mt7', true),
                        hidden: me.lyConf.getScreenHidden('mt') && me.lyConf.getScreenHidden('mt3'),
                        height: '100%',
                    ...
                    me.compRef.mainCt2 = Ext.create('Ext.container.Container', {
                        ...
                        split: true,
                        minWidth: mctMinWidth,
                        hidden: me.lyConf.getScreenHidden('mt2') && me.lyConf.getScreenHidden('mt4'),
                        items: [me.compRef.mt2, me.compRef.mt4],
                        height: '100%',
                        flex: me.lyConf.getScreenSize('mt2', true),
                    ...
                    me.compRef.mainCt3 = Ext.create('Ext.container.Container', {
                        ...
                        split: true,
                        minWidth: mctMinWidth,
                        hidden: me.lyConf.getScreenHidden('mt5') && me.lyConf.getScreenHidden('mt6'),
                        items: [me.compRef.mt5, me.compRef.mt6],
                        height: '100%',
                        flex: me.lyConf.getScreenSize('mt5', true),
                        listeners: me.getMainTabListernerObj()
                    });
                    me.compRef.mainCt4 = Ext.create('Ext.container.Container', {
                        ...
                        split: true,
                        minWidth: mctMinWidth,
                        hidden: me.lyConf.getScreenHidden('mt7') && me.lyConf.getScreenHidden('mt8') && me.lyConf.getScreenHidden('mt9'),
                        items: [me.compRef.mt7, me.compRef.mt8, me.compRef.mt9],
                        height: '100%',
                        flex: me.lyConf.getScreenSize('mt7', true),
                        listeners: me.getMainTabListernerObj()
                    });
                    ...
                    me.compRef.mt10 = Ext.create('Ext.tab.Panel', {
                        ...
                        height: '100%',
                        minWidth: mctMinWidth,
                        flex: 1 - me.lyConf.getScreenSize('mt11', true) - me.lyConf.getScreenSize('mt12', true) - me.lyConf.getScreenSize('mt13', true)
                    });
                    me.compRef.mt11 = Ext.create('Ext.tab.Panel', {
                        ...
                        height: '100%',
                        minWidth: mctMinWidth,
                        flex: me.lyConf.getScreenSize('mt11', true)
                    });
                    me.compRef.mt12 = Ext.create('Ext.tab.Panel', {
                        ...
                        height: '100%',
                        minWidth: mctMinWidth,
                        flex: me.lyConf.getScreenSize('mt12', true)
                    });
                    me.compRef.mt13 = Ext.create('Ext.tab.Panel', {
                        ...
                        height: '100%',
                        minWidth: mctMinWidth,
                        flex: me.lyConf.getScreenSize('mt13', true)
                    });
            ...
            var mainTabPosition = 'top';
            var tabnTabPosition = global_personalizationTheme.indexOf("wh") != -1?"top":'bottom';
            var ct1Region = 'south';
            var ct1Layout = 'hbox';
            var ct1Width = null;
            var ct1Height = 230;
            var ct1AutoScroll = null;
            var subLayout = me.lyConf.getSubLayout();
            var splScr = jsutil.toBoolean(me.lyConf.getSplitScreen());

            if (subLayout == '3') {
                mainTabPosition = 'bottom';
                tabnTabPosition = 'top';
                ct1Region = 'north';
            } else if (subLayout == '4' || subLayout == '5') {
                tabnTabPosition = 'top';
                ct1Region = 'east';
                if (subLayout == '5') {
                    ct1Region = 'west';
            ...
            var ct1Items = [];
            if (subLayout == '2' || subLayout == '3' || subLayout == '4' || subLayout == '5') {
                var bodySize;
                var tabMinSize;
                if (subLayout == '2' || subLayout == '3') {
                    bodySize = window.innerWidth;
                    tabMinSize = N2N_CONFIG.tabMinWidth;
                } else {
                    bodySize = window.innerHeight;
                    tabMinSize = ct1Height;
                }

                var tabNum = bodySize / tabMinSize;
                var tabCount = parseInt(tabNum);
                var diffNum = tabNum - tabCount;
                var diffPer = 0.85;
                if (isTablet) {
                    diffPer = 0.5; // reduces diffPer for tablet
                }
                if (diffNum >= diffPer) {
                    tabCount = tabNum.toFixed();
                }
                if (tabCount > 4) { // maximum tab panels
                    tabCount = 4;
                }

                if (tabCount > 2 && !N2N_CONFIG.announcement) {
                    tabCount -= 1;
                }
                if (tabCount > 2 && !N2N_CONFIG.mktSummary) {
                    tabCount -= 1;
                }

                for (var i = 1; i <= tabCount; i++) {
                    if (i === 3 && !N2N_CONFIG.announcement) {
                        i = 4;
                    }
                    ...
                    if (i == tabCount) {
                        ...
                        tabConf.minWidth = N2N_CONFIG.tabMinWidth;
                        tabConf.flex = 1;

                        if (i == 1) {
                            tabConf.minHeight = ct1Height;
                            tabConf.maxHeight = ct1Height;
                        }
                    }
                    ...
                    if (i > 1) {
                        ...
                        if (N2N_CONFIG.configScreen) {
                            tabConf.settingCt = Ext.create('Ext.container.Container', {
                                xtype: 'container',
                                items: {
                                    xtype: 'button',
                                    icon: icoBtnConfig,
                                    tooltip: touchMode ? '' : languageFormat.getLanguage(31113, 'Set default screen for this tab'),
                                    menu: n2ncomponents.getMiniScreenList(tabId),
                                    handler: function() {
                            ...
                            tabConf.listeners = {
//                            tabchange: function(thisTab, newCard) {
//                                if (newCard.closable) {
//                                    thisTab.settingCt.hide();
//                                } else {
//                                    thisTab.settingCt.show();
//                                }
//                            }
                                tabchange: me.tabchangeHandler
                            };
                    ...
                    me.compRef[tabId] = Ext.create('Ext.tab.Panel', tabConf);

                    if (i > 1) {
                        ct1Items.push(me.compRef[tabId]);
                    }
                }

                ct1Items.push(me.compRef.tab1);

            } else if (subLayout == '6') {
                ct1Layout = 'fit';
                var bodyHeight = window.innerHeight;
                me.compRef.tab6 = Ext.create('Ext.tab.Panel', {
                    height: bodyHeight * 0.45,
                    minHeight: 250,
                    maxHeight: 400,
                    width: '100%'
                });
                ct1Items.push(me.compRef.tab6);
            }

            me.compRef.container1 = Ext.create('Ext.container.Container', {
                region: ct1Region,
                layout: ct1Layout,
                width: ct1Width,
                border: false,
                autoScroll: ct1AutoScroll,
                items: ct1Items
            });
            ...
            if (N2N_CONFIG.splitScreen) {
                mainTabConf.cls = 'splitscr';
                mainTabConf.listeners.afterrender = me._getActiveScreenHandler();

                if (N2N_CONFIG.configScreen)
                    mainTabConf.tabBar = me.getTabConfigureTool('mainTab');

                // creates split screen
                var mtConfTbar2;
                if (N2N_CONFIG.configScreen) {
                    mtConfTbar2 = me.getTabConfigureTool('mainTab2');
                }
                var mainTabConf2 = {
                    mt: 'mainTab2',
                    split: true,
                ...
                if (subLayout == 4 || subLayout == 5) {
                    mainTabConf2.region = 'south';
                    mainTabConf2.height = me.lyConf.getScreenSize('mainTab2');
                    mainTabConf2.minHeight = 200;
                    mainTabConf.minHeight = 200;
                } else {
                    mainTabConf2.region = 'east';
                    mainTabConf2.width = me.lyConf.getScreenSize('mainTab2');
                    mainTabConf2.minWidth = 350;
                    mainTabConf.minWidth = 350;
                }
            }
            
            // Main tab
            me.compRef.mainTab = Ext.create('Ext.tab.Panel', mainTabConf);
            var appMainItems = new Array();
            
            if (mainTabConf2) {
                me.compRef.mainTab2 = Ext.create('Ext.tab.Panel', mainTabConf2);

                me.compRef.mainContainer = Ext.create('Ext.container.Container', {
                    layout: 'border',
                    region: 'center',
                    items: [me.compRef.mainTab, me.compRef.mainTab2]
                });
                appMainItems = [me.compRef.mainContainer, me.compRef.container1];
            } else {
                appMainItems = [me.compRef.mainTab, me.compRef.container1];
            }
    ...
    addItem: function(item, portcol, index, tabCt) {
        ...
        var iconCls = me.getIconCls(item.type);
            ...
            if (isMobile) {
                ...
                if (!newItem.notComp) {
                    newItem.on('close', function(thisComp) {
                        ...
                        me.lyConf.removeScreen('p', thisComp.type, thisComp.key);
        ...
        if (!isMobile && item.screenType == 'main' && !item.ignoredScreen) {
            if (me.isTabLayout() || tabActivate) {
                newItem.on('activate', function() {
                    var activeId = item.getId();
                    if (me.debug) {
                        console.log('N2NLayoutManager > main screen: item activated...');
    ...
    switchToTab: function(item) {
        ...
        if (item._preMaxComp) {
            ...
            var tabItem = me._getTabConfig({
                ...
                stkname: item.stkname,
                screenType: item.screenType,
                iconCls: me.getIconCls(item.type)
            });
    ...
    destroyExtraButtons: function(compItem) {
        ...
        if (compItem.syncCb) {
            compItem.syncCb.destroy();
            compItem.syncCb = null;
        }
    },
    ...
    getActiveRecord: function(wholeRecord) {
        ...
        if (activeItem && activeItem.screenType == 'main') {
            ...
            if (records.length == 0 && quoteScreen) {
                // if no record, get record from quote screen
                records = quoteScreen.getSelectionModel().getSelection();
            }
            if (records.length > 0) {
                return me.getStkRec(records[0], wholeRecord);
            }
        }

        return {};
    },
    getStkRec: function(rec, wholeRec) {
        var record = {};
    ...
    setAppMsg: function(msg) {
        if (this.debug) {
            console.log('setAppMsg...');
        }
        var appMsg = Ext.get('appmsg');
        appMsg.show();
        appMsg.setHtml(msg);
    },
    hideAppMsg: function() {
        if (this.debug) {
            console.log('hideAppMsg...');
        }
        var appMsg = Ext.get('appmsg');
        appMsg.hide();
        appMsg.setHtml('');
            ...
            me.compRef.plainMenu = me.addItem({
                ...
                items: {
                    ...
                    tpl: [
                        ...
                        '<div class="subicon"><img src="{icon}"/></div>',
    ...
    getIconCls: function(compType) {
        if (typeof compType !== 'string' || global_personalizationTheme.indexOf("wh")!= -1) {
            return '';
        }
        
        if (compType === 'gn' || compType === 'sn') {
            compType = 'news';
        }
        if (compType === 'oh') {
            compType = 'os';
        }
        if (compType === 'dp' || compType === 'ep' || compType === 'rp') {
            compType = 'pf';
        }
        return 'comp_icon comp_' + compType;
    },
            ...
            if (isMobile) {
                if (tabId === 'am') {
                    ...
                    for (var s in cfScr) {
                        ...
                        n2ncomponents.loadScreen(tabId, scr.comp, scr.key, scr.stkname);
                        me.setLoading(false);
                        emptyHome = false;
                    }
                    }
    ...
    loadConfiguredPopup: function() {
        ...
        for (var s in cfScr) {
            ...
            if (me.getFeatureStatus(scr.comp)) {
                ...
                n2ncomponents.loadScreen('p', scr.comp, scr.key, scr.stkname, null, {
                    winConfig: extConf
                });
            }
        }
        ...
        if (comp) {
            if (comp.getDockedItems || comp._topBar_) {
                ...
                if (dockedItems.length > 0) {
                    ...
                    // sync checkbox
                    if(isSyncComp(comp.type) && !comp.syncCb){
            ...
            if (!comp.popupBtn) {
                ...
                if (compEl) {
                    var popupBtnEl = Ext.DomHelper.append(compEl, {tag: 'div', cls: 'extrabtn btn' + btnIdx++});

                    comp.popupBtn = Ext.create('Ext.button.Button', {
                        icon: ICON.POPUP,
                        tooltip: 'Pop up this tab',
                        cls: 'fix_btn',
                        handler: function() {
                            me._toPopup(tab);
                        },
            ...
            if (createBtn1) {
                ...
                if (compEl) {
                    var maxBtnEl = Ext.DomHelper.append(compEl, {tag: 'div', cls: 'extrabtn btn' + btnIdx});
            ...
            for (var s in cfScr) {
                if (me.lyConf.isLayoutKey(s)) {
                    ...
                    var bTab = me._getTabConfig({
                        ...
                        iconCls: me.getIconCls(cfScr[s].comp),
                        isBufferredTab: true,
                        mt: tabId,
                        type: cfScr[s].comp,
                        savingComp: cfScr[s].savingComp,
                        key: cfScr[s].key,
                        stkname: cfScr[s].stkname
                    });
    ...
    restorePreviousLayout: function() {
        ...
        if (me.prevLayoutStr) {
            me.loadLayout(null, me.prevLayoutStr);
    ...
    loadLayout: function(layoutId, layoutStr) {
        var me = this;
    ...
    tabchangeHandler: function(thisTp, newTab, oldTab) {
        ...
        if (newTab.isBufferredTab) {
            // load buffer tab
            n2ncomponents.loadScreen(newTab.mt, newTab.type, newTab.key, newTab.stkname, newTab);
    ...
    initializeCompDragZone: function(thisComp) {
        ...
        if (N2N_CONFIG.layoutDD) {
            ...
            thisComp.dragZone = new Ext.dd.DragZone(thisComp.body.component.tab.el, {
                    ...
                    dropComp[dropId] = new Ext.dd.DropZone(els[i], {
                        ...
                        onNodeDrop: function(target, dd, e, data) {
                            ...
                            if (this.compType === 'wl') {
                                ...
                                n2ncomponents.loadScreen(null, this.compType, rec.stkCode, rec.stkName);
                            }
