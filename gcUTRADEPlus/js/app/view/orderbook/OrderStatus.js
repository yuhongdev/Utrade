...
Ext.define('TCPlus.view.orderbook.OrderStatus', {
...
//						}
                        ...
                        if (pagedata.total <= paging.pageSize) {
                            //disable both
                            tbar.getComponent('os_prev').disable();
                            tbar.getComponent('os_next').disable();
                        } else if (page == 1) {
                            //disable prev
                            tbar.getComponent('os_prev').disable();
                            tbar.getComponent('os_next').enable();
                        } else if (page == lastpage) {
                            //disable next
                            tbar.getComponent('os_prev').enable();
                            tbar.getComponent('os_next').disable();
                        } else {
                            //disable none
                            tbar.getComponent('os_prev').enable();
                ...
                }),
                {
                    ...
                    toggleHandler: function (thisBtn, state) {
                        ...
                        if (!panel.amalFeature) {
                            ...
                            if (pview) {
                                panel.amalFeature = pview.getFeature('orderGrouping');
                            }
                        }
                        ...
                        msgutil.confirm(languageFormat.getLanguage(30008, 'You have unsaved settings. Would you like to save your settings?'),
                                function (sResp) {
                                    if (sResp == "yes") {
                                        ...
                                        cookies.procCookie(panel.orckId, panel.tempWidth, cookieExpiryDays);
                                        panel.tempWidth = cookies.toDefaultColumn(panel, panel.orckId);
                                    } else {
                                        panel.isImgBlink = false;
                                    }
                                }
        ...
        panel.compRef.bottomBar = Ext.create('Ext.toolbar.Toolbar', {
            ...
            items: [
                ...
                },
                {
                    ...
                    name: 'chkSavePinw',
                    hideLabel: true,
                    boxLabel: '<img style="width: 16px; height: 16px" src="' + iconBtnSave + '"/>&nbsp;',
                    hidden: this.checkIsHidePin(), //1.3.25.3
                    listeners: {
...
//                    width: 60,
    ...
    procColumnWidth: function () {
        ...
        if (cookies.isCookiesExist(panel.orckId)) {
            var temp = cookies.getCookies(panel.orckId);
            columnID = temp[0];
            columnWidth = temp[1];
        }
        else {
        ...
        try {
            if (!isDealerRemisier || (isDealerRemisier && panel.accNo != '')) {
                ...
                if (!background) {
                    ...
                    this.down('#topToolbar').down('#os_btnDetail').disable();
...
//  			urlbuf.push(dwr.engine._scriptSessionId);
                ...
                if (this.filterExtOpt != '0') {
                    urlbuf.push('&exfilt=');
                    urlbuf.push(this.filterExtOpt);
                } else {
                    urlbuf.push('&exfilt=');
                    urlbuf.push(0);
                }
                    ...
                    success: function (response) {
                        if (orderPad) {
                            if (orderPad.getAccountNo().length > 0) {
                                //						panel.callAccBal(orderPad.getAccountNo());
...
//  						}
                            ...
                            if (datas) {
                                ...
                                for (var i = 0; i < datas.length; i++) {
                                    ...
                                    if (!contains(stkcodes, tempStkCode)) {
    ...
    localSearch: function () {
        ...
        panel.getStore().filterBy(function (record, id) {
            ...
            return blnFilterAccNo && blnFilterSts && blnFilterPaymentCode && blnFilterEx;
        });
...
//			else return;
            ...
            panel.setAccBal(panel.formatTradingLimitDecimal(accBal));
            panel.setNetCashLimit(netCashLimit);
    ...
    setAccBal: function (v) {
        if (v != null && !isNaN(v)) {
            if (this.compRef.hasOwnProperty('accBal')) {
                helper.setHtml(this.compRef.accBal, this.formatCurrency(v));
            }
        }
    },
    setNetCashLimit: function (v) {
        if (v != null && !isNaN(v)) {
            if (this.compRef.hasOwnProperty('netCashLimit')) {
                helper.setHtml(this.compRef.netCashLimit, this.formatCurrency(v));
            }
        }
    },
    ...
    updateSummary: function () {
        ...
        try {
            ...
            for (var i = 0; i < total; i++) {
                ...
                var mtQty = parseInt(recData.MtQty);
                var mtPrc = parseFloat(recData.MtPrc);
...
//							}
                        ...
                        var buff = [
                            ...
                            {id: cmap_osValidity, value: d.dv, cellFormat: 'string'},
                            ...
                            {id: 'lastUpdateTime', value: d.lut, cellFormat: null},
...
//				}
            ...
            //Ext.fly('ordSts_totOrdVal').update(this.currentCurrency + ' ' + panel.formatCurrency(panel.formatDecimal(obj.tov, 2)));
        ...
        try {
            ...
            for (var i = 0; i < total; i++) {
                ...
                if (pagingMode)
                    ...
                    rec = store.getAt(i);
                var recData = rec.data;
                var ordQty = parseInt(recData.OrdQty);
                var ordPrc = parseFloat(recData.OrdPrc);
...
//				console.log("recCurrency: " + recCurrency);
                if (panel.convertCurrencyEnable) {	// convert to the currency value
                    for (var j = 0; j < panel.currencyRateList.size; j++) {
                        var inListCurrency = panel.currencyRateList.d[j].currate[0];
                        if (recCurrency == panel.currentCurrency) {
                            // same currency no need convert
//							console.log("in same currecnt: " + recCurrency);
                            if (!isNaN(ordVal)) {
...
//							console.log("need convert");
                        	var recAction = recData.PrevAct;	// BUY or Sell
                            var buyRate = panel.currencyRateList.d[j].currate[3];
                            var sellRate = panel.currencyRateList.d[j].currate[4];
                            var denomiation = panel.currencyRateList.d[j].currate[5];
                            var valOrd = 0;
                            // convert
                            if (!isNaN(ordVal)) {
                            	if(recAction.toLowerCase() == "buy"){
...
//        var ordno = lsordno.join('|');
        ...
        var params = {"selectedRecords": selectedRecords, "pin2": pin2};
                ...
                try {
                    ...
                    if (o.success == 1) {
                        ...
                        for (var i = 0; i < resultCount; i++) {
                            ...
                            var recidx = store.findExact('OrdNo', r.on, 0);
                            sm.deselect(recidx);
                        }
    ...
    selectRecord: function (recData) {
        if (recData) {
            this.down("#topToolbar").getComponent('os_btnDetail').enable();
            this.tktno = recData.get('TktNo');
            this.ordno = recData.get('OrdNo');
            this.stkname = recData.get('StkName');
        } else {
            this.down("#topToolbar").getComponent('os_btnDetail').disable();
        }
    },
    cleanUp: function () {
        this.accNo = '';
    ...
    createContextMenu: function () {
        if (showV1GUI == "TRUE") {
            ...
            btns.push({
                ...
                popupOnly: true,
                handler: function () {
                    var ordRec = orderStatusPanel.contextMenu.ordRec;
                    if (ordRec != null) {
                        ...
                        if ((tktno || ordno) && canCancelRevise(ordsts)) {
                            closedOrderPad = false;
                            createOrderPad(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName, modeOrdRevise, ordRec, true);
            ...
            btns.push({
                ...
                handler: function () {
                    ...
                    if (ordRec != null) {
                        ...
                        if ((tktno || ordno) && canCancelRevise(ordsts)) {
                            closedOrderPad = false;
                            createOrderPad(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName, modeOrdCancel, orderStatusPanel.contextMenu.ordRec, true);
                            if (n2nLayoutManager.isTabLayout()) {
                                closedMarketDepth = false;
                            }
                        }
            ...
            if (showStkInfoHeader == "TRUE") {
                if (showStkInfoMarketDepth == "TRUE") {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                             ...
                             */
                            closedMarketDepth = false;
                            createMarketDepthPanel(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName, true);
                ...
                if (showStkInfoStkInfo == "TRUE") {
                    ...
                    btns.push({
                        ...
                        icon: icoBtnStkInfo,
                        handler: function () {
                            createStkInfoPanel(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName, false);
                ...
                if (showStkInfoTracker == "TRUE") {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                             ...
                             * marketMoverPanel.contextMenu.stkCode
                             */
                            createTrackerPanel(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName, false);
                        }
                    });
                }
                ...
                if (showStkInfoEquitiesTracker == "TRUE") {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                            n2ncomponents.createEquitiesTracker(orderStatusPanel.contextMenu.stkCode);
                        }
                    });
                }
                ...
                if (N2N_CONFIG.features_HistoricalData) {
                    ...
                    btns.push({
                        ...
                        icon: ICON.HIS_DATA,
                        handler: function () {
                            n2ncomponents.createHistoricalData(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName);
                        }
                    });
                }
            }
                ...
                if (showChartIntradayChart == "TRUE") {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                            ...
                            createChartPanel(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName, false);
                        }
                    });
                }
                ...
                if (showChartAnalysisChart == "TRUE") {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                            ...
                            createAnalysisChartPanel(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName, false);
            ...
            if (showOrdBookHeader == "TRUE") {
                ...
                if (showOrdBookOrderPosition == "TRUE") {
                    this.menuOrdPositionId = Ext.id();
                    btns.push({
                        id: menu.menuOrdPositionId,
                        text: languageFormat.getLanguage(20176, 'Order Position'),
                        icon: icoBtnOrdSts,
                        handler: function () {
                            var ordNo = orderStatusPanel.contextMenu.ordRec.ordNo;
                            var seqNo = orderStatusPanel.contextMenu.ordRec.seqNo;
                            var priceIndicator = orderStatusPanel.contextMenu.ordRec.Action;
                            var orderDateTime = orderStatusPanel.contextMenu.ordRec.dateTime;

                            ordNo = trim(ordNo) == null ? '' : trim(ordNo);
                            seqNo = trim(seqNo) == null ? '' : trim(seqNo);
                            priceIndicator = trim(priceIndicator) == null ? '' : trim(priceIndicator);
                            orderDateTime = trim(orderDateTime) == null ? '' : trim(orderDateTime);

                            if (priceIndicator.length > 0) {
                                if (priceIndicator.toLowerCase() == 'buy') {
                                    priceIndicator = 'b';
                                } else {
                                    priceIndicator = 's';
                                }
                            ...
                            orderStatusPanel.callOrderPosition(orderStatusPanel.contextMenu.stkCode, ordNo, priceIndicator, seqNo, orderDateTime);
                        }
                    });
                }
                if (showOrdBookOrderDetails == "TRUE") {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                             ...
                             * marketMoverPanel.contextMenu.stkCode
                             */
                            var tktno = orderStatusPanel.contextMenu.ordRec.tktNo;
                            var ordno = orderStatusPanel.contextMenu.ordRec.ordNo;
                            var tempAccNo = orderStatusPanel.getAccountValue();
                            tktno = trim(tktno) == null ? '' : trim(tktno);
                            ordno = trim(ordno) == null ? '' : trim(ordno);
                            createOrderDetailPanel(ordno, tktno, orderStatusPanel.contextMenu.stkName, false, 'ordStatusPanel', null, tempAccNo);
            ...
            if (showNewsHeader == "TRUE") {
                if (showNewsStockNews == "TRUE") {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                             ...
                             * marketMoverPanel.contextMenu.stkCode
                             */
                            createStkNewsPanel(orderStatusPanel.contextMenu.stkCode, orderStatusPanel.contextMenu.stkName);
                        }
                    });
                }
                ...
                if (N2N_CONFIG.featuresNews_Archive) {
                    ...
                    btns.push({
                        ...
                        icon: icoBtnArchive,
                        handler: function () {
                            n2ncomponents.openArchiveNews({key: orderStatusPanel.contextMenu.stkCode, name: orderStatusPanel.contextMenu.stkName});
                        }
                    });
                }
                if (N2N_CONFIG.elasticNewsUrl) {
                    ...
                    btns.push({
                        ...
                        icon: icoBtnArchive,
                        handler: function () {
                            n2ncomponents.openElasticNews({key: orderStatusPanel.contextMenu.stkCode, name: orderStatusPanel.contextMenu.stkName, newsOpt: '1'});
                        }
                    });
                }
                ...
                if (N2N_CONFIG.featuresNews_FundamentalCPIQ) {
                    ...
                    btns.push({
                        ...
                        icon: icoBtnNews24,
                        handler: function () {
                            createFundamentalCPIQWin(orderStatusPanel.contextMenu.stkCode);
                        }
                    });
                }
                ...
                if (N2N_CONFIG.featuresNews_FundamentalThomsonReuters) {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                             ...
                             * marketMoverPanel.contextMenu.stkCode
                             */
                            createFundamentalThomsonReutersWin(orderStatusPanel.contextMenu.stkCode);
                        }
                    });
                }
            }
                ...
                if (N2N_CONFIG.featuresSetting_AddStockAlert) {
                    ...
                    btns.push({
                        ...
                        handler: function () {
                             ...
                             * selected StkCode get using
                             */
                            n2ncomponents.createAddStockAlert(orderStatusPanel.contextMenu.stkCode);
                        }
                    });
                }
            }
            ...
            if (showWarrantsInfo == "TRUE") {
                ...
                btns.push({
                    ...
                    handler: function () {
                         ...
                         * marketMoverPanel.contextMenu.stkCode
                         */
                        createWarrantsInfoWin(orderStatusPanel.contextMenu.stkCode);
                    }
                });
            }
            ...
            if (settingSMSStockAlertURL.length > 0) {
                ...
                btns.push({
                    ...
                    handler: function () {
                    	n2ncomponents.createSMSStockAlert(orderStatusPanel.contextMenu.stkCode);
                    }
                });
            }
            ...
            if (N2N_CONFIG.pseEdgeURL.length > 0) {
                ...
                btns.push({
                    ...
                    text: 'PSE Edge',
                    handler: function () {
                    	n2ncomponents.openPseEdge(orderStatusPanel.contextMenu.stkCode);
                    }
                });
            }
...
// region start custom column functions
    ...
    allColumnSetting: function () {
        var allColumnId = global_OSAllColumn;
        return allColumnId;
    },
    defaultColumnSetting: function () {
        var defaultColumnId = global_OSDefaultColumn;
        return defaultColumnId;
    },
    currentColumnSetting: function () {
        var currentColumnId = orderStatusCol;
    ...
    requestSaveColumns: function(newColumnIdArr) {
        ...
        orderStatusCol = newColumnIdArr; // orderStatusCol is from main.jsp
        colutils.saveColumn(osColSetting, newColumnIdArr);
    },
    /**
...
//			value = tempObj.value;
    ...
    updateColWidthCache: function(column, newWidth) {
        ...
        if (global_allowCookies == "TRUE") {
            panel.tempWidth = cookies.procTempCookies(panel, panel.orckId, colId, newWidth);
            cookies.procCookie(panel.orckId, panel.tempWidth, cookieExpiryDays);
        }
    }
});
