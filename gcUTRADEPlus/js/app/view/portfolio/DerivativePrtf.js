    ...
    updateFeedRecord: function (dataObj) {
        ...
        gridStore.each(function (record, i) {
            if (record.get('43') == dataObj[fieldStkCode]) {
                record.data['lastDone'] = dataObj[fieldLast];

                if (dataObj[fieldLast] != null && parseFloat(dataObj[fieldLast]) != 0) {
                    panel.tempStkPrice = dataObj[fieldLast];
                } else if (dataObj[fieldLacp] != null && parseFloat(dataObj[fieldLacp]) != 0) {
                    panel.tempStkPrice = dataObj[fieldLacp];
                } else if (dataObj[fieldOpen] != null && parseFloat(dataObj[fieldOpen]) != 0) {
                    panel.tempStkPrice = dataObj[fieldOpen];
                } else if (dataObj[fieldPrev] != null && parseFloat(dataObj[fieldPrev]) != 0) {
                    panel.tempStkPrice = dataObj[fieldPrev];
                }

                record.data['unrealised'] = parseFloat((panel.tempStkPrice - record.get('78')) * record.get('nettPosition') * record.get('118')).toFixed(6);
                record.data['totalpl'] = parseFloat((+record.get('unrealised')) + (+record.get('125'))).toFixed(6); //add '+' in front of variable to force string to become numbers

                updateCellInfo.push({
                    column: 'lastDone',
                    row: i,
                    value: dataObj[fieldLast]
                },
                {
                    column: 'unrealised',
                    row: i,
                    value: record.data['unrealised']
                },
                {
                    column: 'totalpl',
                    row: i,
                    value: record.data['totalpl']
                });
    ...
    _updateInfo: function (dataObj) {
        ...
        if (dataObj != null) {
            if (dataObj[0] != null) {
                ...
                panel.value_BFCashBalance = parseFloat((dataObj[0]['103'] == null ? 0 : dataObj[0]['103']));
                panel.value_Deposit = parseFloat((dataObj[0]['104'] == null ? 0 : dataObj[0]['104']));
                panel.value_WithDrawal = parseFloat((dataObj[0]['105'] == null ? 0 : dataObj[0]['105']));
                panel.value_RealisedPL = parseFloat((dataObj[0]['125'] == null ? 0 : dataObj[0]['125']));

                panel.value_CurrentBalance = 0;
                panel.value_UnrealisedPL = 0;
                panel.value_Equity = 0;

                panel.value_BuyOptMktVal = parseFloat((dataObj[0]['99'] == null ? 0 : dataObj[0]['99']));
                panel.value_SellOptMktVal = parseFloat((dataObj[0]['100'] == null ? 0 : dataObj[0]['100']));
                panel.value_NetLiquidations = 0;

                panel.value_EligibleCollateral = parseFloat((dataObj[0]['106'] == null ? 0 : dataObj[0]['106']));
                panel.value_InitialMargin = parseFloat((dataObj[0]['107'] == null ? 0 : dataObj[0]['107']));
                panel.value_MaintenanceMargin = parseFloat((dataObj[0]['108'] == null ? 0 : dataObj[0]['108']));
                panel.value_ExcessShortfall = 0;
                panel.value_MarginCall = parseFloat((dataObj[0]['109'] == null ? 0 : dataObj[0]['109']));
    ...
    _updateAccBal: function (dataObj) {
        ...
        try {
            ...
            if (BalanceLimit) {
                ...
                helper.setHtml(panel.tLabel_Balance, panel._formatValue(BalanceLimit));
            }

            if (NetCashLimit) {
                helper.setHtml(panel.tLabel_NetCash, panel._formatValue(NetCashLimit));
            }
