...
//			width: 162,
    ...
    procColumnWidth: function () {
        ...
        if (cookies.isCookiesExist(panel.prckId)) {
            var temp = cookies.getCookies(panel.prckId);
            columnID = temp[0];
            columnWidth = temp[1];
        }
        else {
    ...
    isValidColumnSetting: function () {
        if (portfolioRealCol != null && portfolioRealCol != "") {
            if (portfolioRealCol.indexOf("~") != -1) {
                return true;
            } else
                return false;
        }
    ...
    currentColumnSetting: function () {
        var currentColumnId = portfolioRealCol;
    ...
    requestSaveColumns: function(newColumnIdArr) {
        ...
        portfolioRealCol = newColumnIdArr;
        panel.updateColumnMap();
        colutils.saveColumn(pfRealColSetting, newColumnIdArr);
    },
    /**
    ...
    generateColumnID: function () {
        ...
        if (pfRealColSetting != null && pfRealColSetting != '') {
            var p = [];
    ...
    _setCookieId: function () {
        var me = this;
        me.prckId = cookieKey + '_RealizeGL';
    },
    ...
    updateColWidthCache: function(column, newWidth) {
        ...
        if (global_allowCookies == "TRUE") {
            panel.tempWidth = cookies.procTempCookies(panel, panel.prckId, colId, newWidth);
            cookies.procCookie(panel.prckId, panel.tempWidth, cookieExpiryDays);
        }
    }
});
