...
Ext.define('TCPlus.view.portfolio.EquityPrtf', {
        ...
        panel.searchAccountBtn = new Ext.button.Button({
            ...
            handler: function() {
                if (panel.tbSearchAccount.isHidden()) {
                    ...
                    panel.tComboBoxAccount.setHidden(true);
                } else {
    ...
    procColumnWidth: function() {
        ...
        if (cookies.isCookiesExist(panel.pckId)) {
            var temp = cookies.getCookies(panel.pckId);
            columnID = temp[0];
            columnWidth = temp[1];
        }
        else {
            ...
            panel.onAjaxPrftRequestId = Ext.Ajax.request({
                ...
                success: function (response) {
                    ...
                    try {
                        ...
                        console.log('[EquityPrtf][search][inner] Exception ---> ' + e);
                    }
                }
            });
        } catch (e) {
            console.log('[EquityPrtf][search] Exception ---> ' + e);
        }
    ...
    localSearch: function () {
        ...
        panel.getStore().filterBy(function (record) {
            ...
            return blnFilterPaymentCode && blnFilterEx;
        });
    ...
    callAccBal: function() {
        ...
        try {
            ...
            panel.onAjaxAccRequestId = Ext.Ajax.request({
                ...
                success: function(response) {
                    ...
                    try {
                        ...
                        console.log('[EquityPrtf][callAccBal][inner] Exception ---> ' + e);
                    }
                },
            ...
            console.log('[EquityPrtf][callAccBal] Exception ---> ' + e);
        }
    },
    /**
     * Description <br/>
        ...
        if (obj.s) {
            ...
            if (panelStore.getCount() == 0) {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    data.UrlGLAmt = panel.generateUrlGLAmt(dataObject.lp, dataObject.abp, dataObject.qh);
                    data.UrlGLPc = panel.generateUrlGLPerc(data.UrlGLAmt, dataObject.abp, dataObject.qh);
                    ...
                    data.Volume = 0;
                    data.TradeValue = panel.generateTradeValue(dataObject.qh, dataObject.abp);
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    if (rowNumber != -1) {
                        ...
                        if (record != null) {
                            ...
                            // create new record
                            var newRecord = Ext.create('TCPlus.model.EquityPrftRecord', {
                            ...
                            try {
                                ...
                                console.log('[EquityPrtf][updatePortfolio][inner] Exception ---> ' + e);
                            }
    ...
    updateAccBal: function(obj) {
        ...
        try {
            ...
            console.log('[EquityPrtf][updateAccBal] Exception ---> ' + e);
        }
    },
        ...
        try {
            ...
            if (panelStore != null && panelStore.getCount() > 0) {
                ...
                if (record != null) {
                                ...
                                if (oldValue != newValue) {
                                    if (objs.type == "none") {
                                        ...
                                        if (objs.name == 'RefPrc') {
                                            ...
                                            N2NUtil.updateCell(panel, rowIndex, avgPurPrcColumnIndex, record.data['AvgPurPrc'], fontcss);
                    ...
                    if (dataObj[fieldLacp] != null || dataObj[fieldLast] != null) {
...
//                    }
            ...
            console.log('[EquityPrtf][updateFeedRecord] Exception ---> ' + e);
        }
    },
    _getStockData: function () {
        ...
        if (me.stkcodes && me.stkcodes.length > 0) {
            conn.getStockList({
                list: me.stkcodes,
                f: [fieldStkCode, fieldLacp, fieldCurrency, fieldLotSize, fieldVol, fieldLast, fieldPrev, fieldBuy, fieldSell, fieldHigh, fieldLow, fieldStatus, fieldIndexCode],
                p: 0,
    ...
    updateCalInfo2: function(record, rowNumber) {
        ...
        var gridColumns = helper.getGridColumns(panel);
        if (rowNumber != -1) {
            var valuePrc = isNaN(record.data.Last) || record.data.Last == "" || parseFloat(record.data.Last) == 0 ? record.data.RefPrc : record.data.Last;
    ...
    generateColumnName: function(id) {
        ...
        switch (id) {
            case cmap_pfName:		// 01
                ...
                break;
            case cmap_pfQtyOH:		// 02
                string = languageFormat.getLanguage(10702, "Qty.Hand");
                break;
            case cmap_pfQtyAvl:		// 03
                string = languageFormat.getLanguage(10703, "Qty.Avai");
                break;
            case cmap_pfQtyQue:		// 04
                string = languageFormat.getLanguage(10704, "Qty.Q(S)");
                break;
            case cmap_pfCode:		// 05
                string = languageFormat.getLanguage(10101, "Code");
                break;
            case cmap_pfAvgBPrc:		// 06
                string = languageFormat.getLanguage(10705, "Avg.Buy.Prc");
                break;
            case cmap_pfLast:		// 07
                string = languageFormat.getLanguage(10113, "Last");
                break;
            case cmap_pfMktVal:		// 08
                string = languageFormat.getLanguage(10706, "Mkt.Val");
                break;
            case cmap_pfAccNo:		// 09
                string = languageFormat.getLanguage(10901, "Acc.No.");
                break;
            case cmap_pfYrHigh:		// 10
                string = languageFormat.getLanguage(10719, "Yr.High");
                break;
            case cmap_pfYrLow:		// 11
                string = languageFormat.getLanguage(10720, "Yr.Low");
                break;
            case cmap_pfDayHigh:		// 12
                string = languageFormat.getLanguage(10710, "Day.High");
                break;
            case cmap_pfDayLow:		// 13
                string = languageFormat.getLanguage(10711, "Day.Low");
                break;
            case cmap_pfRef:			// 14
            ...
            case cmap_pfVol:			// 15
                ...
                break;
            case cmap_pfLtSize:		// 16
                string = languageFormat.getLanguage(10712, "Lot.Size");
                break;
            case cmap_pfChg:			// 17
                string = languageFormat.getLanguage(10115, "Chg");
                break;
            case cmap_pfChgPc:		// 18
            ...
            case cmap_pfPL:			// 20
                ...
                break;
            case cmap_pfCurrency:	// 21
                string = languageFormat.getLanguage(10132, "Currency");
                break;
            case cmap_pfExchg:		// 22
                string = languageFormat.getLanguage(10126, "Exchg");
                break;
            case cmap_pfQtySold:		// 23
                string = languageFormat.getLanguage(10709, "Qty.Sold");
                break;
            case cmap_pfQtySusp:		// 24
                string = languageFormat.getLanguage(10718, "Qty.Susp");
                break;
            case cmap_pfAsk:			// 26
            ...
            case cmap_pfavgsp:
                ...
                break;
            case cmap_pfTradeValue:
                string = languageFormat.getLanguage(10714, "Trade.Val");
                break;
            case cmap_pfSettOpt:
                string = languageFormat.getLanguage(10713, "Inv.Portfolio");
                break;
            default:
                string = "";
        }
    ...
    updateSummary: function() {
        ...
        if (store != null) {
            ...
            for (var i = 0; i < total; i++) {
                ...
                var urlGLAmt = panel.generateUrlGLAmt(valuePrc, record.data.AvgPurPrc, record.data.QtyOnHand); // add valuePrc instead of last price
        ...
        var newMenu = [
            {
                ...
                icon: ICON.HIS_DATA,
                hidden: showV1GUI == "TRUE" || !(showStkInfoHeader == "TRUE" && N2N_CONFIG.features_HistoricalData)
            }, {
                ...
                if (func.name != null) {
                    ...
                    switch (func.name) {
                        ...
                        case panel.cMenuWarrantsInfoId:
                            ...
                            btn.setHandler(func.func);
                            break;
                        case panel.cMenuStockAlertId:
                            btn = panel.contextMenu.getComponent(panel.cMenuStockAlertId);
                            btn.setHandler(func.func);
                            break;
                        case panel.cMenuPSEEdgeId:
                            btn = panel.contextMenu.getComponent(panel.cMenuPSEEdgeId);
                            btn.setHandler(func.func);
                            break;
                    }
                }
    ...
    setAccBal: function(value) {
        ...
        if (value != null && !isNaN(value)) {
            ...
            if (obj != null) {
                helper.setHtml(obj, panel.formatCurrency(value));
            }
        }
    },
    /**
    ...
    openDetailPanel: function() {
        ...
        try {
            ...
            console.log('[EquityPrtf][openDetailPanel] Exception ---> ' + e);
        }
    ...
    addForm: function() {
        ...
        /*
         * create store for search stock name and stock code 
         */
        var ds = new Ext.data.Store({
            ...
            fields: [
                ...
                {name: 'stockName', mapping: fieldStkName, convert: stockNameFormat},
                {name: 'stockNameText', mapping: fieldStkName, convert: function (value) {
                        return htmlDecode(stockNameFormat(value));
                    }
                }
            ]
        });
        ...
        /*
         * create combo box drop down list method
         */
        var resultTpl = new Ext.XTemplate(
                '<tpl for="."><div style="width:210px;" class="search-item">',
                '{stockName} <br/>',
                '</div></tpl>'
                );
            ...
            items: [
                    ...
                    items: [
                        ...
                        new Ext.button.Button({
                            ...
                            handler: function () {
                                ...
                                if (addFormSearchAccCmp.isHidden()) {
                                    addFormSearchAccCmp.setVisible(true);
                                    addFormAccCmp.setHidden(true);
                                } else {
                                    if (addFormSearchAccCmp.getValue().trim() == '') {
                ...
                new Ext.form.field.ComboBox({
                    ...
                    tpl: Ext.create('Ext.XTemplate',
                            '<tpl for=".">',
                            '<div class="x-boundlist-item">{stockName}</div>',
                            '</tpl>'
                            ),
                    ...
                    listeners: {
                        afterrender: function(thisCombo) {
                            var task = new Ext.util.DelayedTask(function() {
                                ...
                                var exCmp = Ext.getCmp("eqpf_exchg_txtField").getValue();
                                if (comboValue != "") {
                                    conn.searchStock({
                                        ex: exCmp == "" ? ex : exCmp,
                                        k: comboValue,
                                        field: [fieldStkCode, fieldStkName],
                                        mkt: conn.getMarketCode(exCmp, '[All Stocks]'),
...
//                  		Wait 1s before calling our function. If the user presses another key
// during that 700ms, it will be cancelled and we'll wait another 500ms.
...
//        tempUrl.push(orderDate);
            ...
            success: function(response) {
                ...
                try {
                    ...
                    if (obj != null) {
                        ...
                        if (obj.errorMsg != null) {
                            console.log('[EquityPrtf][addFormSubmit][inner] result : error ---> ' + obj.errorMsg);
                        }
                    }

                } catch (e) {
                    console.log('[EquityPrtf][addFormSubmit][inner] Exception ---> ' + e);
                }
            },
            failure: function(response) {
    ...
    currentColumnSetting: function() {
        var currentColumnId = portfolioCol; // verify from main.jsp
    ...
    requestSaveColumns: function(newColumnId) {
        ...
        portfolioCol = newColumnId;  // portfolioCol is from main.js
        colutils.saveColumn(pfColSetting, newColumnId);
    ...
    updateColWidthCache: function(column, newWidth) {
        ...
        if (global_allowCookies == "TRUE") {
            panel.tempWidth = cookies.procTempCookies(panel, panel.pckId, colId, newWidth);
            cookies.procCookie(panel.pckId, panel.tempWidth, cookieExpiryDays);
        }
    }
});
