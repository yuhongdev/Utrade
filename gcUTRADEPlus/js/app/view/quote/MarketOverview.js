...
//    },
                    ...
                    if (panel.compRef.summaryPie) {
                		...
                		if (newWidth > newHeight) {
                			if (global_personalizationTheme.indexOf("wh") != -1) {
                				...
                				panel.summaryPanel.add({html: panel._getLayoutHtml1().join('')});
                		} else {
                			panel.summaryPanel.add({html: panel._getLayoutHtml2().join('')});
                		}
                		if (N2N_CONFIG.features_MarketSummary_PieChart) {
    ...
    getIndexChart: function (isResize) {
        ...
        if (!isStockChart) {
            ...
            var charturl = embeddedStockChartURL + "?code=" + stockcode + '&Name=' + encodeURIComponent(stockutil.getStockName(this.exchangename)) + "&exchg=" + excode 
            				+ "&isstock=N&mode=d&color=" + c + '&lang=' + global_Language + '&View=' + (stockutil.isDelayStock(key) ? 'D' : 'R') 
                                        + '&type=mo&newOpen=' + (jsutil.boolToStr(panel.newOpen, '1', '0'));
    ...
    setLast: function (last) {
        try {
            var label = Ext.fly('lbl_ExchangeLast');
            if (label != null) {
...
//                last == 0 ? label.update('-') : label.update(last);
                ...
                var arrayCls = [N2NCSS.FontUp, N2NCSS.FontDown];
                el.removeCls(arrayCls, arrayCls);

                if (change > 0) {
                    el.addCls(N2NCSS.FontUp);//v1.3.33.42
                    img = Ext.fly('lbl_summary_updown');
                    if (global_personalizationTheme.indexOf("wh") != -1) {
                        img.update('<img src="' + sbUp + '"/>');
                    } else {
...
//	            var low = d[fieldLow];
    ...
    _updateSummaryTitle: function (obj) {
        var titleEl = Ext.get('lbl_title');
        if (global_personalizationTheme.indexOf("wh") == -1 && titleEl && obj.s && obj.d.length > 0) {
            var dt = obj.d[0];
            if (dt[fieldStkCode] == this.getIdxCode()) {
                if (dt[fieldStkFName]) {
                    titleEl.update(dt[fieldStkFName]);
                } else if (dt[fieldStkName]) {
                    titleEl.update(dt[fieldStkName]);
                }
            }
        }else if(obj.s && obj.d.length > 0){
        	var dt = obj.d[0];
    ...
    createSummaryPieChart: function () {
        ...
        if (!panel.compRef.summaryPie) {
            var summaryPie = Ext.create('Ext.chart.Chart', {
...
//                },
    ...
    _getLayoutHtml1: function () {
        ...
        htmls.push('</tr>');
        htmls.push('<tr>');
        htmls.push('<td colspan="2"><span class="icon-small icon-val" title="Total Value"></span>&nbsp;&nbsp;<span id="summary_TotalValue" style="float:right"></span></td>');
        htmls.push('</tr>');
        htmls.push('<tr>');
        htmls.push('<td colspan="2" ><span class="icon-small icon-trd" title="Total Trades"></span>&nbsp;&nbsp;<span id="summary_TotalTrade" style="float:right" class="N2N_stringStyle"></span></td>');
        htmls.push('</tr>');
        htmls.push('<tr>');
        ...
        htmls.push('</tr>');
        htmls.push('<tr>');
        htmls.push('<td><span class="icon-small icon-unchg" title="Unchanged"></span>&nbsp;&nbsp;<span id="summary_Unchanged"></span></td>');
        htmls.push('<td><span class="icon-small icon-untrd" title="Untraded"></span>&nbsp;&nbsp;<span id="summary_Untraded"></span></span><span style="margin-left: 5px;" id="summary_pie"></td>');
        htmls.push('</tr>');
        htmls.push('</table>');
        htmls.push('</td>');
        htmls.push('</tr>');
    ...
    _getLayoutHtml2: function () {
        ...
        htmls.push('</tr>');
        htmls.push('<tr>');
        htmls.push('<td colspan="2"><span class="icon-small icon-val" title="Total Value"></span>&nbsp;&nbsp;Total Value:<span id="summary_TotalValue" style="float: right"></span></td>');
        htmls.push('</tr>');
        htmls.push('<tr>');
        htmls.push('<td colspan="2" ><span class="icon-small icon-trd" title="Total Trades"></span>&nbsp;&nbsp;Total Trades:<span id="summary_TotalTrade" style="float: right" class="N2N_stringStyle"></span></td>');
        htmls.push('</tr>');
        htmls.push('<tr>');
        htmls.push('<td><span class="icon-small icon-up" title="Up"></span>&nbsp;&nbsp;<span id="summary_Up"></span></td>');
        htmls.push('<td><span class="icon-small icon-down" title="Down"></span>&nbsp;&nbsp;<span id="summary_Down"></span></td>');
        ...
        htmls.push('<tr>');
        htmls.push('<td><span class="icon-small icon-unchg" title="Unchanged"></span>&nbsp;&nbsp;<span id="summary_Unchanged"></span></td>');
        htmls.push('<td><span class="icon-small icon-untrd" title="Untraded"></span>&nbsp;&nbsp;<span id="summary_Untraded"></span><span style="margin-left: 3px;" id="summary_pie"></span></td>');
        htmls.push('</tr>');
        htmls.push('</table>');
        htmls.push('</td>');
        htmls.push('</tr>');
    ...
    _getLayoutHtml3: function () {
        ...
        htmls.push('<div ' + styleinfo + '><span class="icon-small icon-val" title="Total Value" style="margin-right:20px;display:block;float:left;"></span><span id="summary_TotalValue" style="padding-bottom:5px;display:block;"  ></span></div>');
        htmls.push('<div ' + styleinfo + '><span class="icon-small icon-trd" title="Total Trades" style="margin-right:20px;display:block;float:left;"></span><span id="summary_TotalTrade" class="N2N_stringStyle" style="padding-bottom:5px;display:block;" ></span></div>');
        ...
        htmls.push('<div ' + styleinfo + '><span class="icon-small icon-unchg" title="Unchanged" style="margin-right:20px;display:block;float:left;"></span><span id="summary_Unchanged" style="padding-bottom:5px;display:block;" ></span></div>');
        htmls.push('<div ' + styleinfo + 'margin-bottom:0px;><span class="icon-small icon-untrd" title="Untraded" style="margin-right:20px;display:block;float:left;"></span><span id="summary_Untraded" style="padding-bottom:5px;display:block;" ></span></div>');
        htmls.push('</td>');
        htmls.push('</tr>');
        htmls.push('</table>');
