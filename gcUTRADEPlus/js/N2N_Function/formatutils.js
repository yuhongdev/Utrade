...
var formatutils = (function() {
    ...
    function formatNumber(value, colWidth, forceRound, dec, optionalDec) {
        if (isNaN(value)) {
            return value;
        }
        ...
        if (roundQty) {
            if (N2N_CONFIG.round10) {
                ...
                if (Math.abs(value) >= 1.0e+9) { // billions
                    valStr = Ext.util.Format.number(value / 1.0e+9, '0,000.00') + 'B';
                } else if (Math.abs(value) >= 1.0e+6) { // millions
                    valStr = Ext.util.Format.number(value / 1.0e+6, '0,000.00') + 'M';
                } else {
...
*/	
    ...
    function SIPAccountChecking(flag){
    	...
    	if(((flag & condSIP1) == condSIP1) && ((flag & condSIP2) == condSIP2)){
    		msg = '<br>This account is allowed to trade all listed SIPs';
    	}else if(((flag & condSIP1) == condSIP1) || ((flag & condSIP2) == condSIP2)){
    		msg = '<br>This account is allowed to trade certain listed SIPs';
    	}else{
    		msg = '<br>This account is allowed to trade only EIPs.';
    	}
    ...
    function addDelaySuffix(tempStkCode) {
