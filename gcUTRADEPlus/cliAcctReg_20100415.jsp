...
<%
	...
	boolean bIndCli = false, bCorpCli = false;
	...
	if (clibean.getLoginID().trim().length() > 0) {
		...
		saResult = cli.insertNewCli(clibean);
...
<form id="frmUserReg" name="frmUserReg" action="cliAcctReg.jsp?clientType=<%=clientType%>&category=<%=category%>" method="post" onsubmit="javascript:return VerifyControl();">
	<tr>
		<td>
...
%>				
			...
			<% if((bIndCli && bNewCli) || (bIndCli && bExistCli)){ %>
			<tr height=20>
			...
			<!--jsp:include page="cliCorpAcctReg.jsp" flush="true"/-->
...
<% } /* End of if(bExistCli) */ %>
   ...
   var sAddr2 = frm.add2.value;
	...
	if (!bGoNext) {
			...
			<% 	if((bNewCli && bIndCli) || (bCorpCli) ){ %>
			frm.dob.value = formatDateToODBC(1,frm.dob.value);
			<%} %>
			return true;
	}
