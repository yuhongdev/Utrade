...
//				out.println("				var indices = new tickerConnectNew(\"indices\",\"" + g_sHTMLRoot + g_sPath + "/ref/IndicesTicker.xml\",\"" + g_sTickerType + "\",sFontSize,sWidth,sSpeed,sImgUrl);");
	...
	private void genFootnoteContents(int iTblWidth) {
		try {
			System.out.println("footnote");
			out.println("<hr align=left width='"+ iTblWidth +"'>");
			out.println("<table class=clsCopyrightBasic cellPadding=0 cellSpacing=0 width='"+ iTblWidth +"'>");
			out.println("<tr><td><font size=1 color=#990066>| <a href=javascript:OpenLinkWinNormal('"+g_sWebPath+"/corpCIMBSItrade.asp', 'winCorpCIMBSItrade', '');>About i*Trade</a> | ");
			out.println("<a href=javascript:OpenLinkWinNormal('"+g_sWebPath+"/corpContactUs.asp','winCorpContactUs','');>Contact Us</a> | ");
			out.println("<a href=javascript:OpenLinkWinNormal('"+g_sWebPath+"/web/infoPrivatePolicy.asp','winCorpDisclaimer','');>Disclaimer</a> |</font>");	
			out.println("<br><font class=clsCopyrightBasic>All rights reserved. Copyright &copy; " + dfYear.format(new Date()) + " CIMB</font></td></tr></table>");
	...
	public void GenHelpBulb(String sHelp) {
		try {
			out.println("<img id=imgHelp border=0 width=20 height=19 src="+g_sImgPath+"/lightbulboff.jpg onmouseover=this.src='"+g_sImgPath+"/lightbulbon.jpg' onmouseout=this.src='"+g_sImgPath+"/lightbulboff.jpg' title="+sHelp+">");			
		} catch (IOException ioe) {
			ioe.printStackTrace();
		}
	}
	/*
	public void genTopBanner(int iWidth){
		genBannerTop(iWidth);
		out.println("<script language=JavaScript src="+g_sJSPath+"/LinkFunc.js></script>");
		genMenu(iWidth);
		genIFrame();
	}
	
	public void genBannerTop(int iWidth){
		out.println("<script language=JavaScript src="+g_sJSPath+"/menu.js></script>");		
		out.println("<table border=0 cellspacing=0 cellpadding=5 width=" + iWidth + ">");
		out.println("	<tr>");
		out.println("		<td valign='top' width='13%'><font class='clsGrayText'>" + todayDateInDDMMMYYYY() + "</font></td>");
		out.println("		<td width='59%' rowspan='2' align='right'>");

		out.println("		<object classid=clsid:D27CDB6E-AE6D-11cf-96B8-444553540000 codebase='https://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=4,0,2,0' width=175 height=50>");
		out.println("		<param name=movie value="+g_sImgPath+"/Home/RT180x60TopRLogo.swf>");
		out.println("		<param name=quality value=high>");
		out.println("		<param name=wmode value=transparent>");
		out.println("		<embed src=/img/Home/RT180x60TopRLogo.swf quality=high wmode=transparent pluginspage='https://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' type='application/x-shockwave-flash' width=175 height=50></embed></object>");

		out.println("		</td>");
		out.println("	</tr>");
		out.println("	<tr>");
		out.println("		<td colspan='2'><font style='font-family:Arial;font-size:8pt;font-weight:bold;Color:#003366;'>Welcome " & Session("ci_LoginID") & "!</font></td>");
		out.println("	</tr>");
		out.println("</table>");
	}
	
	public void genBannerTop(int iWidth){
		Response.Write("<script language=JavaScript src=/java/others.js></scrip" & "t>" & vbCrLf)
		Response.Write("<script language=JavaScript src=/java/mm_menu.js></scrip" & "t>" & vbCrLf)

		Response.Write("<table border=0 cellspacing=0 cellpadding=0 width=""75%"">" & vbCrLf)
		Response.Write("	<tr bgcolor='#0E3055'>" & vbCrLf)
        ...
        //<!-- Research -->		
		Response.Write("				<td><a name=navResearch id=navResearch style='color:white;text-decoration:none;font-weight:bold' href=/bin/newsResearch.asp target=_blank onclick=""MM_nbGroup('down','group1','navResearch','/img/ITRADE-CIMB/button/logout_roll.gif',1)"" onmouseover=""MM_nbGroup('over','navLogOut','/img/ITRADE-CIMB/button/logout_roll.gif','',1)"" onmouseout=""MM_nbGroup('out')""><span onmouseout=""MM_startTimeout();""></span>&nbsp;&nbsp;Research&nbsp;&nbsp;|</a></td>" & vbCrLf)

		//<!-- Trading Hall -->	
		Response.Write("				<td><a name=navTradingHall id=navTradingHall style='color:white;text-decoration:none;font-weight:bold' href=/bin/qsee.asp target=_top onclick=""MM_nbGroup('down','group1','navTradingHall','/img/ITRADE-CIMB/button/tradinghall_butt.gif',1)"" onmouseover=""MM_nbGroup('over','navTradingHall','/img/ITRADE-CIMB/button/tradinghall_roll.gif','',1)"" onmouseout=""MM_nbGroup('out')""><span onmouseover=""MM_showMenu(window.mm_menu_0719160026_0,0,20,null,'navTradingHall')"" onmouseout=""MM_startTimeout();"" >&nbsp;&nbsp;Trading Hall&nbsp;&nbsp;|</span></a></td>" & vbCrLf)

		//<!-- Settlement -->
		Response.Write("				<td><a name=navSett id=navSett style='color:white;text-decoration:none;font-weight:bold' href='javascript:OpenWinStlOutstandingCliPos();' target=_top onclick=""MM_nbGroup('down','group1','navSett','/img/ITRADE-CIMB/button/settlement_roll.gif',1)"" onmouseover=""MM_nbGroup('over','navSett','/img/ITRADE-CIMB/button/settlement_roll.gif','',1)"" onmouseout=""MM_nbGroup('out')""><span onmouseover=""MM_showMenu(window.mm_menu_0719160027_0,0,20,null,'navSett')"" onmouseout=""MM_startTimeout();"">&nbsp;&nbsp;Settlement&nbsp;&nbsp;|</span></a></td>" & vbCrLf)

		//<!-- My Profile -->
		Response.Write("				<td><a name=navProfile id=navProfile style='color:white;text-decoration:none;font-weight:bold' href='javascript:OpenWinCliChgPwd(\"/gcCIMB/acctInfo.jsp\");' target=_top onclick=""MM_nbGroup('down','group1','navProfile','/img/ITRADE-CIMB/button/myprofile_roll.gif',1)"" onmouseover=""MM_nbGroup('over','navProfile','/img/ITRADE-CIMB/button/myprofile_roll.gif','',1)"" onmouseout=""MM_nbGroup('out')""><span onmouseover=""MM_showMenu(window.mm_menu_0719160028_0,0,20,null,'navProfile')"" onmouseout=""MM_startTimeout();"" />&nbsp;&nbsp;My Profile&nbsp;&nbsp;|</span></a></td>" & vbCrLf)

		//<!-- LogOut -->
		Response.Write("				<td><a name=navLogout id=navLogout style='color:white;text-decoration:none;font-weight:bold' href=/bin/cliLogout_new.asp target=_top onclick=""MM_nbGroup('down','group1','navLogOut','/img/ITRADE-CIMB/button/logout_roll.gif',1)"" onmouseover=""MM_nbGroup('over','navLogOut','/img/ITRADE-CIMB/button/logout_roll.gif','',1)"" onmouseout=""MM_nbGroup('out')""><span onmouseout=""MM_startTimeout();""></span>&nbsp;&nbsp;Logout</a></td>" & vbCrLf)
		...
		if InStr(Request.ServerVariables("SCRIPT_NAME"), "/qsee.asp") > 0 Then
				Response.Write("<scrip" & "t language=""JavaScript1.2"">mmLoadMenus(""RT"",""new"");</scrip" & "t>" & vbCrLf)
		else
				Response.Write("<scrip" & "t language=""JavaScript1.2"">mmLoadMenus(""RT"",""old"");</scrip" & "t>" & vbCrLf)
		end if
	}*/
%>
<%
if(request.getRequestURI().equalsIgnoreCase("/gcCIMB/main.jsp")) {
	session.setAttribute("FromUrl","");
}
else if (request.getRequestURI().equalsIgnoreCase("/gc/isb.jsp")) {
	session.setAttribute("FromUrl","/gcCIMB/applet/pdt_RealTimeSwing_ISB.jsp?lang=EN");
} else {
...
<SCRIPT language=javascript src="<%=g_sJSPath%>/main.js">
<%
	if (oN2NSession.getIsUserLogin()) {
//		setXmlUrl(oN2NSession.getSetting("HTMLRoot") +""+ g_sPath + "/ref/IndicesTicker_REAL.xml");
		setXmlUrl(oN2NSession.getSetting("HTMLRoot") +"/ref/gcCIMB/IndicesTicker_REAL.xml");
	} else {
//		setXmlUrl(oN2NSession.getSetting("HTMLRoot") +""+ g_sPath + "/ref/IndicesTicker_DELAY.xml");
		setXmlUrl(oN2NSession.getSetting("HTMLRoot") +"/ref/gcCIMB/IndicesTicker_DELAY.xml");
	}
%>
</SCRIPT>