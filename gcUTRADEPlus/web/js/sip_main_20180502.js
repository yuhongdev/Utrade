	...
	if(data.s&&data.s=="200"&&JSON.parse(data.ev).SIPScheduleData.length>=1){
	    ...
	    for(var i=0; i < obj.SIPScheduleData.length; i++){
			...
			var cashValue = c.toFixed(4);
				...
				if(nextbuy_date == "") sip += "<td id='nextBuy_"+num+"'>&nbsp</td>"; else sip += "<td id='nextBuy_"+num+"'>"+nextbuy_date+"</td>";
		        
		        if(status == "A") status = "ACTIVE"; else status = "CANCELLED";
...
function SIPedtlsResp(data){
	    ...
	    for(var i=0; i < obj.SIPDetails.length; i++){
			...
			sip += "<td id='refNo_"+num+"' style='display:none'>"+sipObj.RecordID+"</td>";
			...
			sip +="<th class='col-sm-1 center' scope='col'></th>"
				...
				if(sipObj.CashValue == "") sip += "<td id='cashAmount_"+num+"'>&nbsp</td>"; else sip += "<td id='cashAmount_"+num+"'>"+cashValue+"</td>";
		        if(sipObj.Qty == "") sip += "<td id='matchQty_"+num+"'>&nbsp</td>"; else sip += "<td id='matchQty_"+num+"'>"+sipObj.Qty+"</td>";
				if(sipObj.QtyMatch == "") sip += "<td id='matchPrc_"+num+"'>&nbsp</td>"; else sip += "<td id='matchPrc_"+num+"'>"+sipObj.QtyMatch+"</td>";
		        sip += "</tr>";
	    	}
	    
	    $("#tbl_sipschedule tbody").append(sip);
				sip += "<div class='btnRArea'>";
		        sip += "<div class='btn_r'><a data-dismiss='modal' class='btn_gb3 btn_s1' >Cancel</a>";
		        sip += "</div>";
		        sip += "<div class='btn_r'>";
				

		        sip += "<a href='#editSIP-step2' class='btn_gb3 btn_s1' data-toggle='tab' id='edtSIP' onclick='chkEdtNum()'>Next</a>";
		        sip += "</div>";
...
function chkEdtNum(){
		...
		var i=0;
				...
				$("#tbl_sipschedule tbody input:checkbox").each(function(){
					...
					if($this.is(":checked")){
						j++;
					var num = $( "input[type=checkbox][name=sipObjId"+i+"]:checked" ).val();
		...
		$('.icon-calendar22').attr('disabled', 'disabled');
		$('#orderMode2').val(orderMode);
		$('#orderMd').css("display","none");
		}
		else{
		var i=0;
		$("#tbl_sipschedule tbody input:checkbox").each(function(){
		...
		$('.icon-calendar22').attr('enabled', 'enabled');
...
function addSchRev(){
	...
	$('#tradingAcc2').html(splitValue[1]+"-"+splitValue[0]);
