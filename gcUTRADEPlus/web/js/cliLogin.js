...
function initLoginFormValidation(BMSSetting){
	...
	handlePasswordValidationRules(true, "C", function (data) {
		//if (data.hasOwnProperty("LoginIdMinLength"))
		//	lgMin = data.LoginIdMinLength;
		if (data.hasOwnProperty("PwdMinLength"))
			pwdMin = data.PwdMinLength;
		if (data.hasOwnProperty("PwdMaxLength"))
			pwdMax = data.PwdMaxLength;
		if (data.hasOwnProperty("PwdSecLevel"))
			pwdSecLvl = data.PwdSecLevel;
		if (data.hasOwnProperty("DispMessage"))
			validateMessage = data.DispMessage;
	});
	...
	//frmForceChgHint validation
	$("#frmForceChgHint").validate({
		errorElement:'span',
		errorPlacement: function(error, element) {
		    error.insertAfter(element);
		    error.addClass('text-error-top');
		...
		rules:{
			frmForceChgHint_hintType:{
				required:true
			},
			frmForceChgHint_hint:{
				required:true				
			},
			frmForceChgHint_hintAns:{
				required: true
			}
		},
		messages:{
			...
			frmForceChgHint_hint:{
				required:"Please enter hint question."			
			},
			frmForceChgHint_hintAns:{
				required:"Please enter hint answer."
			}
...
function chgLayout(layout){
	$("#divLogon, #divLogin, #divForgetPwd, #divForgetPin, #divForceChgPwd, #divForceChgPin, #divForceChgHint").hide();
	switch(layout){
		case 1: //logon view
			$("#divLogon").show();
			break;
...
function handlePasswordValidationRules(byCategory, filterVal, callback) {
	// filterValue: B (BMS user), C (retail client), D (dealer account)
	$.ajax({
		method: "GET",
		url: `${getContextPath()}/srvs/getPwdSecLevel`,
		dataType: "json",
		async: false,
		data: {
...
$(document).ready(function(){
	...
	$("#frmForceChgHint_hintType").change(function(){
		if($("#frmForceChgHint_hintType").val()=="OTR"){
			$("#frmForceChgHint_hint").show();
		}else{
			$("#frmForceChgHint_hint").hide().blur();
		}
	});
});