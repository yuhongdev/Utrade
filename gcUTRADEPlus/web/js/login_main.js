...
var chainPwdPin = false;
var chainPwdHint = false;
var chainPinHint = false;
var chainAll = false;
...
function chkSessResp(data){
	...
	if(data.s&&data.s=="200"){
		...
		if(logon.s=="true"){
			...
			if (logon.ctgy=="T") {
	            $("li:contains('Change PIN')").addClass("hidden");
	            $("li:contains('Forgot PIN')").addClass("hidden");
			}
        ...
        if (!chkAction()) {
            ...
            if (!activateFlag) {
                ...
                if (logon.ctgy == "T") {
                    ...
                    if (window.location.pathname == "/gcUTRADEPlus/web/html/cliAcctUpgrade.html") {
                        window.location.href = "/gcUTRADEPlus/web/html/cliOpenAcctReg.html?type=1";
                    }
                    else if (window.location.pathname == "/gcUTRADEPlus/web/html/cliLoginActivate.html"){
...
function chgPwdResp(data){
	...
	if(data.s&&data.s=="200"){
		alert(data.msg);
		if (loginTrigger&&chainPwdPin || loginTrigger&&chainAll) {
			//alert("This will change layout");
			loginAction = "2";
			chkAction();
		}

		else if (loginTrigger&&chainPwdHint) {
			loginAction = "4";
			chkAction();
		}

		else {
			alert("Please re-login using your new password.");
			loginAction = "0";
			forceLogout = true;
		...
		if(!chkAction()){
			//console.log("if chkaction false fired");
			chgLayout(1);
		}
	}else{
		alert(data.msg);
...
function chgPinResp(data){
	...
	if(data.s&&data.s=="200"){
		alert(data.msg);
		if (loginTrigger&&chainPinHint || loginTrigger&&chainAll) {
			//alert("This will change layout");
			loginAction = "4";
			chkAction();
		}

		else if (loginTrigger&&chainPwdPin) {
			alert("Please re-login using your new password.");
			loginAction = "0";
			forceLogout = true;
			chkAction();
...
function chgHint(){
	var jsonObject = {};
	jsonObject['ht'] = $("#frmForceChgHint_hintType").val()=="OTR"?$("#frmForceChgHint_hint").val():$("#frmForceChgHint_hintType").val();
	jsonObject['hta'] = $("#frmForceChgHint_hintAns").val();
	console_log("chgHint:"+JSON.stringify(jsonObject));
	chgHintReq(jsonObject);
}
function chgHintResp(data){
	console_log("chgHintResp:"+JSON.stringify(data));
	if(data.s&&data.s=="200"){
		alert(data.msg);
		if (loginTrigger&&chainPwdHint || loginTrigger&&chainAll) {
			alert("Please re-login using your new password.");
			loginAction = "0";
			forceLogout =  true;
			chkAction();
...
function logoutResp(data){
	...
	if(data.s&&data.s=="200"){
		...
		parent.location.href = loginPage_url;
	}else{
		alert(data.msg);
	}
}
